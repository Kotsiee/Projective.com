...
interface OwnerBucket {
	 ...
	 *
	 * Keyed by uuid rather than by slug for the same reason `setups` is: the modal navigates to the
	 * canonical id, a title-derived slug moves on the first rename, and a store keyed by the mutable
	 * half would lose a project through the very address its owner just edited.
	 */
...
export function recordCreatedProject(owner: string, setup: ProjectSetup): void {
	bucketFor(owner).created.set(setup.id, { id: setup.id, setup, createdAt: Date.now() });
}
...
export function storedCreatedProject(owner: string, projectKey: string): ProjectSetup | undefined {
	return findCreated(peekBucket(owner), projectKey)?.setup;
}
...
export function stubSlugTaken(owner: string, slug: string): boolean {
	...
	for (const record of bucket.created.values()) {
		if (record.setup.slug === slug) return true;
	}
	return false;
}
// #endregion
...
export function createdSetup(ref: string, actor?: ReadActor): ProjectSetup | null {
	...
	const record = findCreated(peekBucket(owner), ref);
	if (!record) return null;
	// Through {@link storedSetupFor} with BOTH of the record's own identifiers, never the reference
	// that found it and never one of the two alone. Edits are keyed by the canonical uuid, so a lookup
	// by slug reads back an UNEDITED project — which is how an archived draft went on appearing in the
	// feed after the archive had been recorded perfectly well one map away.
	const stored = storedSetupFor(owner, [record.setup.id, record.setup.slug]);
