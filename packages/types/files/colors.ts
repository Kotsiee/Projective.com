import type { ColorSummary } from "./metadata.ts";
...
/*
 * Lives in `@projective/types/files` (moved from `apps/web/features/files/core/media/`, 2026-09-22) so the
 * server-side media pipeline and the browser extractor run ONE encoder: a placeholder written by the
 * pipeline and one written by the browser must be indistinguishable to every reader. The app keeps a
 * re-export at its old path, so nothing that imported it there changed.
 */
