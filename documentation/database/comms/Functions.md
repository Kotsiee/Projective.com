...
> the signature — leaving both in place would make a six-argument call ambiguous
> (_"function is not unique"_).
...
5. **global toggles**.
6. **quiet hours** — drops `push`/`sms` only (the inbox and email still land), unless the catalog row
   sets `overrides_quiet_hours`.
7. **digest deferral** — a digestible, `low`/`medium` event's **email** is rolled up instead of sent.
   The in-app row is unaffected; the inbox is always real time.
...
Timezone-aware and midnight-crossing aware (`start > end` means the window wraps). Honours the legacy
absolute `quiet_hours tstzrange` as an additional window. An invalid IANA zone degrades to UTC rather
than raising.
