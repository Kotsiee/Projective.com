...
| Pattern                 | File                                                         | URL                                                                                                                                                                                                                                                                           |
| :---------------------- | :----------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| project create          | `projects/create.tsx`                                        | `/projects/create` — the **six-step creation wizard** (Details · Legal & Screening · Stages · Timeline · Budget & Staffing · Review & Publish, per [`../flows/Projects.md`](../flows/Projects.md)). It renders a page, not a redirect: the earlier 307 shim back to `/projects` (which hosted the create modal) is retired. The file must stay a **static** sibling of `[projectId]`, because Fresh resolves a static segment ahead of a dynamic one and without it `create` would be captured as a project slug — so this route is load-bearing even on a day it renders nothing. The step is client state, not a URL segment: one address, one surface, and a half-filled draft is not a shareable link. |
