...
| timeline (project)      | `projects/[projectId]/timeline.tsx`                          | `/projects/:projectId/timeline` (Timeline / Gantt — one lane per stage, the stage's scheduled window + every dated ticket, dependency links; a ONE-OFF engagement's "board" view link lands here. A projection over the board read, so a bar and a card cannot disagree about a date; root `CLAUDE.md` §8 Decision #91) |
| timeline (channel)      | `projects/[projectId]/[channelId]/timeline.tsx`              | `/projects/:projectId/:channelId/timeline` (one stage's Gantt — the stage's window, then one lane per ticket; the channel header's **Timeline** tab, gated exactly like Tasks) |
