...
| **Project invitation**     | `pending → accepted` (the seat is taken) / `declined` (the invitee's answer) / `expired` (time) / `revoked` (the inviter's act). `declined` starts a **48-day re-invitation cooldown** for that `(project, invitee)` pair, counted from `declined_at` — the fat service refuses a new invitation to that person on that project until it lifts, and the profile's Add-to-project rows render it locked with the date. `revoked` starts none. Outbound invitations are further capped per acting identity at 10 per sliding 10 minutes (`HIRE_RATE_LIMIT`) | `projects.project_invitations.status` / `declined_at` · `packages/types/projects/hire.ts` (`activeInviteCooldown`, `hireInvitationRefusal`) · `PRODUCT_SPEC.md` §The Hiring Process |
