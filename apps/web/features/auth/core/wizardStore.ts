 ...
 */
function buildSteps(accountType: Signal<AccountType>, intent: Signal<UserIntent | "">): StepMeta[] {
	const org = accountType.value === "organization";
	const steps: StepMeta[] = [
		{ id: "account", tag: "1", label: "Account", optional: false },
	];
	...
	steps.push({ id: "credentials", tag: "1.6", label: "Finish", optional: false });
	return steps;
}
...
export function createJoinStore(init: JoinInit): JoinStore {
	const accountType = signal<AccountType>(init.accountType);
	...
	const steps = computed(() => buildSteps(accountType, intent));
