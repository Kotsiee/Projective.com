...
import { ProductCard, type ProductCardLayout } from "./ProductCard.tsx";
 ...
 * `productLayout` reaches only a product: a host laying cards in a uniform-height rail or carousel
 * passes `"fixed"`; a masonry host leaves it unset.
 */
export function EntityCard(
	{ item, ctx = { scope: "explore" }, onSelect, authed = false, productLayout }: {
		item: ExploreItem;
		ctx?: HrefContext;
		onSelect?: (item: ExploreItem) => void;
		authed?: boolean;
		productLayout?: ProductCardLayout;
	},
): JSX.Element {
	switch (item.type) {
		case "users":
