...
/** `GET /api/wallet/invoices` — thin route: the Invoices projection (business statements · bills · caps), read as the signed-in viewer. */
export const handler = define.handlers({
	async GET(ctx) {
		const context = asAuthenticatedContext(ctx.state.userContext);
		return toWalletResponse(
			await WalletBackendService.invoices(walletQueryFrom(ctx.url.searchParams, context), readActor(ctx)),
		);
	},
});
