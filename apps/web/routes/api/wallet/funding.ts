...
/** `GET /api/wallet/funding` — thin route: the Funding projection (sources · recurring rules · balance). */
export const handler = define.handlers({
	GET(ctx) {
		const context = asAuthenticatedContext(ctx.state.userContext);
		return toWalletResponse(
			WalletBackendService.funding(walletQueryFrom(ctx.url.searchParams, context)),
		);
	},
});
