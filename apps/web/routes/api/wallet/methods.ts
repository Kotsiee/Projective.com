...
/** `GET /api/wallet/methods` — thin route: the payment-methods projection (spend/earn/both · defaults), read as the signed-in viewer. */
export const handler = define.handlers({
	async GET(ctx) {
		const context = asAuthenticatedContext(ctx.state.userContext);
		return toWalletResponse(
			await WalletBackendService.methods(walletQueryFrom(ctx.url.searchParams, context), readActor(ctx)),
		);
	},
});
