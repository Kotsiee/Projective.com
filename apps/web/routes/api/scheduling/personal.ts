 ...
 *
 * `ctx.state.isAuthenticated` is a skeleton presence check (root CLAUDE.md §8 Decision #14); the
 * read itself runs as the caller under RLS, which is the real gate, and the service's per-viewer
 * projection is the second lock behind it.
 */
export const handler = define.handlers({
	async GET(ctx) {
		...
		return toSchedulingResponse(await ScheduleBackendService.personalCalendar(readActor(ctx)));
	},
});
