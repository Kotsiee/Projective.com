...
export default define.page<typeof handler>(function ChannelCalendarPage(ctx) {
	const { projectSlug: projectId, channelId } = ctx.params;
	const { page: initial } = resolveCalendarPage(projectId, channelId, viewerFromState(ctx.state));
	return (
		<ProjectCalendar
			scope="channel"
			projectId={projectId}
