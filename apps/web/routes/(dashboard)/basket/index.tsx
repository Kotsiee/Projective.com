...
import BasketScreen from "@features/checkout/islands/BasketScreen.island.tsx";
import { resolveBasket } from "@features/checkout/core/checkout-ssr.ts";

/**
 * `/basket` — the acting account's basket.
 *
 ...
 * personal/team/business basket SSR-paints in the first byte. The lane, the header band and the footer
 * band are resolved separately by their own slot resolvers in the dashboard layout — this route owns
 * the BODY only, which is the region contract working as intended.
 *
...
export default define.page<typeof handler>(function BasketPage({ data }) {
	return <BasketScreen initial={data.bootstrap} />;
});
