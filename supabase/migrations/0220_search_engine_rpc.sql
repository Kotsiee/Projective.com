-- =============================================================================
-- 0216_search_engine_rpc.sql
-- The ranking engine: weighted scoring matrix + admin analytics.
--
-- ARCHITECTURE DECISION — Postgres RPC over Rust/WASM:
--   The scoring matrix fuses pgvector cosine similarity (`<=>`), full-text rank
--   (`ts_rank`), and JOIN-derived signals from six schemas (org / marketplace /
--   projects / security / reviews / search). Doing this in Postgres keeps the
--   candidate set, the vector index (HNSW), and the weight config co-located —
--   one planned round trip, no serialisation of large candidate sets across the
--   WASM boundary. A Rust/WASM module would only help for embedding *generation*
--   (a future concern), not for this JOIN-heavy ranking. Weights live in
--   search.search_weights so behaviour is tunable live, without redeploying.
--
-- Exposed (anon + authenticated): fn_search_entities, fn_search_global, fn_log_query.
-- Admin-only (internal is_admin guard): fn_search_analytics, fn_get_weights, fn_set_weight.
-- =============================================================================

-- Normalisation helper: clamp value/cap into [0,1]. Params are `double precision`
-- so callers can pass ts_rank() (double), numeric ratings, or integer caps — all
-- up-cast implicitly (double->numeric is only an assignment cast, so this direction
-- avoids the "function does not exist" resolution failure).
DROP FUNCTION IF EXISTS search.fn_norm(numeric, numeric);
