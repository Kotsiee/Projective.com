...
| 88 | **Route addresses become prefixed opaque slugs — `/projects/prj-…`, and the uuid stops routing (2026-09-06).** A project was addressable two ways: by `projects.projects.id` (what the Quick-Init modal navigated to) and by a TITLE-DERIVED `slug`, with `ck_projects_slug_shape` as the permissive `^[a-z0-9-]{1,96}$`. Both are wrong, in opposite directions. A title-derived slug MOVES on the first rename, so every link built on it — a notification, a bookmark, another member's message — dies the moment the owner edits the title, silently and with a clean 404; deriving an address from mutable content is the defect, and shortening it is not the fix. A uuid never moves but is 36 characters of undifferentiated hex, so it says nothing about what it addresses and a segment pasted into the wrong route resolves against the wrong table with no shape to refuse it. **Product owner chose a HARD CUT (2026-09-06): `prj-…` and nothing else routes** — the uuid and every old title slug now 404. **(A) THE FORMAT** is one SSOT, `@projective/types/slugs`: prefix (`prj`/`stg`/`svc`/`ssn`) + 10 symbols from a 32-character alphabet, minted from `crypto.getRandomValues`, derived from NOTHING — which is what makes it idempotent across every future edit, because there is no input that could change. **32 is load-bearing, not aesthetic**: 256 is a whole multiple of 32, so `byte % 32` is uniform, where the 31-symbol alphabet you get by ALSO dropping `o` needs rejection sampling and the modulo written without it — the form somebody reaches for first — is silently biased toward the first symbols. A biased address still routes, so nothing would ever have reported it. One member of each confusable group is dropped rather than the whole group (`0` goes, `o` stays and is unambiguous BECAUSE `0` is gone); a docblock claiming otherwise was caught by its own test and corrected in three places. **No `Math.random()` fallback**: `getRandomValues` exists everywhere including insecure origins, so the only way to reach a fallback is an environment already badly wrong, and quietly minting a guessable address there is worse than throwing. **(B) SCHEMA, folded in place** (§1, no new timestamped files): `slug` on `projects.projects` (retightened), `projects.project_stages`, `marketplace.service_blueprints`, `projects.session_events` — each `NOT NULL`, UNIQUE, and CHECKed to the EXACT minted shape. **Tight rather than permissive on purpose**: the old CHECK still ACCEPTED a title slug, so "immutable and title-independent" rested on every write path remembering to be; the database refuses it now, so the guarantee holds for paths nobody has written yet, and a lowercase uuid — which satisfied the old form — can no longer be confused for an address. **`NOT NULL` with NO `DEFAULT`**, because `security.fn_slug_guard` (one `BEFORE INSERT OR UPDATE` trigger per table, `00001890_triggers_slugs.sql`) fills a missing slug BEFORE the `NOT NULL` is checked and RAISES on any UPDATE that would move an existing one. A `DEFAULT` could do neither job: it may not contain a subquery, so ten uniform symbols inline would be a ten-line expression copied into four tables, and defaults are laid down in category 0 before any function exists to call. **Verified by execution, not inspection** — all 88 migrations + every seed applied clean, then: all four tables mint canonically with no slug supplied; a rename does not move a slug; a RESCHEDULE does not move a session's; all four refuse an explicit mutation; a title slug is unstorable; a duplicate raises `23505` carrying the constraint name the retry matches on. **(C) THE RETRY** (`packages/backend/core/slug-retry.ts`) is on the CONSTRAINT, never on `23505` alone. Checking availability first costs a round trip whose answer is stale before the insert runs, so the unique index at the moment of the write is the only authority — and a handler that retried any unique violation would spin a caller's REAL conflict (a duplicate primary key) five times against fresh slugs and then report it as an address problem, telling them to fix the one thing that was fine. `insertWithSlugRetry` is generic over the caller's own result type and takes a `PromiseLike`, because a PostgREST builder is a thenable and forcing it through a narrower shape widens `error.message` to `string | null | undefined` at every call site. **(D) THE BLAST RADIUS**: ten resolvers dropped their uuid-vs-slug branch; the route directory is `[projectSlug]` (18 param reads); `create_project` mints instead of slugifying; and the 16 fixture project slugs migrated (48 occurrences, 12 files) so the stub corpus cannot show a shape the database refuses. **SIX DEFECTS FOUND BY MEASUREMENT, NOT INSPECTION, and the two worst were mine.** (1) `applyProjectUpdate` re-read its projection with `row.id` through a resolver I had just made slug-only, so **every successful save committed its write and then answered "No project found"** — the write landed, the owner was told it had not, and nothing in the type system had an opinion. Fixed by splitting the lookup in two by NAME (`fetchSetupProject` by slug, `fetchSetupProjectById` by primary key): a single resolver accepting both is what let a route key and a row id look identical at the call site. (2) The write store still answered to the uuid, making the DEFAULT mode strictly more permissive than the one that ships — a URL that works all through development and 404s the day the gate is turned on. (3) `QuickInitModal` handed `onCreated` the uuid, so a successful create navigated to a 404; its docblock argued FOR the uuid on reasoning the new format retires. (4) Two sidebar links, (5) three wallet-ledger links and (6) a simulated booking href were dead — found by auditing every rendered `a[href^="/projects/"]` in a browser, which caught what a token-level search could not: slugs embedded inside longer string literals. Three of those were pre-existing (`atlas-landing-sprint` names no project anywhere), and the prefix is what makes that class visible now — an id that is not `prj-` cannot be a project, and it no longer takes a click to find out. **A SEVENTH was a latent fixture defect this change EXPOSED rather than caused**: `hash` multiplies by 31 and 31 ≡ 1 (mod 3), so ids differing by a trailing digit walk a `% 3` bucket in lockstep, and any STRIDE through them lands on one residue forever — the scheduling corpus lost every `pending` ballot and then every open vote. The file's own docblock records this biting once before on the neighbouring key. Fixed at the class: a `mix32` avalanche beside `hash`, and the ballot archetype + negotiation gate moved from a hash to the event's POSITION, because three buckets drawn by hash over the five group votes a calendar actually has miss one about two times in five. Coverage is now a property of the construction. **VERIFIED END TO END against the live local stack**: create → `prj-z935vjrcfm` (not derived from "Website Refresh") with its stage at `stg-335mkttp4r`; that URL renders, the uuid does not; rename → title changes, slug does not, URL still resolves; `/board`, `/files`, `/members` all render; every `/projects/` link on seven surfaces is canonical (0 non-canonical); API endpoints resolve by slug and refuse the uuid. Probe rows removed. `deno task check` + `deno lint` (1,501 files) clean, **1,016 unit tests** (28 new: generator properties, retry semantics, and a contract test that READS the migrations and fails if the SQL alphabet, body length, CHECKs or trigger prefixes drift from the TypeScript). **FLAGGED — needs a human, do NOT silently resolve:** (a) **RESOLVED by Decision #90 (same day, at the product owner's direction).** `projects.create_project` was BROKEN at HEAD — it inserted into `nda_mode`, a column that does not exist (the table stores the pair `nda_required` + `nda_source`), so every call raised `42703`. Flagged here rather than fixed because it is not a rename and the value domains differ; on inspection the answer was already made by the schema and the SSOT, and the real defect was a missing mapping function. See #90. (b) A missing project returns **200 with a "not found" body**, not a 404 — pre-existing role-dispatcher behaviour (#85), untouched. (c) The `northern-grid`/`coastal` workspace fixture tenants link to `/projects/` + their own demo ids, which name no project in the corpus and 404 — pre-existing, and left alone rather than repointed at unrelated real projects. (d) The route directory rename needed a plain `mv`; `git mv` fails with `Permission denied` while a dev server holds the directory handle. | root `CLAUDE.md` §1/§2/§6/§7 · `packages/types/slugs/` (+ `slug.test.ts`, `slug.contract.test.ts`) · `packages/backend/core/slug-retry.ts` (+ test) · `packages/backend/services/projects/{project-identity,live-*,write-store,draft-store,fixtures,ProjectBackendService}.ts` · `packages/backend/services/scheduling/{derive,coordination-fixtures,calendar-fixtures,availability-fixtures,schedule-fixtures}.ts` · `packages/backend/services/{finance/wallet-fixtures,booking/BookingBackendService}.ts` · `apps/web/routes/(dashboard)/projects/[projectSlug]/**` · `apps/web/features/projects/{components/QuickInitModal,islands/ProjectsLane.island}.tsx` · `apps/web/features/shell/core/nav-fixtures.ts` · `supabase/migrations/{00000014,00000015,00001001,00001100,00001890}*` · `documentation/database/{projects,marketplace,security}/*` · Decisions #3 / #21 / #48 / #71 / #82 / #83 / #84 / #85 |

| 89 | **Post-onboarding immutability, the publish confirmation, and the stage Details tab (2026-09-06).** A client could rewrite the terms a freelancer had already accepted. The project's delivery shape and every stage's ticket price were editable for the life of the engagement, so the work being done and the work that was agreed could silently diverge with nothing on any screen to say so. Both now freeze on the first onboarding, the client is told BEFORE it happens, and the rule is enforced where it actually holds. **(A) ONE IMPLEMENTATION, THREE READERS.** The rule lives in `packages/types/projects/setup.ts` (`shapeLocked` · `lockedStagePriceIds` · `projectPriceLocked` · `priceLockReasonFor` · `PUBLISH_LOCK_NOTICES`) and is read by the form that disables the control, the dialog that promises the lock, and the write path that refuses the mutation. A second copy is precisely how a disabled input and a server refusal come to disagree about the same field — and the dialog's three sentences are a constant rather than JSX literals, so it cannot promise a lock the guard does not apply. New SERVER-derived `onboardedCount` on both `StageSetup` and `ProjectSetup`; `reconcileSetup` **re-grafts the stage counts from the base by id and ignores the patch entirely**, so a payload cannot unlock the very field it is trying to change — the treatment `completeness` and `liveVisibility` already get. **(B) THE SHAPE LOCK IS BOTH CONTROLS.** `format` and `structure` are the two halves of one answer (`createFormatToColumns`), so freezing the Project type and leaving the has-stages toggle open would be a lock a caller walks around — turning a staffed pipeline flat strands every provider hired onto stages 2..n. `sessionKind` is deliberately NOT frozen: it is a detail inside a format, and its control is only reachable while the format is `session`, which this rule already holds still. **(C) THE PRICE LOCK IS PER STAGE**, so a run whose second milestone is staffed can still be priced at its fourth. A FLAT engagement is the exception and locks its ROOT stage on the PROJECT's count, because that stage IS the engagement (`fn_hold_ticket_escrow` reads its `unit_price_cents`) — and its leftover rows from a previously staged run are honoured too, even though no section renders them, or a hidden stage could be repriced out from under somebody working it. Everything else about a staffed stage stays editable: scope, checklist and skills are clarifications a provider benefits from. **(D) A FIFTH STATUS SET OVER `stage_assignments.status`, and it is a different question.** The four that exist ask whether somebody has access, appears in a roster, or holds the seat now (`live-members` · `live-detail` · `live-submissions` · the partial index). This asks whether anybody EVER took it — an agreement survives the person leaving, and escrow may already have moved — so `released`, `cancelled` and `completed` all KEEP the lock. A DENY-list over free text, and the direction is the point: an unrecognised value must LOCK, because over-locking withholds an edit somebody can ask about while under-locking reprices agreed work invisibly. That is the reverse of `live-submissions`'s reasoning and not an inconsistency — the consequence of guessing wrong is reversed too. Only two states never onboarded anybody: `declined`, and `pending_funding` (Decision #80 documents it as "nobody is committed to anything and no escrow exists"; counting it would price-lock a blueprint draft from birth). It agrees with the shipped sibling rule, `projects.fn_stage_reorder_lock`, on every other value — two locks on one stage that disagreed about when it was "started or claimed" would be indefensible. **(E) THE GUARD RUNS ON BOTH BRANCHES.** `onboardingLockRefusal` is called by the live path and by the stub, from the same module — `PROJECTS_BACKEND_LIVE` ships OFF, so a lock the form draws and the stub then accepts would be a control that lies in the configuration most people run. **A field sent UNCHANGED is never refused**, and that is load-bearing rather than a nicety: `toPayload` sends the whole form on every save, so refusing on presence would make a staffed project unsavable in any respect the moment its first freelancer joined. **(F) A BLOCKING GAP CLOSED ADDITIVELY.** `StageChannel` carried no `stageId`, and on the LIVE path its `id` is a `comms.project_channels` id while `StageSetup.id` is a `projects.project_stages` id — so a surface routed to a channel had no way back to the stage's configuration. The fixtures make the two strings EQUAL, which is exactly why this could not be found by using the stub: a lookup on `id` works there and finds nothing in production. `stageId` is now projected by `buildStageChannels`, the one place that holds both. **(G) THE STAGE DETAILS TAB** (`/projects/[project]/[channel]/details`) is a per-stage configuration form for the owner or an admin. `StageFields` was EXTRACTED from the stage accordion rather than rewritten, so the two surfaces are one component tree; the page seeds the SAME `setup-state.ts` store, which is where Ctrl+S, the collapsing save serialiser and auto-save-on-blur already live (both extracted into shared hooks). Tab visibility and the route guard call ONE predicate, `canConfigureStage` — a tab hidden by one rule and a route guarded by another is a link that disappears while its URL stays open. The guard sits in `define.handlers`, never the page component (Decision #61), and answers **303** to the channel one level up rather than a dead-end 403. `canConfigure` is deliberately NARROWER than `isReviewer`: reviewing work and rewriting the terms it is done under are not the same seat. **(H) DEV PARITY (§5).** A `projectOnboarding` axis (`auto`/`none`/`first_stage`/`all_stages`) travels as a validated `sim` query param on BOTH the setup read and the save — not a client overlay, because the counts decide what the write path accepts and simulating them only on the client would draw an unlocked control the save then refuses. Fixtures-only, and discarded outside development by a SERVER-read `DENO_ENV` (the shape Decision #72 had to retrofit after an ungated overlay proved to be a privilege-forgery primitive). **DEFECTS FOUND BY MEASUREMENT, NOT INSPECTION:** (1) adding the tab left `TAB_ICONS` without a `details` entry, and the header does `cloneElement(TAB_ICONS[tab.key])` — `cloneElement(undefined)` THROWS, so the channel header island would have crashed on every channel route for an owner; (2) `PROJECT_PRICE_LOCK_REASON` was documented for the flat case but `projectPriceLocked` is gated on `pricedAtProjectLevel`, so a flat project explained its frozen price by naming a "stage" on a surface whose stage list is switched off (`priceLockReasonFor` now picks by shape); (3) the collapsed stage row still said "Needs pricing" beside a control explaining it can never be priced again; (4) the simulation reverted on the first save, because the response was built from the unsimulated base and the form adopts it as its new baseline; (5) `auto-save` would have been silently dead on the new page — `hydrateAutoSave()` was only ever called by the project rig, so an owner who had turned it on would watch nothing save. **VERIFIED IN-BROWSER** on the real fixture corpus across all four shapes: a staged run with stages `[3,1,2,2,3,3,0]` locks six prices and leaves the seventh editable; a flat engagement locks its root and says "onto this project"; a role-staffed one locks its budget; a draft locks nothing. The publish dialog opens with all three notices verbatim, `role="alertdialog"`, focus on Cancel; Cancel leaves `status: draft` / `unlisted`; Confirm moves it to `active` with the visibility promoted and Publish correctly gone. On the Details tab: a locked stage's price is disabled with the lock copy, an unstaffed stage's is editable, editing raises Save/Discard in the footer band, a non-owner is REDIRECTED and never sees the tab, and a general channel neither offers it nor renders a form. The write path was separately exercised head-on: the three frozen mutations answer `422 field_locked_post_onboarding` while a rename, a stage rename and the whole form re-sent unchanged all save. `deno task check`, `deno task lint` (1,501 files) and `deno task test:unit` (**1,016 passed / 0 failed**, 30 new) all green. **FLAGGED — surface, do not silently resolve:** (a) **EVERY SAVE, PUBLISH AND ARCHIVE FROM THE SETUP SURFACE WAS ANSWERING 404, and the cause was not this change** — found here, fixed here, and worth recording because of HOW it hid. `commit(draft.id, …)` sent the project's UUID, unchanged from HEAD; Decision #88 landed concurrently and retired the uuid as an address. That decision's own sweep audited every rendered `a[href^="/projects/"]` in a browser, which is thorough and structurally could not see this one: it is a `fetch`, not an anchor. Four call sites now send `draft.slug`, and the docblock arguing FOR the uuid — "renaming the project regenerates its slug" — is retired by that same decision's immutable, title-independent slug. Measured before and after: `404` on every save, `200` after. (b) **A named role's `budgetCents` has no lock**, because nothing counts onboarding per ROLE — inventing a count with no backing data would be fabricating one; the task's "cannot delete a role with assigned freelancers" needs that column first. (c) **Deleting a staffed STAGE is not guarded**, only repricing it — and `fn_stage_delete_cascade` releases that stage's escrow. Out of this brief, and closing it is a product decision about whether a refund flow exists. (d) `cancelled` keeps the lock, so an assignment created and immediately cancelled by mistake leaves the stage permanently priced — the safe side of a rule whose other side is invisible, but unpicking it needs a distinction the column does not carry. (e) The route folder is `[projectSlug]` because that same in-flight rename moved it; if the rename is reverted the new route needs two edits (its path and its param name). | root `CLAUDE.md` §2/§3/§5/§6/§7 · `PRODUCT_SPEC.md` §Stage Management #4 · `PRODUCT_MANAGEMENT.md` §3.5 · `packages/types/projects/{setup,detail}.ts` + `setup.test.ts` · `packages/backend/services/projects/{live-writes,setup-fixtures,ProjectBackendService,live-detail,detail-fixtures}.ts` + `{live-writes,setup-fixtures}.test.ts` · `apps/web/features/projects/{core/{channel-view,channel-header-slot,setup-state,ProjectSidebarService,stage-details-footer-slot},components/setup/SetupSections,hooks/*,islands/{ProjectSetupRig,ProjectSetupForm,ChannelHeader,StageDetailsForm,StageDetailsRig}}` · `apps/web/routes/(dashboard)/projects/[projectSlug]/[channelId]/details.tsx` · `apps/web/routes/api/projects/{setup,[id]}.ts` · `apps/web/utils/dev-seam.ts` · `apps/web/features/devtools/*` · Decisions #21 / #35 / #61 / #64 / #65 / #72 / #80 / #84 / #85 |
| 90 | **`projects.create_project`'s phantom `nda_mode` column — resolved (2026-09-06). CLOSES Decision #88 flag (a).** The RPC's INSERT named `nda_mode`, so **every call raised `42703 column "nda_mode" of relation "projects" does not exist`** — the one atomic write behind `POST /api/projects/create` could not work at all. It was invisible for two reasons worth separating: the statement is a STRING, so no type-checker sees it, and the app's create path does a direct insert rather than calling the RPC, so nothing in the product exercised it. #88 flagged it as needing a human because the two vocabularies are not a rename — `projects.nda_mode` is the enum `none|platform_standard|custom`, the column is `nda_source` with `platform|custom` — and picking what `nda_source` holds when no NDA applies looked like a product decision. **It is not: the schema and the SSOT had already made it, and the real defect was a missing function.** `projects.projects` stores the mode as a PAIR — `nda_required` (does one apply) and `nda_source` (which instrument) — which is isomorphic to the enum plus one state the enum cannot reach, and `@projective/types/projects` already carried TWO thirds of the mapping (`ndaRequiredFor`, `ndaDocumentFor`, both DEAD: their only legitimate caller is a `plpgsql` function that cannot import TypeScript). The third, mode → source, was never written — which is precisely why the RPC had nowhere to put the mode and wrote it to a column name instead. **`ndaSourceFor` completes the trio**: `custom` → `custom`, everything else → `platform`. `none` maps to `platform` on the schema's own authority, not a judgement made here — the column is `NOT NULL DEFAULT 'platform'`, its comment calls that "the answer that needs no upload and no legal review, and therefore the default", `DEFAULT_PROJECT_RULES` already pairs `ndaRequired: false` with `ndaSource: "platform"`, `ProjectRules.ndaSource` documents itself as "meaningless, and ignored" while the boolean is false, and the READ projection already collapses anything not-`custom` to `platform`, so a row written this way reads back as the mode it was created under. There is no third value to reach for: two members, `NOT NULL`. **The document is deliberately NOT normalised.** `ndaDocumentFor` DROPS a reference for a non-custom mode; the RPC passes it through and lets `ck_projects_nda_document` refuse. Those are not in conflict — the SSOT function is an UPDATE rule, for tidying a document away after a mode change, and on CREATE there is no earlier document, so a payload naming one alongside a mode that forbids it is a caller contradiction rather than a leftover. Refusing it is the same philosophy the neighbouring `allow_deadline_bonuses` comment already states. **A CONTRACT TEST is the actual fix** (`nda.contract.test.ts`, the `slug.contract.test.ts` technique): it reads the migrations and asserts the INSERT's column list names `nda_source` and not `nda_mode`, that both halves of the pair are written together, that `projects.projects` declares no `nda_mode` COLUMN, and that the SQL's two derivations are character-identical to `ndaRequiredFor`/`ndaSourceFor`. The column list is EXTRACTED rather than substring-matched, because `nda_mode` appears legitimately throughout that file as a cast, a variable and prose — only its appearance in the INSERT is the defect. **Confirmed to FAIL against the original bug** (2 of 7 red when the old column name is put back), because a test that has never failed proves nothing. **VERIFIED BY EXECUTION**, not inspection: after a full `db reset` (all 88 migrations + every seed clean), all six payload shapes through the real RPC as `authenticated` produce the right pair — `none` → `(false, platform)`, `platform_standard` → `(true, platform)`, `custom` → `(true, custom)`, legacy `nda_required: true` → `(true, platform)`, legacy `false` and absent → `(false, platform)` — `custom` + a real `files.items` id stores the document, and both contradictory payloads (`none` and `platform_standard` carrying a document) are refused by `ck_projects_nda_document`, 2 of 2. `create_project` now returns `{id, slug}` and provisions its stage on a from-scratch schema. **Also corrected, all stale rather than wrong-by-design:** `NdaMode`'s docblock claimed the column `projects.projects.nda_mode`; `ndaDocumentFor`'s claimed a `CHECK (nda_mode = 'custom' …)` that is really `ck_projects_nda_document` on `nda_source`; `ProjectRules.ndaRequired`'s claimed it was derived from an `ndaMode` field that does not exist on that schema (it is independent on the SETUP surface, which edits the stored pair directly, and derived only on CREATE — the two shapes describe the same columns and which one a layer sees depends on whether it is creating or editing); and `Functions.md`'s slug paragraph still described the title-derived form retired by #88. `deno task check` + `deno lint` (1,502 files) clean; **1,023 unit tests** (7 new). **FLAGGED — unchanged and still needing a human:** `00001100` contains a bare `CREATE FUNCTION projects.get_project_details`, so the file is not idempotent and cannot be re-applied over an existing database without erroring at that statement (inherits Decision #86(c)); a full `db reset` is unaffected. | root `CLAUDE.md` §1/§6/§7/§8 · `packages/types/projects/{create,setup}.ts` + `nda.contract.test.ts` · `supabase/migrations/00001100_functions_projects_read_access.sql` · `documentation/database/projects/Functions.md` · Decisions #84 / #86 / #88 |

| 91 | **Timeline / Gantt — `@projective/ui/gantt`, the project timeline, the calendar's fourth view and the ticket modal's Timeline tab (2026-09-06).** A NEW 15th `@projective/ui` sub-path (the `display` sub-path already exports a `Timeline` list component, so the engine is named for what it draws) implementing SYSTEM_ARCHITECTURE §Charts' `GanttChart` / `GanttStore` / `ThemeBridge` entries, which had described code that did not exist. **(A) THE ENGINE follows the Week grid's architecture exactly and reuses its pure modules rather than copying them.** Tiered rendering: HTML chrome (`GanttTaskList`, the tiered `GanttHeader`, the popover) around ONE immediate-mode `<canvas>` stage painted by one ordered pass (`paintGantt`) over one `GanttScene`; a `@preact/signals` store (`createGanttStore`) owning `scrollX`/`scrollY`/`pxPerDay`/`rowScale`/hover/focus/fly and the computed `tier`/`visibleRange`, so a pan, a zoom or a hover moves pixels with no VDOM render in the path; sync paint in a layout effect, so NO FRAME IS EVER REQUIRED — every rAF loop (momentum, fly mode, the zoom spring, the hover expansion) is decoration over geometry the store already settled. The time axis is linear (`x = (ms − origin) / DAY × pxPerDay`) with seven tiers from `year/quarter` to `day/hour`, ticks generated through the calendar's zoned-time matrix so a 23-hour DST day is 23 hours wide (pinned by test), `snapTo` at the live grain, `fitZoom`, weekend bands, two centuries of axis each way. Rows are virtualized by lane window; the hovered lane grows by the store's `expandPx` and every lane beneath shifts by exactly that amount in the DOM list AND the canvas, which is what keeps them aligned for free. `hitTestItems` is pure arithmetic over the same `itemBox` the paint used. The full gesture map of the brief ships: wheel/Shift+wheel/Ctrl+wheel (cursor-anchored, the anchor captured once and held so a trackpad burst compounds around one point)/Ctrl+Shift+wheel; Ctrl-drag or middle-drag 2D pan with momentum; a middle CLICK enters FLY MODE (the calendar's `joystickVelocity` reused per axis, exited by any press, Escape, wheel or blur); Ctrl+middle strict drag; Left/Right/Ctrl+Up/Down/Up/Down/Enter/Escape/Tab on a parallel, visually-hidden ACCESSIBLE LAYER whose focus the canvas paints as a two-tone ring; left-drag on empty lane space creates (snapped, dashed draft, absent under `readOnly`); one finger pans, two fingers pinch-zoom, a tap opens. **The THEME BRIDGE** (`core/theme-bridge.ts`) is the swatch-reading technique the calendar proved: every colour, width, radius and font is authored in `gantt.css` against `var(--*)` on a hidden probe subtree and read back RESOLVED through `getComputedStyle` — with a swatch per ACCENT token the data names, re-resolved on any root theme write and on any change to the accent set — plus `resolveCssColor` for an arbitrary expression and `cssColorToHex` for the numeric form the WebGL tier would take. Verified live: flipping `data-theme="dark"` repainted the canvas's today cap to the dark `--primary` (`rgb(0,146,158)`) with no reload. **(B) THE DATA LAYER IS A PROJECTION OVER THE BOARD, and nothing on it is fabricated.** `@projective/types/projects/timeline.ts` (`TimelineLane`/`TimelineItem`/`TimelinePage`, pure total `buildProjectTimeline`/`buildTicketTimeline`, 14 tests) takes the resolved `BoardPage` — the same stages, cards and viewer flags the Kanban uses — and lays them on a time axis, EMBEDDING the board it was built from so the surface opens the SAME ticket modal without a second round trip; the fat `ProjectBackendService.timeline` is `board()` + the builder (same live query, same cache entry, the fixture corpus's pinned clock on the stub path — a "today" rule from the real clock would put every fixture ticket weeks in the past), and the island re-derives the timeline locally with the SAME builder on every card change, so a save, a dragged due date or an added stage move the pixels by exactly the rule the server applied. Additive Zod: `BoardStageRef.startAt`/`endAt`/`dependsOnStageId` and `BoardCard.claimedAt`; on the live path the six existing `project_stages` scheduling columns (`fixed_start_date`, `start_dependency_stage_id` + `_lag_days`, `file_duration_mode`/`_days`, `file_due_date`) resolve to a window in one fixed-point pass (a dependency chain arriving in sort order rather than dependency order; a cycle simply stops making progress and stays unresolved), and `tickets.claimed_at` becomes the bar's start. A stage with no window is an EMPTY LANE; a ticket with no date is COUNTED in its lane's caption ("4 tickets · 1 undated") and in the footer rig ("1 undated ticket not drawn") and drawn nowhere; a claimed ticket with no due date is a milestone at its claim, never a bar to an invented end; a due date before the claim collapses to the deadline milestone rather than a negative span; "Overdue" is derived from the page's instant and never claimed of finished work. **No DB migration** (every field maps to a column that exists) → no `documentation/database/*` change; no lifecycle change → no `PRODUCT_MANAGEMENT.md` change. **(C) THE SURFACES.** `/projects/[id]/timeline` (one lane per stage: the stage's window as a bar with progress = completed tickets over all, every dated ticket, dependency links, a trailing New lane for the backlog) and `/projects/[id]/[channel]/timeline` (the stage's lane, then one lane per TICKET with the assignee's face — a channel-header **Timeline** tab gated exactly like Tasks, because it is the same tickets on a time axis); a one-off engagement's "board" view link now lands on `/timeline` rather than on a Kanban titled "Timeline". The footer rig (`TimelineControlRig`, resolved by `timelineFooterFor`, composed after `boardFooterFor`) composes the SAME `.brd-rig` control set as the board's rig — Today · Fit · a LOG-scale zoom slider bounded by the engine's own clamp, printing the unit the axis is divided into ("Days") · Create Ticket / Create Stage — bridged by `timeline-state.ts` and a one-shot engine `commands` channel (`today`/`fit`/`center`; the engine acts and CLEARS, which a "focus" prop that reacts only to change cannot express). Drag-to-create makes a ticket in that stage due at the range's end (quick-create commits it at once through the SAME `/api/projects/board/ticket` the board uses, via the hoisted `ticketCommitPayload`); drag-to-move changes the DUE DATE ONLY — the claim instant is a fact the system recorded, not a plan the client may redraw. The ticket modal gains a **Timeline** tab (one lane per required stage, step numbers from `executionBands` so two stages that start together read as one step, links band to band, the live stage's window on each lane, a trailing "This ticket" lane; read-only by construction; "Open stage" jumps to the Stages tab + inspector); `TicketView` takes an optional `now` so a fixture-driven host passes its corpus's clock. The calendar gains a fourth `CalendarViewMode`, **`timeline`** (`components/TimelineView.tsx` — a pure MAPPING from events onto one lane per KIND and from the engine's gestures back onto the calendar's own `onOpenEvent`/`onQuickCreate`/`onExpandCreate`/`onMoveEvent`/`renderEventActions`; the composer's typed title is threaded through `onCreateRange`'s new fourth argument so an expand never loses it), wired through the header's view switch, the hub's `CALENDAR_VIEWS`/band letter "T"/rail glyph/`periodLabelFor`/step (a week — a month jump would leap past the viewport at every zoom but the widest); verified on `/calendar` at 383 items across five kind lanes on one canvas. **Region contract** honoured (lane = navigation, body = viewing and selecting, footer = every action) and **§B.6 icon-first** on the rig. **NO NEW DEV CONTEXT AXIS (§5), and that is a judgement:** every gate on the surface — drag-to-create, drag-to-move, the rig's create actions, the modal's edit rights — reads Decision #64's `board-access.ts`, so the existing persona/role axes already move the whole surface, and nothing here reads a `data-dev-*` attribute that did not exist. **DEFECTS FOUND BY MEASUREMENT, NOT INSPECTION:** (1) the zoom's "persist only if changed" guard skipped exactly the writes the ENGINE makes — a wheel zoom or Fit writes the host signal directly and then reports, so the value was already equal and never stored (measured `null` after Fit; now persisted unconditionally); (2) three hand-built card/stage literals (the write store's stub stage, the board's optimistic stage, the blank ticket) and one test compiled against the widened Zod only once the additive fields were added — Zod `.default()` makes the INPUT optional and the OUTPUT required, so an additive column is never free for a literal; (3) `commitPayload` was module-private to the board island and would have been duplicated — hoisted into `ticket-model.ts` as `ticketCommitPayload`, one builder for both surfaces. **VERIFIED IN-BROWSER** at 1440 × 900 and 390 × 844, LTR + RTL, light + dark, on the real fixture corpus: the project timeline (lanes 240px, header 48px, canvas 824 × 746 at DPR 1, rig 50px, zero document overflow), the stage timeline with its Timeline tab active, the popover ("Lane · When · Status · Note" + "Open ticket") → modal → Timeline tab → "Open stage" → Stages inspector, Fit (44 → 21 header cells, the range Jun 26 – Aug 12 = the corpus's extent + 8%) and Today (re-centred on Jul 17), the slider (`aria-valuetext` "61 pixels per day", 44 → 31 → 22 cells, stored `85.43`), Create Stage and Create Ticket from the rig, mobile (lanes 119px = 7.5rem, action labels folded to glyphs, `touch-action: none`, a tap opening the bottom SHEET at 390 × 288 and no anchored panel), RTL (lanes mirrored to x = 841 right of the canvas, the header strip translated +380px, zero overflow in either direction), the calendar's Timeline view, and the whole-repo `deno task check` + `deno lint` + `deno task test:unit` green (57 new tests: 43 engine, 14 builder). **NOT VERIFIED, stated rather than claimed:** anything needing a composited frame — this session's preview pane is frameless (rAF never fires while `document.hidden` is false), so hover expansion, momentum, fly mode, the zoom spring's motion and drag-to-create/drag-to-move were exercised by their pure geometry and tests, not by watching them; the popover's presence in that pane arrives through the spring's timer watchdog rather than a frame, which is the Decision #75(2) mechanism doing its job. Also not exercised: the live Postgres branch (the window resolution is pinned by inspection of the six columns and the fixed-point pass, not by a returned row). **FLAGGED — surface, do not silently resolve:** (a) **`touch-action: none` on the Gantt viewport** — the brief mandates native pinch-to-zoom of the time scale and two-finger pan, which this engine owns; that reverses the calendar's logged WCAG 1.4.4 position that two-finger gestures belong to the browser (Decision #75(c), where pinch ships OFF), so the two canvas surfaces now disagree, and one policy needs a human. (b) **`MobilePopup` names no primitive in this codebase** — the mobile popover is a bottom-sheet `Drawer`, the primitive the package already had. (c) **The WebGL/PIXI tier above ~1,000 nodes is documented, not built**: Canvas2D carried 383 calendar items without a measured cost, the theme bridge already returns the numeric hex that tier would take, and the escalation belongs to a density the fixtures do not produce. (d) "Open stage" from a stage's popover navigates to `/projects/[id]/[stageId]/timeline`, which on the LIVE path addresses a stage by its stage id where the channel header expects a `comms.project_channels` id — the same pre-existing convention the Tasks board relies on (`stageForChannel` accepts either), so the body renders and the header band does not; fixing it means projecting a stage's channel id onto `BoardStageRef`. (e) A dragged bar keeps its claim and takes the new end; the engine draws the bar where the reader dropped it for one frame and then re-derives it start-anchored from the data, which reads as a snap-back — deliberate (the claim is not the client's to move), but worth a word in the popover. (f) `prj-3389ufcjs2`, listed in this repo's memory as a one-off fixture, is a one-off SERVICE instantiation whose board is titled "Calendar", so its lane link stays `/board`; `prj-mc9r4c9ha2` and `prj-cujw52gg3p` are the client-viewer one-off projects. (g) The pre-existing `CalendarMonthTrack.tsx` `TS2322` errors (Decisions #85(f)/#87(d)) no longer reproduce — `deno task check` is clean on the whole tree at HEAD; not this pass's doing. | `DESIGN_SYSTEM.md` §C.1 + Part C · `SYSTEM_ARCHITECTURE.md` §Charts · `ROUTING.md` · `PRODUCT_SPEC.md` §Sitemap · `packages/ui/gantt/**` (+ `core/{time-scale,layout,gantt-store}.test.ts`) · `packages/ui/deno.json` · `packages/ui/calendar/{core/types,components/{TimelineView,CalendarHeader,glyphs},islands/Calendar,styles/shell,mod}` · `packages/types/projects/{timeline,timeline.test,board,mod}.ts` · `packages/backend/services/projects/{board-fixtures,live-board,ProjectBackendService,write-store,write-store.test}.ts` · `apps/web/features/projects/{core/{timeline-state,timeline-model,TimelineService,timeline-ssr,timeline-footer-slot,channel-view,ticket-view,ticket-model},islands/{ProjectTimeline,TimelineControlRig,ProjectBoard,ChannelHeader}.island,components/{detail-glyphs,ticket/{TicketView,tabs/TicketTimelineTab}},styles/timeline.css}` · `apps/web/features/calendar/{core/calendar-state,islands/{CalendarHeaderBand,CalendarLane}.island}` · `apps/web/routes/(dashboard)/projects/[projectSlug]/{timeline,[channelId]/timeline}.tsx` · `apps/web/routes/api/projects/timeline.ts` · `apps/web/routes/(dashboard)/_layout.tsx` · `apps/web/utils/storage-keys.ts` · `.claude/launch.json` · Decisions #1 / #35 / #37 / #62 / #64 / #65 / #66 / #73 / #75 / #84 / #88 |
| 92 | **Button tactile feedback, and the `--on-primary` contrast defect fixed at the token layer (2026-09-07).** A press-feedback model for every pressable control, and the closure of the defect flagged and routed around across Decisions #64 / #65 / #74 / #76 / #77 / #78 / #79. **(A) THE INVARIANT COULD NOT BE SATISFIED AS BRIEFED, and measurement is what settled it.** The brief asked for both "lock primary ink to `#FFFFFF` / `P.tone(98)`" and "dark mode must never drop below 7:1". Those are mutually exclusive against this theme: white on dark `--primary` (`#00929e`) measures **3.75:1**, and the token pair measured **3.57:1**. For white to clear 7:1 the fill needs a luminance ≤ 0.10, which on a near-black page (`--bg` is tone 2) is a button that has to be hunted for — a dark page forces a light fill, and a light fill forces dark ink. There is no third arrangement, so the INVARIANT was implemented and the prescription was not (the Decision #70(a) precedent: the tokens a brief names are used, the literals it names are not). **(B) THE ROOT CAUSE, and a worse number than anyone had recorded.** `theme-engine.ts` set dark `--primary: a1.tone(fg(55))` / `--on-primary: a1.tone(on(98))` — both tones ABOVE mid — violating the file's own stated invariant that a colour and its `on-` pair must straddle it, with the comment directly above describing code that was not there. Beyond the known 3.57:1: because `fgTone` widens a fill UP in dark mode while `onTone` widens an `on-` role DOWN, the two **converged** under `data-contrast="high"` and measured **1.75:1** — the accessibility overlay made the product's most-used control unreadable, the exact inverse of its job. **RESOLVED BY MODE-INVARIANCE, not by straddling (product owner, 2026-09-07).** The first fix straddled the dark pair (`fg(80)`/`on(20)`, 7.74:1) at the cost of the brand reading as a different colour in dark mode; the owner ruled that `--primary` is core brand identity and must be **strictly identical across themes**. It now resolves from raw tone constants — `BRAND_TONE = 45` / `BRAND_ON_TONE = 98` — in BOTH branches. **The constants deliberately bypass `fg()`/`on()`, and that is the subtle half of this row:** those helpers take the mode, so writing `fg(45)`/`on(98)` in both branches — one expression, apparently symmetric — resolves identically only while high contrast is OFF; switch `data-contrast="high"` on and the dark branch lands on tone 57 over tone 86, `#0097a4` on `#63e9f9`, **2.44:1**, while light widens to 8.42:1. Identical source, different brand, in the one state a struggling reader opts into. Raw tones remove the mode input entirely. Measured result: `#007680` on `#ebfdff`, **5.13:1**, byte-identical in all four states, pinned by a test that asserts the resolved hex rather than the expression. **The cost is stated, not hidden:** `--primary` is the one role that does not widen under the high-contrast overlay (AA, not AAA), and the only role whose POLARITY does not flip with the theme — dark fill, light ink, in both modes, where `--secondary`, `--tertiary` and every semantic ramp run light-fill/dark-ink in dark. That single fact drives (C) below. **(B2) THE BRAND FILL HAS NO CONTRAST HEADROOM, AND PRESS FEEDBACK HAD TO MOVE CHANNELS.** A mode-invariant fill cannot move away from whichever page is behind it, so one value must be readable under a near-white ink AND findable on a near-black page at once. In dark it clears both only just — 5.13:1 against its ink, **3.05:1** against `--surface-3`, i.e. 0.05 of a ratio point spare — so measurement, not preference, ruled out both blend directions: darkening toward `--scrim` lifts the label but sinks page visibility to 2.69:1 hovered and **2.51:1** pressed, under the 3:1 non-text floor; lightening toward `--on-surface` lifts visibility but sinks the label to 4.53:1 at 8% and **4.29:1** at 12%. The 8% step passes by 0.03, which is a rounding artefact rather than a margin. So `--primary` carries `--btn-mix-hover`/`--btn-mix-active` of **100%** — a zero luminance step — and draws its hover on the **border**, a channel with no bearing on either ratio, at a width the base already reserves as `transparent`; the press is carried by the scale compression every button shares. Generalised into §B.12.4 as: when a state cannot be afforded in one channel, move it to another rather than shaving a floor. **(C) THE BRIEF'S HOVER FORMULA BROKE WCAG AA, found by measuring rather than reading.** It specified `color-mix(in srgb, var(--btn-bg) 92%, var(--btn-ink))` — mixing the fill toward its own ink, which walks both sides of the pair together: measured, a light-mode primary goes 5.13 → **4.42 hover** → **4.12 pressed**, under the 4.5:1 floor, in the state a reader spends the most time in. The blend now runs toward **`--on-surface`** (via a per-severity `--btn-shade`), which is the pole opposite the PAGE — and for any role whose polarity FOLLOWS the theme that is also away from the ink, so contrast RISES with pressure: `--secondary` runs 7.72 → 7.91 → 8.00 dark and 6.13 → 6.69 → 6.99 light. This is the house precedent, not an invention — `psu-rig` already blended that way. The brand is excluded by a 100% step per (B2); the per-severity split exists precisely BECAUSE `--primary` stopped flipping polarity with the theme, so no single direction serves every severity any more. **(D) `filter: brightness()` REMOVED** from the base button: it is applied to the rendered element, so it darkens the label and icon by exactly as much as the fill, both sides move together, contrast is preserved only by luck, and the result cannot be measured statically at all because it depends on whatever the fill resolved to. **(E) THE PRESS MODEL.** `--btn-press-scale` 0.97 / `--btn-press-scale-icon` 0.92 / `--dur-press` 50ms in against `--dur-fast` out — compression is read RELATIVE to the control's size, so 3% off a 96px button is ~3px of travel while 3% off a 32px square is under a pixel and reads as nothing. The resting `scale(1)` is DECLARED, because a transform existing only in `:active` animates from `none`, which is not interpolable with `scale()` in every engine, and the return then snaps. It replaces a 0.5px `translateY` that on a 2× display lands on a physical pixel boundary and renders as nothing at all. Reduced motion sets `transform: none` under BOTH channels and by `none` rather than a zero duration — the token layer already zeroes every `--dur-*`, which removes the animation and LEAVES the compression, so the button still jumps to 97% instantly, which is the motion the setting was turned on to avoid. **(F) DEFECTS FOUND IN MY OWN WORK BY MEASUREMENT.** (1) The base hover I first wrote was `(0,4,0)` and beat `.ui-button--outlined:hover` at `(0,3,0)`, so it painted a near-solid fill on every outlined button on hover — the exact failure `entity-view.css` carries a comment warning about; the tonal blend is now scoped away from the three non-filled variants while the TRANSFORM stays on all of them. (2) My first two tests were wrong in opposite directions and both failed loudly: a flat 7:1 floor fails LIGHT mode, which ships at 5.13:1 and was never broken (the floor is now per-mode, 7:1 dark / 4.5:1 light), and my "straddle" assertion compared distances the wrong way round — replaced by a test that reproduces the two `color-mix()` declarations and asserts contrast never falls, which tests the actual claim instead of a proxy. (3) The patch tool I was using ran replacements through `String.replace`, which interprets `` $` `` in the REPLACEMENT as "everything before the match" — a `^[a-z0-9-]{1,96}$` regex in prose spliced the whole of `ROUTING.md` into itself; caught by reading the result back, fixed with a function replacer, and worth remembering for any scripted edit of a file containing regex literals. **(G) THE AUDITED SURFACE FAILED ITS OWN NEW GATE.** `.psu-rig__action` (Save · Discard · Publish · Archive) had a hover and a focus ring and **no `:active` at all**, so the most consequential presses on the project editor acknowledged nothing — which is what invites a second press on a control that is already working. Press feedback added, and deliberately WITHHELD from a `[data-locked="true"]` action: that action refuses on press, and giving the tactile acknowledgement of a press and then doing nothing reports success for a refusal. Its §B.8.2 compliance was verified across all four render states and needed no change — the rig already emits exactly one `primary` (Save leads while it is on screen; Publish takes over under auto-save, when Save is absent rather than disabled). **(H) DOCS.** New merge-gated **§B.12** — numbered 12, NOT 11 as briefed, because §B.11 is Anti-Tagification and is referenced by name from root §3 gate 7, §9, Part E and Part F; renumbering it would have silently redirected five live cross-references. Part E's "six component laws" became seven plus a machine-checked clause; root §3 gained gate 12 and §9 a checklist row; the two stale "routed around, needs a human" notes in §D.8.1 and Part F are closed. `ROUTING.md` formalises the two workspace view modes as URLs (`/projects/:projectSlug` Details ⇄ `/projects/:projectSlug/preview` Preview — real `<a href>` anchors with `aria-current="page"`, resolved server-side by `modeOf()`, never island state; a mode held in a signal has no address, cannot be linked or reloaded, and — decisively — cannot be guarded, while Preview's owner-only + ladder-complete gate is a 303 in `define.handlers`). **Corrected while there:** ROUTING.md still told every link-minter that a project is addressed by its **UUID** and that "the Quick-Init modal navigates to `/projects/<uuid>`" — retired by Decision #88, whose `prj-` slugs are now the only address (`project-identity.ts`: "The row's uuid does not route"). The doc also named a route directory that does not exist — `[projectId]` in 20 places against a real `[projectSlug]`. **FLAGGED — surface, do not silently resolve:** (a) `--on-primary-solid: #ffffff` was **not** added: it is a hardcoded hex (§3 token-only), and `#fff !important` would break **§D.7.7**, the inverted conversion-lane CTA, which is itself a merge gate and works by overriding `--btn-on`. It is also no longer needed — the brand ink is now a resolved token that clears AA in every state, and `#ffffff` would in any case measure 5.38:1 against the brand fill where `--on-primary` measures 5.13:1, a difference too small to justify a literal. (b) `[data-motion="reduced"]` still does **not** zero the `--dur-*` tokens product-wide — only the media query does, as `wallet/core/money-format.ts` already laments — so the overlay is honoured control-by-control rather than at the token layer; fixed for buttons and the setup rig here, but the general gap is a motion pass of its own. (c) The press model is applied to `.ui-button` and `.psu-rig__action`; other bespoke control families (`.brd-rig`, `.lp-card`, the lane icon buttons) are not yet covered by the gate they are now subject to. (d) **A regression this change introduced and then fixed, worth recording because the cause is general:** two footer rigs (`.psu-rig__action` and `.proj-sdrig__action`) inked their `primary` tone with `--surface`, which was correct *only while dark `--primary` was a LIGHT tone*. Making the brand mode-invariant turned that pairing into **3.58:1** in dark, and their 12% lightening hover into 4.29:1 — both under AA, in files this task never set out to touch. Both now ink with `--on-primary` (5.13:1, pinned) and hover on the border. The general lesson: a workaround written against a broken token silently inverts when the token is fixed, and `grep` for the workaround is the only thing that finds it. (e) Both modes sit at 5.13:1 — AA, not AAA — which is the accepted cost of brand invariance; only the mode-adaptive roles reach AAA. (f) The press model reaches `.ui-button`, `.psu-rig__action` and `.proj-sdrig__action`; other bespoke families (`.brd-rig`, the lane icon buttons) are still uncovered by the gate they are now subject to. (g) `--evp-cta-brand-fill` and `--evp-cta-brand-ink` are BOTH `var(--primary)` in `entity-view.css`, which is 1:1 if that control ever renders filled — pre-existing, unchanged by this pass, and worth a look by whoever owns §D.7.7. | root `CLAUDE.md` §3 gate 12 · §9 · `DESIGN_SYSTEM.md` **§B.12** (new) + Part E + §D.8.1 + Part F · `documentation/architecture/ROUTING.md` · `packages/ui/system/core/theme-engine.ts` + `theme-engine.test.ts` · `packages/ui/styles/index.css` · `packages/ui/fields/styles/button.css` · `apps/web/features/projects/styles/project-setup.css` · Decisions #64 / #65 / #70 / #74 / #76 / #77 / #78 / #79 / #88 |
| 93 | **Stages are addressed by their slug — the channel segment stops being a uuid (2026-09-07). EXTENDS Decision #88.** #88 minted an immutable `stg-…` address for every `projects.project_stages` row and routed NOTHING to it: `/projects/[projectSlug]/[channelId]` resolved its second segment to a `comms.project_channels` uuid, so a stage's own address existed and reached nowhere while stage links carried 36 characters of hex — the defect already flagged as #91(d). **The brief asked for a title-derived slug** (`stage-1`, `stage-1-1`, compound-unique on `(project_id, slug)`, a collision suffix and a rename policy). That is refused by the schema at three layers — `ck_project_stages_slug_shape` makes `stage-1` unstorable, `security.fn_slug_guard` raises `23514` on any UPDATE that moves a slug, and `slug.contract.test.ts` reads the migrations and pins both — and by #88's stated reasoning, which the column comment records: an address derived from mutable content dies on the first rename, silently and with a clean 404. **Product owner chose (2026-09-07) to route the EXISTING opaque slug**, so this pass adds no column, no migration and no collision logic: the DB half was already shipped, and the whole change is routing. **(A) THE SEGMENT IS POLYMORPHIC AND THE PREFIX IS WHAT PAYS FOR IT.** One route serves four channel kinds, so a stage carries `stg-…` and general/team/DM keep their own ids, told apart by `isSlug(ref, "stage")` — shape alone, no query, and unfoolable, because a stage slug is not a legal uuid and a uuid is not a legal slug. **(B) THREE KEYS, NAMED FOR THEIR JOB.** `StageChannel` gains `slug` beside `id` (the room the tree opens) and `stageId` (the row the Details tab writes), and `ChannelMeta` gains `ref` (the URL) + `stageId` while `channelId` KEEPS its meaning — which is why most consumers needed no change at all. Collapsing any two of these type-checks, because all three are `string`. **(C) ONE RESOLVER.** `findStageChannel` in the Zod SSOT replaces four independent `find` calls that had ALREADY drifted — two matched the stage's id, one the channel's, one both plus a `stage-{id}` string — and `resolveChannelMeta` delegates to it. It matches the slug and nothing else: accepting the internal keys would make the fixtures, where all three coincide, more permissive than production. It lives in `packages/types` because `channel-view.ts` reaches `dev-seam` transitively, which reads `import.meta.env` and therefore cannot be imported by a plain `deno test` — so the rule is testable only where it now lives. **(D) FIVE UUID GATES ON THE LIVE PATH WOULD HAVE SILENTLY REFUSED IT.** `live-submissions` (`isUuid` → zero stages), `live-members`, `live-messages`, `live-files` and the `live-writes` send path each gated on `UUID_RE` before stage slugs existed; a `stg-` segment would have resolved to an empty tab with no error anywhere. One shared `resolveChannelRef` in `live-support.ts` now answers "which room does this segment open", scoped by `project_id` on both arms — the slug is globally unique, so the scope is not about ambiguity but about stopping ANOTHER project's stage rendering under this project's chrome. `live-submissions` needs it least and gained the most: the slug names the stage directly, so its channel round-trip is gone entirely and a stage whose room is not yet provisioned now scopes correctly where it used to resolve to nothing. **(E) TWO BROKEN LINKS CLOSED.** `BoardStageRef` gains `slug`, which is what #91(d) needed; the timeline's stage jump now navigates to a real address and is guarded by `isSlug`, so an optimistic draft stage — which has no address until the server mints one, and is deliberately given a NON-`stg-` placeholder — navigates nowhere rather than to a 404. `live-overview` and its fixtures mint stage hrefs from the slug. **VERIFIED BY EXECUTION against the live local Postgres with a real GoTrue-issued token** (`PROJECTS_BACKEND_LIVE=true`), not by inspection: all four stage rows in the middle-nav lane link by slug with no uuid; the header band resolves ("Discovery" + all seven tabs) and every tab returns 200 with the header; the Details tab loads the right stage's own configuration with the right row highlighted; a message POSTed by stage slug commits and reads back in that stage's room; and the hard cut holds in both directions — the stage ROW uuid, the stage ROOM uuid, another project's stage slug and a nonexistent slug all fail to resolve, on the read AND the write path (404 on send). `deno task check` clean, `deno lint` clean, 1156 unit tests pass (9 new, pinning shape, collision, cross-project isolation and totality). **FLAGGED — surface, do not silently resolve:** (a) **`service_role` has NO `USAGE` on schema `comms`** — the same class as #68(a)'s `finance` gap, found because a probe row could not be cleaned up through PostgREST; one test message therefore remains in the local dev database until the next `db reset`. (b) **`buildStageChannels` still OMITS a stage whose `stage_all` room is unprovisioned**, so such a stage has a valid address and no tree row. Slug routing removes the reason for that omission — the address no longer depends on the room — but changing it is a behaviour change and a concurrent session's `sidebar-overlay.ts` is actively handling that case. (c) `basket-model.ts` `stageHref` still puts a raw `stageId` in the channel slot and reaches nothing; it was broken before this pass (its own docblock flags it) and fixing it needs the slug on the basket line. (d) An unresolvable segment returns **200 with no channel chrome**, not a 404 — pre-existing (#88(b)), untouched. (e) `LocalKeys.PROJECT_CHANNEL_PREFS` is keyed by the routed segment, so a stage's star/mute/pin prefs are re-keyed onto its slug; no production data exists and the slug is the more stable key, but the orphaning is real. | root `CLAUDE.md` §2/§7 · `ROUTING.md` §Conversations · `packages/types/projects/{detail,board}.ts` + `stage-routing.test.ts` · `packages/backend/services/projects/{live-support,live-detail,live-board,live-submissions,live-members,live-messages,live-files,live-writes,live-overview,detail-fixtures,board-fixtures,files-fixtures,members-fixtures,submissions-fixtures,overview-fixtures,write-store}.ts` · `packages/backend/services/scheduling/calendar-fixtures.ts` · `apps/web/features/projects/{components/ChannelTree,core/{channel-view,chat-context,stage-details-footer-slot},islands/{ProjectTimeline,ProjectBoard}.island}` · `apps/web/routes/(dashboard)/projects/[projectSlug]/[channelId]/details.tsx` · Decisions #21 / #22 / #83 / #88 / #91 |
| 94 | **Kanban — the whole card is the handle, and a ticket's funding scope decides who sees it and who may move it (2026-09-07).** Three coupled changes on `/projects/[projectSlug]/board` and the stage `/tasks` board. **(A) THE GRIP IS GONE.** `KanbanCard` rendered a 6-dot `<button class="ui-kanban__card-grip">` as the keyboard drag handle beside a card body that was ITSELF a `role="button"` — two tab stops per card, both of which had to be explained, and a `cursor: grab` on the whole card that only the pointer honoured. The card root is now the single interactive element: it is the pointer drag surface, the one tab stop, and the thing that opens the ticket — Space picks it up for a keyboard drag (Arrows move · Enter drops · Escape cancels), Enter or a click ACTIVATES it through a new `onItemActivate` on `KanbanBoard` (threaded via `KanbanColumnItem.onActivate`), and `data-draggable` lets the sheet promise `grab` only where a drag exists, `pointer` where the card only opens, and the default where it does neither (§3 gate 11 — the old sheet put `grab` on cards that refused to move). `TicketCard` became NON-interactive content (no `role`, no `tabIndex` on its root, no `onOpen`). **One guard is load-bearing:** the keyboard sensor drops on Enter from a window CAPTURE listener that tears the drag down before the card's bubble handler runs, so `keyboardActive()` is already false when the drop's keydown reaches the card — without an `e.defaultPrevented` check every keyboard drop would also OPEN the ticket it had just placed. Measured: 0 grips, every card `role="button"` + one tab stop, click/Enter open the modal, Space picks up ("Picked up … Use arrow keys…"), a drop opens nothing, a locked card's Space opens rather than drags. **(B) FUNDING SCOPE, in the Zod SSOT and nowhere else.** `TicketPaymentScope` = `unpaid` · `per_stage` (+ `paidStageIds`) · `full` on `BoardCard`, and `viewerStageIds` on `BoardPage`; the rules are pure functions every layer calls — `ticketPaidForStage` / `ticketPaidHere` (DERIVED from the stage the card sits in, never stored: a per-stage ticket dragged into an unpaid stage reads **Unpaid** and back into a paid one reads **Paid**, with no write, because nothing about the purchase changed), `ticketWorkLocked` (the client's drag locks while a claimed ticket is `claimed`/`in_progress`; `in_review` is the client's turn and `completed` is the existing revision path), `providerCanSeeCard` / `providerVisibleCards` / `providerScopedPage` (a provider sees a ticket only when it is paid for its current stage AND they are onboarded to that stage; the New backlog is never theirs). **The filter is applied on the READ**: `fetchBoardPage` (live — `viewerStageIds` from `stage_assignments` rows that `countsAsOnboarded`, the same guard the setup locks read, plus any stage whose ticket the viewer holds), `findBoardPage` (fixtures), and `providerScopedPage` again after the write-store overlay in `board()`/`timeline()`; the island re-applies the same function over its optimistic state so a card leaves at once. The spec's "clients move tickets at will" was **refined, not contradicted**: `PRODUCT_SPEC.md` §Ticket Movement now carries the lock and the funding rule, `PRODUCT_MANAGEMENT.md` §3.5 a row stating that funding scope is an ATTRIBUTE the purchase sets and not a state machine. **(C) LIVE PROJECTION, stated honestly.** `finance.escrows` holds the per-stage truth and is unreadable by `authenticated` (#68(a)), so the live path projects `payment_status` + `current_stage_id`: `escrow_funded` → per-stage paid for the current stage (`fn_hold_ticket_escrow` holds ONE escrow against `current_stage_id` and refuses a second), `partially_released` → per-stage with NO paid stage (the hold was released against a stage the ticket has left), `released` → full, anything else → unpaid. A pre-claim purchase has no column at all today, so every pre-claim ticket reads `unpaid` live until a purchase record exists. **Dev Context Switcher (§5 gate) — no new axis, and that is a judgement:** the persona flip and the existing `stageAssignment` axis already move every branch (assigned → every stage counts as onboarded, unassigned → none — the seam is a client surface the server cannot see, so it stands in for the server's list), and funding is DATA the fixtures spread across all four states (unpaid · per-stage paid here · per-stage paid elsewhere · full), not an ambient condition a seam simulates. **Verified in-browser** (fixtures path; the local gate is live but a non-verifying token falls back): Stages view 33 cards / 30 draggable / 3 Unpaid / 28 paid marks / 2 locks; Status view the same 33 with the 3 Unpaid in New and the 2 locks in In Progress; freelancer persona → 27 cards, 0 Unpaid, 0 in New, 0 draggable on the project board and 4 of 4 draggable on the stage board; unassigned → 0; a keyboard drop of a per-stage ticket into an unpaid stage flipped it to UNPAID **58 ms after the drop** and rolled back to paid when the server refused — that refusal (`No ticket found for "prj-…-t31"`) is the live gate rejecting a fixture id that is not a uuid, the pre-existing behaviour of EVERY move in this configuration, so the persisted flip is pinned by unit test instead (`board-funding.test.ts` ×2). `deno task check`, `deno task lint` and `deno task test:unit` (1,181 passed, 15 new) green. **FLAGGED — surface, do not silently resolve:** (a) a funded ticket that `move_ticket` carries to another stage keeps `escrow_funded` while its hold stays pinned to the OLD stage, and the live projection would call the new stage paid — unreachable through the board (the drag is locked while claimed) but reachable by a direct RPC; closing it needs `finance` USAGE or a projects-side mirror of the funded stage set. (b) A viewer onboarded only THROUGH a team assignment resolves to no stages on the live path (`team_id`, no user; the `org` membership read belongs to another module). (c) "Reassign/reset" as the way to unlock a working ticket is named in the spec text and not built — the lock lifts on hand-back today. (d) The basket now counts described, unclaimed tickets that are not paid where they sit (was: backlog only), so a `todo` ticket in an unfunded stage is checkout-able. (e) `GripIcon` in `board-glyphs.tsx` is now dead (it was never the kanban grip); left for a glyph-module pass. (f) The client-side RLS `"View tickets"` policy still lets any participant READ every ticket — the service filter is the enforcement layer this pass adds; a policy-level funding predicate is a human's schema decision. | root `CLAUDE.md` §2/§3/§5/§7 · `PRODUCT_SPEC.md` §Ticket Movement · `PRODUCT_MANAGEMENT.md` §3.5 · `DESIGN_SYSTEM.md` §C.1 (kanban) · `packages/types/projects/board.ts` + `board-funding.test.ts` · `packages/ui/kanban/{components/{KanbanCard,KanbanColumn},islands/KanbanBoard,styles/kanban.css}` · `packages/backend/services/projects/{board-fixtures,live-board,write-store,ProjectBackendService}.ts` + `board-funding.test.ts` · `apps/web/features/projects/{islands/ProjectBoard.island,components/{TicketCard,TicketListView,ticket/tabs/TicketFinanceTab},core/{board-access,ticket-model},styles/board.css}` · Decisions #35 / #53 / #60 / #64 / #65 / #68 / #84 / #89 |
| 95 | **Ticket deep links — `?tkv=<ticket-slug>`, the `tkt-` address, and the root cause of "Back reloads the document" (2026-09-08). EXTENDS Decisions #65 / #88.** A ticket had no address: the View Ticket modal opened only from a board or timeline it was already on, and nothing a person could copy pointed at it. **(A) THE ADDRESS IS A SLUG, FOLDED IN PLACE.** `projects.tickets.slug` — `NOT NULL`, no `DEFAULT`, `UNIQUE`, `ck_tickets_slug_shape` (`^tkt-[…]{10}$`) — plus the fifth `security.fn_slug_guard('tkt')` trigger in `00001890`, `ticket: "tkt"` in `SLUG_PREFIXES`, and a fifth row in `slug.contract.test.ts` so the SQL and the TypeScript cannot drift. The brief asked for "a migration script to generate slugs for existing tickets"; root §1 forbids a patch migration and the schema is reset-driven, so there is nothing to backfill — every insert passes through the trigger, seeds included (the seed corpus holds zero tickets today), and **a reset was run to prove it**: minted on insert, refused on update (`23514`), refused by shape, unchanged across a rename. It is GLOBALLY unique like a stage's, because the link arrives on pages that know no project. **(B) ONE READ, ONE ACCESS DECISION.** `BoardCard.slug` is OPTIONAL rather than nullable, and the distinction is the contract: a saved ticket always has one, so `undefined` means exactly "not saved yet" — a draft or an optimistic row — and those are the cards the URL sync must never address. `ProjectBackendService.ticket(slug)` resolves the ticket's project (RLS-scoped `tickets`→`projects` on the live path; the stub store then a corpus walk on the fixture path) and then performs the ORDINARY `board` read and finds the card in it — so the deep link inherits the board's cache, its stub overlay and its provider scoping, and a ticket the viewer may not open is simply absent, reported with the same words as one that does not exist (the `/share` rule, #67). Thin `GET|HEAD|OPTIONS /api/projects/ticket?slug=` is the one read that refuses a guest (401): its whole job is to open a modal on a page the viewer merely happened to be on. **(C) ONE OWNER OF THE URL.** `TicketDeepLinkHost` (mounted once per shell: `(dashboard)`, `(public)`, `[handle]`, both auth branches) owns URL ⇄ modal in both directions over the shared `ticketStack`; the pure half — parameter read/write preserving every other parameter, the exclusion policy (`/`, `/about`, `/help/**`, `/share/**`, the four auth screens, exactly `/checkout/details` and `/checkout/payment`), and a surface registry — is `core/ticket-link.ts`, unit-tested. On a page that already holds the ticket, the board or timeline island REGISTERS as a surface and the link opens on its own chain with its own cards, so an edit lands beside the board (verified: zero fetches on a board deep link); anywhere else the host fetches and renders `TicketStandalone` from a LAZY chunk, which is also that chunk's job as the CSS carrier (Decision #39) — the host ships to every page and must not drag the ticket surface's sheets and Quill with it. Registration NOTIFIES, and the fetch waits a 150 ms grace, because island hydration order is not something either side may assume. **(D) THE ROOT CAUSE OF DECISION #65's TRAP.** #65 measured `history.back()` from a pushed entry RELOADING the document and built the modal chain on `replaceState` alone. Read rather than measured around this time: Fresh's client runtime stamps every history entry it sees with `fClientNav: true` and, on `popstate`, treats any entry carrying it as a partial navigation — which on this app, where partials were removed (#52), ends in `location.reload()`. Spreading `history.state` into a pushed entry inherited the flag. Every entry the host writes now carries `fClientNav: false`, and the page's OWN entry is re-stamped `false` before a ticket is pushed on top of it, so the pair traverse as plain same-document `popstate`s — **verified with a document marker: Back closes the modal and Forward reopens the right ticket in the same document, and Back to a parameter-less entry closes it.** Push on open from a clean URL, replace thereafter, never `history.back()` to close; the accepted cost is one leftover entry after an in-page close whose first Back is a visible no-op. **TWO DEFECTS OF MY OWN FOUND BY MEASUREMENT:** (1) the modal→URL signal effect runs synchronously on first render, before the mount effect has read the address bar, and STRIPPED the very parameter it was about to resolve (`/messages?tkv=…` loaded as `/messages` with nothing open) — it now subscribes first, stays inert until the URL has been read, and never strips while a resolution is pending; (2) the board's `popFrame` restored `boardUrl` AFTER the host had synchronously re-written `?tkv=` onto the review's URL, so the return from a review lost the parameter — it now carries the restored frame's slug itself, and `boardUrl` is captured without any `tkv` the page arrived with. **VERIFIED:** DB by execution (above); the endpoint on the LIVE path with a real GoTrue token (200 with the card and its slug, HEAD parity, 404 unknown/malformed, 401 guest, 400 missing); in-browser on both paths — standalone open on `/messages`, board hand-off with zero fetches, Close and Escape stripping in place, Back/Forward/close-on-Back, an excluded checkout step keeping `step=2`, the landing page while signed in, an unknown slug stripping with one `role="alert"` toast, a guest stripping silently; `deno task check` + `deno lint` clean, 1200 unit tests green (10 new). **NOT VERIFIED, stated:** the production build's CSS emission for the lazy chunk — dev mode injects it; a `deno task build` should confirm the async chunk's sheets are linked. **FLAGGED — surface, do not silently resolve:** (a) **cookies are per-HOST, not per-port** — a concurrent session's dev server on another `localhost` port replaced this session's `sb-access-token` mid-verification with a token this server's GoTrue rejects (`No suitable key or wrong key type`), silently dropping every live read to fixtures; anyone verifying the live path in this repo with two dev servers up is measuring whichever server wrote the cookie last. (b) `packages/ui/hooks/zz-repro.test.ts` (untracked, another session's reproduction) fails 2 tests in `deno task test:unit`; not this pass's. (c) The `TicketStandalone` path commits through the same endpoint but its board copy is per-modal — a save there does not update a board island that mounts later on the same page, which no route produces today. (d) #65's `replaceState`-only rule for the REVIEW chain is left as it is; only the ticket parameter pushes. | root `CLAUDE.md` §1/§2/§7 · `ROUTING.md` §Global routing rules · `documentation/database/projects/Tables.md` · `documentation/database/security/Functions.md` · `packages/types/slugs/{slug,slug.contract.test}.ts` · `packages/types/projects/board.ts` · `packages/backend/services/projects/{ProjectBackendService,live-board,board-fixtures,write-store}.ts` · `apps/web/routes/api/projects/ticket.ts` · `apps/web/features/projects/{core/{ticket-link,ticket-link.test,ticket-view,BoardService},islands/{TicketDeepLinkHost,ProjectBoard,ProjectTimeline}.island,components/ticket/TicketStandalone}` · `apps/web/routes/{(dashboard),(public),[handle]}/_layout.tsx` · `supabase/migrations/{00000015,00001890}*` · Decisions #39 / #52 / #65 / #67 / #85 / #88 / #93 |
| 96 | **Entity view — projects fold into the shared frame; the Gutenberg reading gravity is the contract (2026-09-09). SUPERSEDES the bespoke project template of Decisions #43/#44; EXTENDS #78/#79.** `/view/[id]?type=projects` was a second design system beside the one Decision #78 built: a profile page's 7:2 `pf-header` banner as its hero, a `.vw` page frame with its own padding vocabulary, a boxed `StageFlow` accordion (`--surface-1` cards, a tinted "Active" flag, skill pills, role rows on tonal fills — §B.9.7 and §B.11 breached on the one page a freelancer and a client open back to back), a `ProjectViewLane` resolved into the SHELL's navigation slot (so a signed-in reader got a drag-resizable middle-nav lane and a guest a floating glass aside, neither of which is what a service shows), an Apply control in the sticky header band (§D.7.6 forbids a purchase control there), and its own `ProjectStickyHeader`. **(A) ONE FRAME.** `EntityViewPage` now hosts a project as one more branch of the `.evp` four-track frame: the `evp-frame--nomedia` variant (the same shape a session's scheduler stage takes) spans the hero across both content tracks, so the title is the page's PRIMARY optical area top-left after the start strip; `ProjectLane` is the END column on the SAME `pf-lane`/`.evp-lane*` skeleton as `EntityLane`; `ProjectApplyBar` re-homes the transaction below the frame breakpoint (§D.7.4); the header band is the ONE `EntityStickyHeader`, labelled _Project_; `viewLaneFor` returns `null` for every archetype but an article. **(B) THE BANNER SURVIVES, QUIETER.** The client's banner is a slim `aria-hidden` strip at the top of the hero — 7:1, capped `8.5rem`, `--radius-lg` to match the media canvas a listing renders in that position — with a 64px face overlapping its lower edge at half height: Cord/LinkedIn listing prominence, not a profile's. **(C) ONE BODY VOCABULARY.** Details are a two-column `SpecLedger`, roles are ONE `MetaLine`, and the stage run is the shared `StageProgressLedger` (native `<details>`, the §D.8.1 track) with `showSeats` — so the seat pool, open roles and per-stage prices a project carries render in the same rows a service's deliverables do. The quick-jump `jumpToStage` is now DOM-only: the signal it also wrote was read by the island this pass deleted, and a signal nobody reads is a control that appears to work (§3 gate 11). **(D) THE LANE PARTS ARE SHARED, NOT COPIED.** `lane-parts.tsx` — `LaneIdentity` (face · name · earned badges · the kebab menu as data) · `PriceBlock` · `LaneStages` · `LaneLedger` — is rendered by both lanes and by `EntityBuyBar`; the commerce lane's own copies were deleted, and the lane price on the phone and the desktop are now literally one component. Every figure a project prints — ticket floor, seat sentence, stage-of-N — derives from one pure module (`project-view-model.ts`) in the currency `StageProgressLedger` already assumes (`PROJECT_CURRENCY = "USD"` — a project item carries no `currency`, and two fallbacks would let the lane and the ledger label one number two ways). **(E) DEAD CODE REMOVED**, verified by a script that greps every `.vw*` selector against every `.tsx`/`.ts` rather than by reading: `ProjectViewScreen`, `ProjectShowcaseBody`, `StageFlow`, `ProjectViewHeader`, `ProjectViewLane`, `ProjectStickyHeader`, `ViewLaneHeader`, the orphaned `MediaGallery` + `ServiceShowcase` islands (imported by nothing since #78), `project-view.css` entirely, six dead regions of `view.css` (hero grid, media gallery, entity overview, sidebar lane content, mobile transaction block, service showcase/team roles — ~700 lines), `selectedStageId`/`STAGE_FLOW_ANCHOR`/`projectSaved`, `badgeTagsFor`/`commerceFor`/`laneMenuActionsFor`/`entityNounFor`/`scheduleHrefFor`, and 20 of 37 `view-glyphs` entries plus `trustIcon`. The article template keeps its `.vw` frame but now shares `.evp`'s padding (block `--space-5`/`--space-7`, inline `--space-5`, `--space-4` ≤767px) and `.vw-h2` drops `--fw-bold` for `--fw-medium` (§A.4 — a heading is never bold). The dashboard's member header band (`project-header-slot.tsx`) and `ProjectPreviewRig` are repointed and unchanged in behaviour. **Verified in-browser** at 1280 × 900 (guest shell): pipeline `pj-helia-wallet-redesign` frame `48 / 778 / 352`, hero top-left at (96, 112) with the banner 778 × 111 and the face 64 × 64 overlapping it, lane pinned `sticky` at 88px with the 36px Apply primary in its footer at the bottom-right, exactly **1** `filled` control on the page, **0** cards/chips/`pf-header`, 5 track items with 1 (the active stage) open, 5 lane quick-jumps (a click opened stage 3's `<details>`), a 3-item kebab, 3 ledger rows, no rails, no reviews, no console errors; one-off `pj-verdant-brand-refresh` collapses to one un-numbered "Delivery" stage with the rule hidden; the service `sv-brand-identity-sprint` regression holds (`48 / 368 / 386 / 352`, 36px figure, 4 quick-jumps, 4-item kebab, buy bar `display: none`); `dir="rtl"` mirrors the lane 898→24 and the strip 24→1202 with `scrollWidth === clientWidth`; dark theme resolves the banner ring to `--surface`. `deno task check`, `deno lint` (1,558 files) and the feature's tests are green. **FLAGGED — surface, do not silently resolve:** (a) **the commerce frame still places the MEDIA column left of the hero** (a recorded owner directive in `entity-view.css`), so on a listing the Gutenberg primary optical area is the canvas, not the title — a project has no canvas and so honours it literally; reconciling the two is the owner's call. (b) Apply remains an optimistic client stub (`projectApplied`) shared with the dashboard rig; the application write path is still Phase 2. (c) `offerFor("project")` still declares `escrows: true` and is read by nothing on the project path (Decision #44 removed escrow chrome from projects); left rather than re-decided. (d) `sellerBadges` rendered **0** badges on both the service and the project identity bands in the corpus — unchanged logic, worth checking the `ratingSignals` threshold against the fixtures. (e) The preview pane could not exercise scroll: the band's condense and the quick-jump's smooth scroll are verified by DOM state (`open` set) and by the shipped `scrollToElement` watchdog, not by watching them move. | `DESIGN_SYSTEM.md` **§D.8.6** (new) + the §D.8 gate · `PRODUCT_SPEC.md` §The Entity View / §Sitemap · `apps/web/features/view/{components/{EntityViewPage,project-view-parts,lane-parts,ProjectCtaRig,entity-view-parts,view-glyphs},islands/{ProjectLane,ProjectApplyBar,EntityLane,EntityBuyBar}.island,core/{project-view-model,view-lane-slot,view-header-slot,view-state,view-model},styles/{entity-view,view}.css}` · `apps/web/features/projects/{core/project-header-slot,islands/ShowcaseStyleAnchor.island}` · `apps/web/routes/{(public),[handle]}/_layout.tsx` · Decisions #43 / #44 / #64 / #78 / #79 / #80 |

| 96 | **Public profile rebuilt as an editorial single column — the split hero, four sections, and a monochrome chrome (2026-09-10).** `/[handle]` is rebuilt to the product owner's Contra-inspired brief, and the brief's four palette/structure/geometry/availability directives are applied as decisions. **(A) THE LANE, THE SPLITTER AND THE MIGRATING STICKY HEADER ARE GONE.** The profile mounts nothing in the shell's lane slot, so the authenticated `UserShell` renders a bare `PageCanvas` and the guest `GuestShell` its lane-less header + body + footer flow; the surface is ONE column in the native window scroll. Measured: no `.ui-splitter`, no `.ui-guest-aside`, no `.pf-stickyhead` on any profile route, in both shells. **But the skeleton those three were built from is NOT deleted**, because the `/view` feature layers on it — thirteen files select on `pf-lane` / `pf-header` / `pf-stickyhead` / `pf-btn` / `pf-availtoggle` (Decisions #41/#43/#79) and `guest-shell.css` keys its sub-header glass off the literal `.pf-stickyhead[data-condensed]`. Those regions were EXTRACTED verbatim into `profile-skeleton.css`, the six view islands repointed to it, and it carries the two explore sheets they used to receive transitively (a server component's CSS import reaches nothing — Decision #39). It keeps the `pf-` prefix because renaming a skeleton thirteen files select on is a pass of its own; it is owned by `/view` in everything but its path. **(B) THE HERO.** A 45/55 grid (`minmax(0,45fr) minmax(0,55fr)`, measured 597.6 / 730.4px at 1376px of content): 72px avatar · name at `--text-3xl` / **`--fw-bold`** · `@handle · Kind` in Meta register · **Message** (inverted `--on-surface`/`--surface` fill, §D.7.7's idiom) + **Follow** (outlined, monochrome) · an unboxed metrics strip (rating → the reviews anchor; completed stages; the earned Standing rung with delivered volume) beside a 16:10 showcase frame holding a looping showreel or a cover. **When `showcase` is `null` the hero collapses to one column capped at 45rem with no frame at all** (measured `grid-template-columns: 1376px`, `.pf-hero__id` 720px) — the collapse state is reachable from the stub because the fixtures spread the three states by hash, so it is not a branch that appears the day a real profile has nothing uploaded. The `--fw-bold` name is a **scoped exception to §A.4**, recorded there: it is the only element on the page above `--fw-medium`, so the "next register has nowhere to go" cost does not arise. Under either reduced-motion channel the island withdraws `autoplay` and pauses to the poster. **(C) THE CONTEXT BAR** is 65/35 (852.8 / 459.2px): headline at `--text-xl` + the inline-editable story capped at `--measure` beside Skills and Languages as monochrome pills (`--surface-1` / `--text-secondary` / `--text-xs`, no border, no category glyph, no colour-coded proficiency — Decision #40(E) is reversed). Skills are links into the Explore search and are controls; **Languages are the one non-actionable pill row on the platform**, recorded as a scoped §B.11 exception at the owner's direction. **(D) FOUR SECTIONS, ONE ADDRESS EACH.** `ProfileTab` is now `work | experience | reviews | posts` in the Zod SSOT; `LegacyProfileTab` + `LEGACY_TAB_TARGET` carry the eleven retired segments and `[tab].tsx` answers each with a **308** (measured: `/portfolio` `/services` `/about` `/work` → `/@handle`; `/education` → `/experience`; `/articles` → `/posts`), and an off-matrix section on a real profile is now a real **404** (it answered 200 with a "not found" body). Work is the index: client-proof strip (grayscale, 0.6 opacity, unboxed) · **Selected work** (a CSS-column masonry at `--pf-tile-gap` = `--space-2`, `--radius-xs` corners, media geometry reserved through `width`/`height` ATTRIBUTES derived from the piece's `aspect` so a column never reflows — measured 4 → 3 → 2 → 1 columns by container width) · Services and Completed projects through the SAME explore collections `/explore` renders · a buyer's Hiring now · the roster of a multi-member entity (department-grouped for an organisation). A tile at rest is media only; the scrim (`linear-gradient(to top, color-mix(in srgb, var(--surface) 95%, transparent), transparent)`) and the caption (`translateY(--space-2)` → 0, opacity 0 → 1, `--dur-medium` `--ease-standard`) arrive on hover/focus and are **permanent below 768px** (measured: caption opacity 0 / transform +8px at rest on desktop, 1 / none on hover and at 390px). Experience merges career history, education and certifications as hairline-separated rows with the trust crest ONLY on a verified credential; it renders for individuals only — a company has no CV. Reviews and Posts reuse the existing bodies. **(E) MONOCHROME BY MEASUREMENT.** A DOM sweep over every `.pf *` for colour, background, border and SVG fill resolving to `--primary` returns **zero hits** on the index and on Reviews, after four leaks were closed: the reused explore cards' status chips, skill pills, owner crest and card actions (scoped `.pf .ex-*` overrides in `profile-work.css`) and the package `RatingStars` fill (`profile-base.css`). Worst text contrast **5.85:1 light** (the tags) / **7.77:1 dark**; the inverted CTA measures 17.14 / 14.93. Zero four-sided borders on static content; zero horizontal overflow at 1440 and 390, LTR and RTL (the hero mirrors: avatar x 32 → 1336). **Additive Zod SSOT** (`ProfileShowcase`, `ProfileStats` + `ProfileStanding`, `WorkPiece`, `CertificationEntry`; `ProfileTabPayload` re-keyed to typed collections) — a read projection over fixtures like Decision #36, **no DB migration** → no `documentation/database/*` change; no lifecycle change → no `PRODUCT_MANAGEMENT.md` change; no new `@projective/ui` primitive → no §C.1 change; the profile branches on no `data-dev-*` seam (ownership is server-resolved from the JWT) → nothing to mirror in the Dev Context Switcher (§5). **DEFECTS FOUND BY MEASUREMENT, NOT INSPECTION:** (1) the owner's "Change showcase image" `Tooltip` anchored to an absolutely-positioned button measured a 0×0 span and floated to the frame's far corner — a positioned `.pf-hero__imgslot` wrapper now owns the geometry; (2) the four-agent build left `.pf-panel*` in the Experience sheet, `ProfileMessagePopover` without the barrel import, a raw `1px solid` where `--hairline-strong` exists, and the not-found branch with NO CSS carrier (no profile island mounts there — `ProfileStyleAnchor` now does, the `ViewStyleAnchor` precedent); (3) a fake session token with a one-character signature decodes to a guest through `djwt` and would have made the owner branch look broken in any probe that used one — the probe, not the code. **VERIFIED IN-BROWSER** (a driven Chromium, `localhost` only — this machine's Sophos web filter intercepts `127.0.0.1` with a warn page) on `/@juno` (video), `/@kenji` (image), `/@ivy` (no showcase), `/@northwind` (organisation), all three sections, guest + authed viewer + owner (Settings/Share rig, both image pickers at their real positions, inline story editing, "Add work"), light + dark, LTR + RTL, 1440 + 390; `deno task check` (whole repo, routes included), `deno lint` (1,555 files) and 1,221 unit tests green. **NOT VERIFIED, stated:** external media never loaded in the probe (Unsplash + MDN are filtered here), so frames were measured, not seen; `navigator.share`/clipboard in headless; the showcase `Tooltip`'s final position (the slot geometry is confirmed, the portal was not caught open). **FLAGGED — surface, do not silently resolve:** (a) **the four `theme-engine.test.ts` failures are pre-existing at HEAD** — commit `b4a28bc` re-split the brand tones (dark 55 / light 45) that Decision #92 pinned mode-invariant; the §B.12 invariant those tests guard is currently broken by the owner's own commit and needs a decision, not a patch here; (b) the `Hire` CTA is gone by the brief — a seller is hired through a listing in Work; (c) Teams · Businesses tabs are retired outright (the roster survives in Work; the entities-this-profile-belongs-to lists do not); (d) `/[handle]/availability` still resolves but nothing on the profile links to it — it is orphaned pending a product decision on where availability lives; (e) `volumeLabel` is a server-formatted string and does not follow the header currency switch (the ~180-site backlog of Decision #69(b)); (f) `--text-main` in the brief names no token in this theme — `--on-surface` is the ink (the Decision #70(a) precedent); (g) the explore `ProjectCard` title renders at `--fw-bold` inside the profile's Completed projects — the card family's own pre-existing §A.4 breach, reused not introduced; (h) the fixture corpus resolves ANY non-reserved handle to a synthesised profile, so `/@nobody-here` is 200 — pre-existing stub behaviour. | root `CLAUDE.md` §2/§3/§5/§7 · `PRODUCT_SPEC.md` §The Public Profile + §Sitemap · `ROUTING.md` · `DESIGN_SYSTEM.md` §A.4 / §B.11 (scoped exceptions) / §D.5 · `packages/types/profile/{profile,tabs}.ts` · `packages/backend/services/profile/profile-fixtures.ts` · `packages/backend/mocks/assets.ts` · `apps/web/routes/[handle]/{_layout,index,[tab]}.tsx` · `apps/web/features/profile/**` · `apps/web/features/view/**` (skeleton repoint) · Decisions #3 / #36 / #37 / #39 / #40 / #41 / #43 / #69 / #70 / #79 / #92 |

| 97 | **Public profile — the seller rig (Hire), the Services row, availability + at-a-glance facts, the guest sign-in prompt, and the Reviews stance filter (2026-09-15). REVERSES two positions of Decision #96 at the product owner's direction** — "there is no Hire control on a profile" and "availability, presence and clocks are not part of a profile" — with `PRODUCT_SPEC.md` §The Public Profile rewritten in the same change (root §0: the spec wins, so it moves with the code). **(A) THE RIG IS THREE SHAPES, decided by one pure rule.** `ctaFor(kind, hasServices)`: a SELLER (freelancer · team) leads with **Hire** and folds Message + Follow into icon-only circles (portal `Tooltip` + `aria-label`, §B.6); a BUYER keeps Message as its text primary; the owner keeps Settings + Share. Hire is honest about what it can name — the spec's own reasoning for removing it was that "a control that cannot name [the listing] would only route to that section anyway", so that is exactly what it does: a real `<a href="#profile-services">` when there is a listing (works with JavaScript off and for a guest; `scroll-margin` clears the sticky header, verified landing at 84/64px under the guest/authed headers), and the conversation when there is none. **(B) THERE WAS NO AUTHENTICATION MODAL TO REUSE.** The codebase's guest idiom is a `/login?redirectTo=…` bounce link (`CardActions`, `ContactPopover`, `view-model.signInHref`). Rather than invent a third login form, the new `SignInPrompt` island (`features/auth/`) wraps the standard flow's own two entry points — `withRedirect("/login")` and `withRedirect("/join")`, so the auth routes, the OAuth callback and the guard all already honour the return path — in the `@projective/ui` `Dialog`, reached from any surface through the `sign-in-prompt.ts` bridge (`requestSignIn({intent, returnTo, subject})`). A guest's Follow ⁄ Message open it with a sentence keyed on the intent; the controls keep their shape and position (the star precedent). **(C) THE SERVICES ROW** is a fat read of its own — `ProfileBackendService.services` → `findProfileServices`, the SAME `reown(pick(SERVICES…))` slice the Work payload carries, extracted so both cannot drift — resolved ONCE by `[handle]/_layout.tsx` (it feeds three regions: the row, the hero's Hire target, the context bar's spend floor) and rendered above `.pf-sections` on every section; Services is REMOVED from the Work body and from its emptiness gate. The one-row clamp is pure CSS (`grid-auto-rows: 0` + `overflow: hidden` under `data-expanded="false"`, so the column count stays the grid's own); the `ServicesRow` island only measures which cells fell below the first row (`offsetTop`), stamps them `data-hidden` (`visibility: hidden` — out of the tab order and the accessibility tree, not focusable behind a clip) and renders "Show all N" only when something genuinely is hidden at the live width (a `ResizeObserver` re-measures). **(D) AVAILABILITY AND THE AT-A-GLANCE FACTS ARE DERIVED, NEVER ASSERTED.** Additive Zod SSOT on `ProfileView`: `hours` (`ProfileHoursSchema` — timezone + the `working_hours` bands, REUSING `AvailabilityRuleSchema`), `responseMinutes` (the measured datum; `responseTime` is now derived from it) and `freeConsultation`. Two derivations were EXTRACTED so one fact has one author: `scheduling/hours.ts` (`buildRules`, previously private to `availability-fixtures.ts`, which imports the profile fixtures — importing it back was a cycle, and a cycle in a module whose corpus builds at import time is the TDZ class of #49) so the `/[handle]/availability` calendar and the profile summary are one derivation (probe: `agreesWithCalendar: true` on every seller), and `booking/call-offer.ts` (`callOfferKindFor`, previously inline in `contactOfferFor`) so the profile's "Free consultation" mark and the listing's Contact menu cannot disagree. The pure, clock-injected `profile/core/hours.ts` derives the schedule summary (`Mon–Fri · 9:00 AM – 5:00 PM`), the live **Available now ⁄ Away** badge with its next edge (`Until 12:30 PM` · `Back Mon 9:00 AM`) and the seller's local time from `Intl` wall-clock PARTS in the seller's zone — no offset arithmetic — in ONE fixed locale so SSR and the island agree character for character; `ProfileAvailability` re-derives on a minute-ALIGNED timer and on tab re-focus. The badge is the bar's one status container (§B.11.3), monochrome by shape (solid dot ⁄ hollow dot). `ProfileGlance` (server): `responseLabel`, **Fast responder** gated on the discovery card's own `FAST_REPLY_MINUTES`, **Free consultation** as a disclosure, and **Est. project spend** = `estimatedSpendFor(services)` — the cheapest floor through `serviceStartingPrice`, the rule the cards beneath it print, rendered via `MoneyView` (it follows the header currency switch). Presence (`online`) stays unpainted: the badge derives from published bands and the clock, never from a pip. **(E) THE STICKY TAB BAR PINS FLUSH.** Under the guest shell it pinned to `--site-header-h` (the 5.5rem RESERVED flow band) while the scrolled header is a floating pill whose bottom edge is at 4.25rem — a 1.25rem seam the tiles scrolled through. New token `--site-header-pinned-h` (`site-shell.css`, = bar + scrolled gap, which the pill now also reads) — measured: pill bottom 68px, tabs top 68px, guest and authed (48px) alike, desktop and phone. The ground is full-bleed (a negative `--pf-gutter` margin, solid `--surface`, never glass) with `--space-6` ⁄ `--space-4` inset, and the list is centred on the OUTER row while the scroller shrinks to content — `justify-content: center` on an overflowing scroll container strands the first tab (#77(D)). **(F) REVIEWS FILTER** — `ReviewsList` island with the shared `SelectButton` (its `--primary` selected ink neutralised under `.pf`), All · As freelancer · As client with counts; the stance is the AUTHOR's stored `role` INVERTED (`reviewStanceOf`, unit-tested — a review whose author was the client is a review of this profile as a freelancer), mirrored into `?as=` with `replaceState` (SSR honours it; history length unchanged); rows now say "as their client" so a row under "As freelancer" reads as a client's review. **(G)** Message below 768px opens a bottom-sheet `Drawer` (`useIsMobile`; measured 390×315 docked at the viewport bottom), desktop keeps the `DraggablePopover`. **(H) THE FOLLOW ACKNOWLEDGEMENT** is a `scale(1.15) → 1` settle (ease-out, no overshoot, §B.5) plus a six-dot burst behind the control, on `transform` ⁄ `opacity` only (§B.12), removed by BOTH reduced-motion channels (measured `animation: none` under the media query AND `data-motion`); the pressed state carries the fact, the motion decorates it. `.pf-hero__showcase` takes `--radius-base` (8px) and clips to it. **THREE DEFECTS FOUND BY MEASUREMENT, NOT INSPECTION:** (1) `Button iconOnly` renders its `icon` PROP and drops its children, so the first build shipped two EMPTY 40px circles carrying `aria-label`s — a control that renders nothing (§3 gate 11), invisible to the type-checker; (2) a probe cannot switch theme by flipping `data-theme` — `applyConfig` writes the scheme INLINE on `<html>`, so the attribute flip measured byte-identical colours in "both" modes; a probe must set `localStorage.theme` and reload; (3) the first probe against a COLD Vite server measured an unstyled page (Hire 30×24px, hero at y=5138) — a measurement taken before the module graph is warm is a measurement of nothing. **Verified in-browser** (a driven headless Chromium over CDP; `localhost` only): guest + authed at 1440 and 390, tabs flush, Show all ⁄ Show less (5 of 9 hidden → 0 → 5; 8 of 9 on a phone), the guest prompt's two links carrying `redirectTo=%2F%40juno`, follow → pressed + 6 dots + status announcement, the sheet, the popover, the Hire landing, the stance filter on `?as=client`, a buyer entity (message rig, no row, no hours, response only), the owner rig (a forged decodable token — Settings ⁄ Share, both pickers, the editable story), light (worst text 5.85:1) + dark (7.46:1), `dir="rtl"` with zero overflow either direction, zero console errors; 13 new unit tests (hours · model), `deno task check` clean over the whole tree. **NOT verified, stated:** `:focus-visible` rendering (the pane never takes real keyboard focus), and the `hours` ⁄ services LIVE path (both branches answer from fixtures until `PROFILE_BACKEND_LIVE` has a read). **FLAGGED — surface, do not silently resolve:** (a) **34 files under `explore/`, `view/`, `types/services` and `types/explore` were modified in the working tree by a CONCURRENT session during this pass** and are untouched here; the 4 `theme-engine.test.ts` failures (#96(a)) and one `jsx-curly-braces` lint finding in `ServiceCard.tsx` belong to that in-flight work, not this one. (b) No Dev Context Switcher axis was added (§5): guest ⁄ authed ⁄ owner are server-resolved, and the availability state is DATA plus the clock, not a persona a seam simulates. (c) Follow remains a session-local stub; the sign-in prompt does not carry the intent through the redirect, so a follow is not auto-completed after signing in. (d) The spend floor's unit follows the cheapest listing (`/ seat` for a group session) — the brief's "/ ticket" is an example, not a rule. (e) The badge consults the weekly bands only, not blackouts; a seller on leave inside their usual hours reads "Available now" until the live path carries blackouts. (f) `/[handle]/availability` is reconnected ("Full calendar" in the availability block), closing #96(d). (g) The two marks are an earned signal and a disclosure — §B.11.3's carve-out, extended, not breached. | root `CLAUDE.md` §0/§2/§3/§5 · `PRODUCT_SPEC.md` §The Public Profile · `ROUTING.md` · `packages/types/profile/profile.ts` · `packages/backend/services/{profile/{ProfileBackendService,profile-fixtures},scheduling/{hours,availability-fixtures},booking/{call-offer,BookingBackendService}}.ts` · `apps/web/features/auth/{core/sign-in-prompt.ts,islands/SignInPrompt.island.tsx,styles/sign-in-prompt.css}` · `apps/web/features/profile/{core/{hours,profile-model,profile-ssr},islands/{ProfileHero,ProfileMessagePopover,ProfileAvailability,ReviewsList,ServicesRow}.island,components/{ProfileContextBar,ProfileGlance,ProfileServicesSection,ProfileTabContent,tabs/{ReviewsTab,WorkTab}},styles/*}` · `apps/web/features/marketing/styles/site-shell.css` · `apps/web/routes/[handle]/{_layout,[tab]}.tsx` · Decisions #3 / #37 / #56 / #69 / #74 / #77 / #79 / #92 / #96 |

| 98 | **Entity viewer + Explore cards — token contract, iconography and the provider line aligned to the profile overview (2026-09-15).** A refactor-and-polish pass over `/view/[entity]` (+ its `schedule` leaf and the `/[handle]/view/[item]` mirror) and the Explore card family, against the `/[handle]` overview as the reference. **(A) Zero literal colours** in the view and explore sheets: `entity-view.css`'s owner-directed white CTA ink is `--on-primary` (the theme engine now resolves it from one tone in both branches, so "white in both themes" IS a token); every mark over photography (`.evp-canvas__zoom`/`__more`, the article play/duration marks, the `AD` token) takes the `--scrim`/`--on-scrim` pair; `--text-disabled`, read at five sites and declared NOWHERE in the theme (Decision #70a), became `--on-surface-variant`. **(B) The type ramp.** ~110 literal `font-size`/`font-weight`/`letter-spacing`/`line-height` declarations across `explore.css` · `explore-results.css` · `explore-home.css` · `article-view.css` · `service-booking.css` moved onto `--text-*` · `--fw-*` · `--tracking-*` · `--leading-*`; the eyebrow, `.ex-kind`, `.ex-detail__label` and `.ex-detail__fact dt` are the §A.4 section-header register (`--text-xs` · uppercase · `--tracking-wide`); three bold headings (`.ex-proj__title`, `.ex-detail__title`, `.art-header__title`/`.art-h2`, the schedule leaf's `h1`) stepped down per §A.4; `--fw-light` body copy (`.ex-proj__desc`, the promo bodies) returned to `--fw-normal` because the token's own note reserves 300 for large display type. **(C) The card contract**: `.ex-card` takes `--card-radius-media`, its media tile `--card-radius` (the comment had recorded that intent while the rule said `--radius-lg`); the primary canvas (`.evp-canvas__stage`, `.evp-scheduler__cal`) takes `--radius-container-xl` (measured 27.28px at 1440, 19.84 at 820, 16 at 390). **(D) §B.7 iconography**: the explore `RatingStars`'s hand-authored star, the `ProjectCard` `›` chevron, four typed `→` arrows, the `ProfileCard`'s ` • ` bullet, the filter lane's border-drawn caret and checkbox tick all render through the registry; `MediaLightbox` and the article islands dropped `view-glyphs`' duplicate geometries for `<Icon>` at ramp steps (the lightbox's "Download" control had been drawing the `expand` glyph), and `view-glyphs.tsx` keeps only the vocabulary the registry lacks. **(E) The provider line** mirrors `.pf-hero__meta`: avatar · name · crest carrying the attained TIER on hover (`TIER_META`, the profile hero's copy) · one aggregate score as ONE anchor to the reviews · then `@handle · kind · Standing` in the Meta register — additive Zod SSOT `EntitySellerSchema` on `EntityViewSchema`, derived in `buildViewPage` from the SAME `findProfile` projection the project banner reads (no DB migration; the tier enum is IMPORTED from the profile SSOT, never restated). Standing is stated, never badged (§B.11). **(F) Sessions link to the Proactive Calendar.** `BookingActionKind` gains `view_schedule` and `resolveCta` takes an optional `scheduleHref`: the scheduled formats' SECONDARY is an `outlined` navigation to `/view/[id]/schedule` (profile-scoped under `/[handle]/…`), beside the in-page picker that stays the primary because it is the flow that reaches checkout; pinned by 3 new tests. **(G) Two pre-existing carrier holes closed** (§C.1's island-carrier rule, Decision #39): `explore-results.css` reached the Search Results state on NEITHER dev server — the filter lane rendered a native `<details>` marker, unstyled option buttons and a bare range input — so `CardStyleAnchor` now carries it; and the schedule leaf's populated branch mounted no `ViewStyleAnchor`, shipping a default blue underlined back link flush against the viewport edge above a fully styled calendar. Also: `.ex-pricebadge__from`'s `&#32;` was dropped by the JSX transform (`From£70.87`, measured) — an explicit `{" "}` node; the seller meta and the profile-card meta carry real whitespace between spans so a screen reader hears three facts, not one word; a session's "Format" ledger row no longer repeats its "Delivery" sentence. **Verified in-browser** (driven Chromium, `localhost:5373`) on all seven listing shapes + `/explore`, `/explore?category=projects|talent`, `/@juno`, the schedule leaf and the profile-scoped mirror, light + dark, 1440/820/390, LTR + RTL: zero page errors, zero horizontal overflow anywhere, exactly one `filled` control per decision region, no typed glyph characters, worst body-text contrast 6.46:1 light / 8.39:1 dark; `deno task check` (routes included), `deno lint` (1,563 files) and the touched-domain tests green. **FLAGGED — surface, do not silently resolve:** (a) **the inverted primary's label measures 3.57:1 in dark** — `--on-primary` on `--primary` at the owner's `b4a28bc` tones (Decision #96a); the token is the right token and the pair is the theme engine's to fix. (b) `ExploreHomeHeader`'s `HeaderArt` and the marketing shell's footer/header marks are the remaining bare `<svg>` roots — the former is a 1200×260 illustration, not an icon; the footer glyphs (`lp-footer__group-chevron`/`news-arrow`/`tip-icon`) are out of this pass's scope. (c) `.ex-rail__lead` stays `--fw-bold` italic — Decision #76's designed rail heading, not re-decided here. (d) `.ex-hero__title`, `.ex-results-head__q` and the promo titles keep fluid `clamp()` display sizes, which the ramp has no step for. (e) Five DOM-sizing `… svg { inline-size }` rules in `explore.css` still outrank `.ui-icon`'s ramp at (0,1,1); they set the same sizes today. (f) The concurrent profile session's uncommitted edits (Hire rig, `ProfileView.hours`/`responseMinutes`/`freeConsultation`) were left untouched; `view.ts` imports only `VerificationTier`/`ProfileStandingSchema`, which it does not change. | root `CLAUDE.md` §3/§7 · `packages/types/{explore/view,services/common}.ts` + `common.test.ts` · `packages/backend/services/{explore/view-fixtures,booking/BookingBackendService}.ts` · `apps/web/features/view/{components/{entity-view-parts,EntityViewPage,project-view-parts,BookingCtaRig,CtaButton,MediaLightbox,ProjectActions,view-glyphs,ArticleViewScreen},core/view-model,islands/{EntityLane,Article*}.island,styles/*}` · `apps/web/features/explore/{components/**,islands/CardStyleAnchor.island,styles/*}` · `apps/web/routes/{(public)/view/[entity],[handle]/view/[item]}/schedule.tsx` · Decisions #39 / #62 / #74 / #76 / #77 / #78 / #79 / #80 / #92 / #96 |

| 99 | **Explore filters — the facet SSOT, three range controls, the section-header standard, the refine field and the guest sidebar toggle (2026-09-16).** The `/explore` Search-Results sidebar (guest `ui-guest-aside` / authed middle-nav lane, Decision #40) is rebuilt as a RENDERER over a facet list. **(A) THE SCHEMA IS THE SSOT.** New `@projective/types/explore/facets.ts`: a Zod discriminated union of seven facet kinds (`chips · checkbox · select · multiselect · range · rating · milestones`), the pure helpers that are the ONE place "is this filter set" is decided (`facetIsDefault` / `normaliseFacetValues` / `rangeFacetValue` / `ratingFacetValue` / `milestoneFacetValue` / `mergeFacets`), and an additive optional `SearchPayload.facets` — the seam a scope-specific facet (a voice actor's accent, a rate per word) arrives through. The app's `FILTER_CONFIG` is authored against the same shape and parsed through `FacetSchema` at module load; `activeFilterConfigs(category, payload.facets)` merges the two (a same-`id` runtime facet REPLACES the static one) and is what the panel maps over — `FilterPanel` holds a `FACET_RENDERERS` registry keyed by the discriminant, so a new control kind is one entry plus its schema and the section chrome, the reset, the active count and the URL contract need no change. Renderers are mounted as COMPONENTS keyed by kind, never called: each owns its hooks, so a facet whose kind changes under one id remounts instead of corrupting the section's hook order. **(B) THREE `@projective/ui/fields` CONTROLS**, all folding onto one `core/range.ts` (`snapValue` / `resolvePairMove` / `nearestOfPair` / `nearestMilestone`, unit-pinned): `Slider` gains `allowCross` and a focus-gated wheel; **`RangeSlider`** (alias `DualSlider`) composes it over two adorned `NumberInput` boxes; **`StarRatingInput`** is a half-precision star SLIDER (one tab stop, `role="slider"`) over the registry `star` glyph twice — an outline under a width-clipped filled copy — filled with `--ui-star-fill` (default `--warning`); **`MilestoneSlider`** is the discrete non-linear track (the brief's `DeliverySlider`, named for what it IS in a portable package — the delivery stops are app data in `DELIVERY_MILESTONES`). Handle pass-through keeps DOM IDENTITY across the crossing — the sorted pair is re-mapped onto the two handles through a `swapped` orientation — so pointer capture and focus stay on the element the reader is holding while only its spoken role flips; verified by keyboard (End on the minimum past a 6000 maximum: same element keeps focus, now "Price maximum", ArrowLeft moves the max) and by pointer drag. **(C) THE SECTION-HEADER STANDARD**: the label is a real `<button aria-expanded>` and a ghost icon-only Reset (`Button variant="text" iconOnly` + portal `Tooltip` + `aria-label`) sits OUTSIDE it, rendered ONLY once the facet is off its default — absence, not a disabled control, when there is nothing to undo (owner directive), with the slot reserved so the chevron does not shift when it appears; the panel head says `n filters applied` as muted meta text rather than a count badge (owner directive) — a button in a button toggles the section on every reset — replacing the native `<details>`. A range commits after a 220ms settle so a drag does not push a history entry per pointer sample; the URL contract is `?price=100,500` (a pair; the legacy single "up to" value still resolves) · `?rating=3.5` · `?delivery=7` (days), and `query.ts` reads the pair and a new `deliveryDays` facet. **(D) THE RESULTS SEARCH BAR** under the title mirrors `site-header__search` part for part (owner directive) — a fused SCOPE selector (the shared `SEARCH_SCOPES` vocabulary, a keyboard-navigable listbox: ArrowDown opens, Arrows/Home/End move, Enter/Space pick, Escape and an outside click close) → the field → a submit glyph on one hairline-ringed pill (measured byte-identical radius/fill/border to the header's), plus a clear control once typed; the registry's glyphs rather than the header's hand-authored ones. Picking a scope re-scopes IN PLACE through the same `commit` and DROPS the previous scope's facets — they belong to a vocabulary the new scope does not have (`?cat=web` carried from services into products was the measured leak). **(E) THE GUEST SIDEBAR TOGGLE** (a ghost icon-only `filter` funnel beside Sort whose glyph becomes the new registry `filter-off` — the same funnel struck through with the set's one slash — while hidden, owner directive; the active count rides the Button's corner badge) is guest-desktop-only by construction — the authed lane is the shell's splitter, whose collapsed state is a rail and whose width is an inline custom property — and is expressed as `:root[data-explore-filters]`, pre-painted in `_app.tsx` from the new `LocalKeys.EXPLORE_FILTERS_HIDDEN` (the `data-guest-nav` idiom) so a hidden sidebar does not paint and slide away on every navigation; hidden means `visibility: hidden` too, or a collapsed panel's controls would remain a keyboard trap into an invisible region. **THREE DEFECTS FOUND BY MEASUREMENT:** (1) after typing a maximum BELOW the minimum, the still-focused box kept its stale draft and would have re-committed it on blur — `NumberInput` now refreshes a focused draft when its CONTROLLED value changes from outside (a swap, a reset); (2) the pre-existing `Slider` handle sat one full width OUTSIDE the track at either end under `dir="rtl"` (a logical inset centred by a physical `translate(-50%)`) — the Decision #75(10) class, fixed in `slider.css`; (3) a shell heredoc stripped every backslash from a regex before it reached the file (`/(\d+)/` became `/(d+)/`), caught by reading the line back — scripted edits here go through a file, never a heredoc. **Verified in-browser** (driven Chromium, `localhost:5373`) at 1440 and 390, guest and forged-token authed, light and dark (star fill 8.77:1, outline 6.32:1, labels ≥ 8.75:1 dark), LTR and RTL with zero horizontal overflow: keyboard (Tab order handle→handle→box→box, Arrows, Home/End, PageUp/Down on stars), wheel on a focused handle with no page scroll, half-star pointer previews, the reset and Clear-all, the refine field, the toggle persisting across a reload, and the mobile sheet. `deno task check`, `deno lint` and the unit tests (21 new) green. **FLAGGED — surface, do not silently resolve:** (a) the product removed keyboard focus rings globally in `152a27a "Remove tab focus"`; the new controls follow that convention (`:focus-visible { box-shadow: none }`), so keyboard operation is fully functional but visually unmarked — an a11y position that is the owner's, not this pass's; (b) the star fill uses `--warning`, a STATUS token, as an ornament colour at the brief's request — `--ui-star-fill` exists so a monochrome surface (the profile, Decision #96) can re-tone it; (c) sidebar visibility is per-DEVICE (`localStorage`), not a URL parameter — the brief allowed either and the codebase idiom is the pre-painted local key; (d) the authed lane has no hide toggle (see E) — its collapse mechanism is the shell's, and a filter lane collapsed to a 70px rail has no rail presentation; (e) `members`, `budget` and `readMinutes` remain range facets the fixture query does not apply (pre-existing), and `SearchPayload.facets` is emitted by nothing yet — the seam exists, the service does not use it. | `DESIGN_SYSTEM.md` §C.1 (fields roster) · `packages/types/explore/{facets,facets.test,discovery,mod}.ts` · `packages/ui/fields/{core/{range,range.test},islands/{Slider,RangeSlider,StarRatingInput,MilestoneSlider,NumberInput},styles/{slider,range-slider,star-rating,milestone-slider},mod}` · `packages/backend/services/explore/{query,query.test}.ts` · `apps/web/features/explore/{core/{filter-config,filter-bridge,filter-visibility,explore-state},components/{FilterPanel,ResultsHeader},islands/{ExploreFilterLane,SearchDashboard}.island,styles/explore-results.css}` · `apps/web/features/shell/styles/guest-shell.css` · `apps/web/utils/storage-keys.ts` · `apps/web/routes/_app.tsx` · Decisions #12 / #39 / #40 / #62 / #75 / #76 / #96 |

| 100 | **Progressive images, the Recommended fold's dead band, and the hero search bar aligned to the header (2026-09-17).** Three coupled fixes. **(A) THE FOLD'S CARDS HAD A HOLE, AND THE HOLE HAD TWO CAUSES.** The `/explore` Recommended panel hid its cards' classification row (`.ex-fold .ex-card__kindrow { display: none }`, plus the profile card's `location · languages` line) and floored every cell at `min-block-size: 380px` — both leftovers of the fit-to-screen fold (Decision #76) that the owner's `d0940ac` commit had already retired for auto-height rows. A card fills its cell and pins its foot with `margin-block-start: auto`, so the floor opened ~90px between title and foot on a 347px card. The floor is gone and the rows are back, scaled (`--text-2xs` aside, one-line title/headline/meta), so the fold's card discloses the same facts as the body's. **THE SECOND CAUSE WAS LATENT AND SITE-WIDE:** a card is a flex column, its media tile a flex item, and a flex item's `min-block-size: auto` resolves to its content's min-content height — for a tile holding an in-flow `<img>`, the photograph's own intrinsic height. Every `aspect-ratio` in the card family was therefore only ever a FLOOR: measured, a "16:10" service tile at 240px rendered **180px** (its 800×600 photo's 4:3), a freelancer's 1.8:1 cover grew the profile banner to 133px beside a team's 105px in the same row, and the one card whose image had 404'd rendered the declared 150px — 30px shorter than its siblings, handed back to it by the equalised row as a hole above its foot. `min-block-size: 0` on `.ex-media` and `.ex-pcard__banner` makes the declared ratio govern regardless of load state (services 347→317px in the fold, 440→404 in the body — the design's 16:10, for the first time). Products in a RAIL take a fixed 16:10 (`.ex-rail__track .ex-media--free`): the content-height frame exists to make the masonry interlock, and in an equal-height row it only opens a hole (measured 8–168px). Also: `.ex-home__section:empty { display: none }` — the Continue rail's wrapper cost a 52px flex gap on every first visit while its island rendered `null`. **(B) THE PROGRESSIVE-IMAGE PIPELINE — `@projective/ui/display` `ProgressiveImage` (§C.1).** Picture → BlurHash → neutral fallback, as ONE server frame and ONE per-page island. The frame writes the hash's AVERAGE colour as `--pimg-color` at SSR (`blurHashAverage` reads six characters, no trig — so the first byte carries a correctly-toned ground for every image with no JS), and the client watcher (`watchProgressiveImages`, mounted once by `ImageFallbackBridge` in `_app.tsx`) decodes each hash to a 32px canvas → `--pimg-hash`. **The watcher attaches no per-image listener and never rewrites a `src`:** `load`/`error` do not bubble but DO capture, so two `document` listeners see every image including ones that do not exist yet; the race a listener cannot see — an image that finished or failed before hydration — is read from `complete`/`naturalWidth`; a `MutationObserver` re-settles inserted frames and re-sourced or re-hashed ones. No `src` is ever written, which is the only way a reload loop could start. `Avatar` renders its photo through the frame with its initials/icon chain as the fallback slot, so a dead photo degrades to the person's initials on the avatar fill — and does so hash or no hash, because a blurred face says less than initials (a scoped override; elsewhere a hash outranks the glyph on failure). The decoder is self-contained in `packages/ui` (it may import neither the app nor `@projective/types`) and the ENCODER stays app-side; the app's test round-trips a hash through the package decoder, which is the only proof either half reads the format the same way. Narrow sub-paths `./display/image` and `./display/blurhash` exist for the same reason `./display/money` does. **SSOT:** `ImagePlaceholderSchema` (`{blurhash, color?}`, `files/metadata.ts`) + `imagePlaceholderOf(metadata)`; additive optional `mediaPlaceholder` / `coverPlaceholder` / `avatarPlaceholder` on the explore items, owner, sponsored slot and profile view, `placeholder` on `WorkPieceMedia`, `placeholder`/`thumbPlaceholder` on `EntityMedia` — **no DB migration** (a read projection; the live path reads `files.items.metadata`). **Fixtures carry REAL hashes:** `scripts/gen-mock-placeholders.ts` (root tooling — it imports the app's encoder and writes the backend's mocks, which neither member may do) fetched all 84 corpus photographs at three crops and committed 81 to `mocks/placeholders.ts`; the 3 the host no longer serves have no entry and take the fallback, which is the honest outcome for a dead image and exactly what was rendering as a broken icon. Adopted on the explore cards + `OwnerBadge`, the marketing twins, `DetailPanel`, the profile hero avatar + work masonry, the entity canvas + thumbnails, the project banner, and `FileCard` (real metadata). **(C) THE HERO SEARCH BAR IS THE HEADER'S BAR AT A LARGER SIZE.** `hero-search.css` was a 74% glass slab with its own blur and `--elevation-high`, a divider on the field's leading edge, a `--primary`-filled 52px submit inset 8px on every side, and a placeholder in `--on-surface` at medium weight. It now reads the header's tokens part for part (the Decision #99 results-bar precedent): 55% `--surface-2` tint, `--hairline-color`, `--radius-lg`, no blur (§B.4.3 licenses none here), the divider as the selector's trailing edge, `--fw-semibold` selector, `--on-surface-variant` placeholder at inherited weight, and a bare 16px registry `search` glyph in muted ink that goes `--primary` on hover — `data-magnetic` dropped from a control that is now part of the field. Measured: background, border, radius, gap, padding, weight and glyph size byte-identical to `site-header__search`; the hero is 48px to the header's 40px by design. **One contrast miss found by measurement:** the header's 55% tint is a whisper over a near-white page and a MID-TONE over `/explore`'s teal band, where the placeholder measured **4.26:1** in light. Same token, heavier mix on that band only (72% → 5.47:1 light / 6.95:1 dark), scoped in `explore.css`. **NOT verified, stated:** `:focus-visible` rendering (the pane never takes real keyboard focus); the live-path hash flow (no `files.items` row with metadata exists in the stub). **Flagged (surface, do not silently resolve):** (a) the marketing landing's own `landing-data.ts` corpus carries no hashes — the app cannot reach the backend mock table — so `/` thumbnails get the colour-less ground and the glyph on failure, never the blur; (b) `ClientProofStrip` logos and inline article images stay raw `<img>` — an intrinsically-sized image cannot host a sized fallback slot; (c) `.ex-media--free` inside a rail is now 16:10 while the same card in the masonry is content-height — one card, two frames, by container. | `DESIGN_SYSTEM.md` §C.1 + Part C · `packages/ui/display/{components/{ProgressiveImage,Avatar}.tsx,core/{blurhash,progressive-image}.ts,styles/{progressive-image,avatar}.css,image.ts,blurhash.ts,mod.ts}` · `packages/ui/deno.json` · `packages/types/{files/metadata,explore/{items,view},profile/{profile,tabs}}.ts` · `packages/backend/mocks/{assets,placeholders}.ts` · `packages/backend/services/{explore/{fixtures,view-fixtures},profile/profile-fixtures}.ts` · `scripts/gen-mock-placeholders.ts` · `apps/web/routes/_app.tsx` · `apps/web/features/shell/islands/ImageFallbackBridge.island.tsx` · `apps/web/features/explore/{components/**,styles/{explore,explore-home}.css}` · `apps/web/features/marketing/{components/*Card.tsx,islands/SearchBar.island.tsx,styles/hero-search.css}` · `apps/web/features/{profile,view,projects}/**` (adoption sites) · Decisions #39 / #62 / #69 / #74 / #76 / #77 / #99 |
| 101 | **Profile — the showcase carousel, the in-place photo editor, and the owner-written headline (2026-09-17). EXTENDS Decisions #96 / #97.** Three changes to `/[handle]`, and the reasoning that decided each. **(A) THE SHOWCASE IS A SET, NOT A SLIDE.** The Zod SSOT's `ProfileShowcase` was one `{kind, src, poster, alt}`; it is now `{ primary, extras }` — ONE primary still (typed `kind: "image"` by literal, because it is the thumbnail every card and preview of the profile leads with and a video cannot be a thumbnail) plus up to `SHOWCASE_EXTRA_MAX = 4` slides of stills or full-length videos, with `showcaseSlides()` as the one flattening. The hero's frame is the page's one raised surface (`--radius-lg` + `--elevation-low` — a carousel is a control, so §B.4's elevated-control allowance is its due), and the new `ProfileShowcase` (a stateful component inside the hero island) drives it: the track moves on `transform` alone in container-query units (`100cqi` of the viewport), so no script measures a width to place a slide, and the writing direction flips the sign through one `--pf-dir` custom property (measured: `+730px` for slide 2 under `dir="rtl"`, `-730px` under LTR, rail centred to the pixel in both). **The playback rule is a pure function** (`advancePlan`, `showcase-model.ts`): a still dwells 5s; a video is `await-video` while it plays — no timer armed — then holds its last frame for the same dwell with a Replay control before advancing; hover or focus-within holds it (the WCAG 2.2.2 pause, reachable by keyboard); under either reduced-motion channel nothing moves on its own and the video slide offers Play instead of starting. **Manual activation, deliberately:** the rail is a `tablist` whose arrows move the ROVING FOCUS between dots without selecting and Enter/Space selects — the brief's model, and verified: ArrowRight moved focus 1→2 with the selection unchanged, Enter then selected. A real pointer swipe commits on distance or a flick (`resolveSwipe`), damped past the ends (`dampedDrag`), with `touch-action: pan-y` leaving vertical scroll to the browser — both direction-aware, since the same physical drag reveals the PREVIOUS slide under RTL. Inactive slides are `inert`. The fixtures spread three states (a set with a showreel · stills only · none) so the collapse case stays reachable from the stub. **(B) THE AVATAR IS THE CONTROL.** The satellite `.pf-hero__imgbtn` beside the avatar is gone; for the owner the avatar is a circular `<button aria-haspopup="dialog">` with an "Edit photo" scrim overlay on hover AND focus (a keyboard user sees the same promise), collapsing to a small badge where there is no hover. It opens `AvatarEditor`, a two-column `Dialog` (`--overlay-w-xl` cap; the two columns STACK on a phone through a `.pf-cropscope` container — the first build put `container-type` on the grid itself, and a container cannot query itself, so at 390px the crop column measured **0px**; found by measurement). LEFT: a square stage with the circle inscribed, the picture under a `::before` scrim that is darkened + blurred with the circle cut out by a radial `mask-image` (§B.4.3's licensed glass-over-photography, on the underlay), lifting while the pointer is down; drag pans, Ctrl + wheel zooms (a plain wheel scrolls the dialog — `{ passive: false }` by hand so `preventDefault` holds); beneath it Zoom (`Slider`), Rotation (the package `Knob`, `{value}°`), and Offset X ⁄ Y each as a `Slider` beside a `NumberInput` bound to one value. RIGHT: "Upload from this device" (a hidden file input, an object URL) and "Choose from your files" — the shared `AssetPicker` mounted INSIDE the dialog so the overlay registry names the dialog as its opener (verified: the picker opens above at z 1310, the dialog stays; Escape closes the picker only, a second Escape the dialog, and focus returns to the avatar button). **THE INVARIANT HAS ONE IMPLEMENTATION.** `crop-model.ts` is pure and total: the state is *which point of the picture sits at the circle's centre* (`cx, cy` in source pixels) plus zoom and rotation, so zoom and rotation happen ABOUT the crop centre, and — because a rotation maps a circle onto a circle — "the circle lies inside the picture" is an axis-aligned rectangle of legal centres in the picture's own frame that does not change with rotation. Every gesture and every control writes through `clampState`; there is no second arithmetic path. Verified live with a generated 400×300 upload: fit scale 1.0667 (= 320/300), X bounds ±50, Y pinned and its controls disabled at zoom 1; a 40px drag moved the picture exactly 40px and the X box to 38 (40/1.0667); Ctrl + wheel → 1.82× with the bounds widening to ±117/±67; a typed 99999 clamped to 117; three PageUps on the dial → 30° with the translate rotated to match. **Save is honest about pixels.** The picture is loaded once in CORS mode (the only mode a canvas may read back) and, where the host refuses, plainly for the preview alone; Save then applies the picture uncropped and SAYS so, because a crop the browser will not let us read cannot be claimed to have been applied. The export is a 512×512 PNG (verified: the hero avatar became a `blob:` URL with `naturalWidth` 512) — PNG so an uncovered corner of a rotated crop is transparent rather than painted, and square because the circle is the renderer's to draw. Session-local through `editedAvatar`, like every inline profile edit, pending the write path. **(C) THE HEADLINE IS THE OWNER'S, OR IT IS NOTHING.** The fixture fallback `"Independent maker on Projective"` is gone — a synthesised handle now has an EMPTY headline, and `storyOf` no longer interpolates it (an empty headline produced "I'm Ivy, ." as the person's own words). The `ProfileStory` island now renders the headline over the story through one internal `EditableText` (a single-line `InputText` at the headline's own metrics — measured 22px in edit mode — beside the auto-resizing `Textarea`), so the two lines cannot drift into two editing idioms sixteen pixels apart; the headline commits on Enter and both cancel on Escape (the keys are read on the wrapper, since `InputText` exposes no `onKeyDown`); a visitor's empty headline renders nothing, never a default. **(D) LABEL CLEANUP:** the Reviews stance segments carry the stance alone (the counts stay in the status line and the empty note) and the Services heading loses its count. **Contrast, measured over a blown-out white photograph — the worst case for a control over media:** the rail ink 5.32:1 (a 45% scrim measured 3.35, which passes and leaves nothing spare; now 58%), the muted dot 3.65:1 against its own glass (a 55% dot measured **2.82**, under the 3:1 floor a graphical control owes; now 72%). **Also fixed from the probe:** a video whose media never arrives now fires `onError` → treated as finished, so a dead video cannot hold the carousel hostage (the harness blocks external media, which is exactly the state that exposed it); the `grabbing` cursor lost on specificity to the resting `grab`. **No DB migration** — the profile stays a read projection over fixtures and every edit is optimistic (Decision #96's footing); **no new `@projective/ui` primitive** (reuses Dialog · Slider · Knob · NumberInput · Tooltip · ProgressiveImage · AssetPicker) → no §C.1 change; no lifecycle change → no `PRODUCT_MANAGEMENT.md`; no new simulatable axis a surface branches on (ownership is server-resolved from the JWT) → nothing to mirror in the Dev Context Switcher (§5). `PRODUCT_SPEC.md` §The Public Profile updated in the same change. Verified in-browser (driven headless Chromium, `localhost:5373`) at 1440 / 820 / 390, LTR + RTL, reduced motion, guest + forged-token owner: 16 new unit tests, whole-repo `deno task check` clean, zero console errors, zero horizontal overflow anywhere. **NOT verified, stated rather than claimed:** the video play-through → hold → advance sequence and the Replay control in a live browser — the harness blocks external media (`duration` never arrives), so the rule is pinned by `advancePlan`'s unit tests and the video slide's `onError` path is what actually ran; `:focus-visible` rendering (the pane never takes real keyboard focus); the `(hover: none)` badge (no touch emulation in this probe). **FLAGGED — surface, do not silently resolve:** (a) **the rail's focused dot brightens to its hover ink** — a ring-less marker, because the product draws no focus rings (commit `152a27a`), but a roving-focus tablist whose focused dot is invisible is unusable by sight, so this one control carries a colour marker; (b) the dot-to-pill morph transitions `inline-size`, a selection-state change rather than press feedback (§B.12 governs the latter) — reduced motion lands it instantly; (c) the carousel's pause is hover ⁄ focus-within, not an explicit pause control — WCAG 2.2.2 is satisfied for pointer AND keyboard users (tabbing to the rail holds it), but a visible pause button would be the belt to that brace; (d) a cross-origin picture whose host sends no CORS header is saved UNCROPPED with a status note — the honest outcome, but the owner asked for a crop and did not get one; the fix is to serve the profile's own avatar from an origin that allows it (the live storage bucket does); (e) `SHOWCASE_EXTRA_MAX` is enforced by the SSOT, but no owner surface yet adds or orders extra slides — the picker still replaces the PRIMARY still only, and slide management is a write-path feature. | `PRODUCT_SPEC.md` §The Public Profile · `packages/types/profile/profile.ts` · `packages/backend/services/profile/profile-fixtures.ts` · `apps/web/features/profile/{core/{showcase-model,crop-model}(+tests),components/{ProfileShowcase,AvatarEditor,ProfileContextBar,ProfileServicesSection},islands/{ProfileHero,ProfileStory,ReviewsList}.island,styles/{profile,profile-hero,profile-showcase,profile-avatar-editor,profile-context}.css}` · Decisions #60 / #62 / #72 / #96 / #97 / #100 |

| 102 | **Messaging — one lane on every `/messages` route, group creation, the relationship-ranked people picker, and the unified share modal (2026-09-18). PARTLY REVERSES the messaging half of Decision #63 at the product owner's direction.** #63 moved the inbox list from the lane into the root BODY and retired the root's placeholder; the owner now wants the lane on `/messages` **identical** to the lane beside an open conversation, and the root body to be an empty state with a CTA. So `messagesLaneFor` resolves ONE `MessagesSidebar` for the whole hierarchy (search · partitions · filters · row actions · New message · Settings, state persisted in `LocalKeys.MESSAGES_FILTERS` / `CONVERSATION_PREFS`, the active row from the pathname), the root's header and footer bands are gone (the lane owns search and both global actions; a second search on the same screen is a control that can disagree with its twin), and the body is `MessagesEmptyState` — glyph, one sentence, **New message** + **New group**. **#63's mobile finding is kept, not reversed:** below 767px the shell removes the lane, so `MessagesRoot` renders BOTH presentations and CSS shows exactly one — the empty state on desktop, the transferred `InboxHeader` + `InboxFooter` + `InboxView` on a phone (the `/wallet` header-switcher precedent: moved, never duplicated). `InboxScopeLane` and the count computeds it consumed are deleted. **Measured, not inspected:** the transferred header's 69px of content was being squeezed to 48px with its title clipped under the top bar — `.inbox { min-block-size: 100% }` is right for the lane, where it is the only child, and wrong as a flex sibling, where the percentage resolves against the whole column and shrinks both strips to their minimum; the strips are now `flex: none` and the list takes the remainder. **(B) GROUP CREATION WAS BROKEN END TO END**: `POST /api/messaging/conversations` fabricated a `grp-new-…` id no read could resolve, and the "Add members" modal IGNORED its request's `conversationId` and minted a new group instead of extending the thread — a control that appeared to work and did something else. Now: `CreateConversationSchema` / `AddConversationMembersSchema` in the SSOT (`packages/types/messaging/contacts.ts`); `MessagingBackendService.{createConversation,addMembers}` (fat); `ContactsService` (thin client, beside `MessagingService`); the RPCs **`comms.create_group_thread`** and **`comms.add_dm_thread_members`** (`00001300`, definer, `EXECUTE` to `authenticated` only) because `dm_threads`/`dm_participants` carry no client `INSERT` policy on purpose and a policy admitting "a participant may add a row for somebody else" is the shape that lets anyone be added to anything. One pick → the pair's DM (reopened if it exists — `created: false`); several, or a named pick → a group; an opening `message` posts in the same act (Decision #79). Adding a third person flips a DM to `group`; a member who self-deleted is RESTORED by being added back. **`comms.get_or_create_dm_thread` gained `WHERE t.kind <> 'group'`**: the join alone returned any thread holding both people, so "message this person" from a profile could land in a group everyone else reads; a `service_inquiry` stays the pair's thread (one record per pair). The stub path mints into a new per-process **`conversation-store.ts`** (the sibling of the message write store) which `findConversationSummary` consults FIRST, so a created group resolves through detail, messages and the peer session's `sendMessage` unchanged, and joins the list the moment its first message lands (the `messageCount > 0` rule, counted from the write store). **(C) THE RANKED PICKER.** `GET /api/messaging/suggestions` → `MessagingBackendService.suggestions`: no query → the viewer's relationships ranked **shared workspace → mutual follow → follow → collaboration → conversation**, each by most-recent interaction; a query → the same people narrowed plus directory hits at tier `none`. **The rule lives once** — `deriveContactRank` over a `ContactEvidence` record in the SSOT — and both branches produce EVIDENCE, never a tier: the live path from `org.team_members`/`business_members`/`organisation_members` (+ names), `org.profile_follows` in BOTH directions, `projects.projects` + `project_participants` (two shapes, because the participants policy is owner-or-public: the freelancers on projects the viewer OWNS directly, the owners of projects the viewer was hired onto through the rows `has_project_access` admits, narrowed by `or(status.neq.active,visibility.neq.public)` so the public marketplace is never read as a collaboration — the #82 trap), and the viewer's threads; the stub from the same casts the app renders plus a small declared follow graph (the only one that exists anywhere). `conversed` sits BELOW the brief's four tiers deliberately — the weakest evidence, but on the live path today the only durable one (`live-contacts.ts`, which records that "nothing stores a follow", predates `org.profile_follows`). Being followed WITHOUT following back earns no tier. A brand-new account gets an honest EMPTY suggestion list and a search box — never a fabricated relationship — which is why a `[]` here never falls through to fixtures. The directory search is narrowed in SQL (sound here where `live-contacts` explains it is not: the set is unbounded), by a needle tokenised to `[letters·digits]` before it touches PostgREST's `or=(…)` grammar. `ContactList` + `useContactSearch` render the server's order and never sort. **(D) THE SHARE MODAL** (`apps/web/features/share/`): one `ShareHost` island per shell (`UserShell` + `GuestShell`, not the layouts) driven by the `share-request` bridge, lazy-loading its body on first open with BOTH stylesheets imported statically by the host (Decision #39's carrier rule; a lazy chunk's CSS emission in a production build is not something the host may rely on). Tier 1 is the SAME `ContactList` — a share is a message with a link, sent per recipient through `createConversation({message})` and reported per recipient ("Sent to Ivy Chen." / "Couldn't reach …"); a guest gets the standard sign-in route instead of a picker that would 401 on Send. Tier 2: Snapchat · WhatsApp · Facebook · Instagram · X · Telegram · Copy link, plus `•••` for `navigator.share` **rendered only where the sheet exists** (§3 gate 11). The intents are pure, unit-tested URL templates (`share-targets.ts`) escaped exactly once. **Instagram has no web share intent** — no URL opens it with a link or caption pre-filled — so its button hands the request to the device sheet where one exists and otherwise copies the link and opens Instagram to paste into, recorded as a distinct `mode` rather than a fake URL. Brand marks are feature-local, quarantined like `footer-icons.tsx` (Decision #62). Wired: explore `CardActions`, the entity + project view lanes, the projects card and sidebar-header kebabs. **(E) A SECURITY HOLE FOUND BY BEING ITS FIRST READER**: `org.profile_follows` was created with RLS OFF and never named in `00002001`, under `00002500`'s `GRANT ALL … TO anon, authenticated` — any caller, signed in or not, could forge a follow as anyone, delete anybody's, or rewrite the graph; every profile's follower count was forgeable by anyone with the URL. RLS on + three policies (public SELECT — the table's own comment calls it "a public, counted edge" and a mutual follow needs the incoming half; INSERT/DELETE own edges only; no UPDATE, a follow has no mutable column). **Verified by execution** inside `BEGIN … ROLLBACK` against the local Postgres and then applied: both RPCs on every branch (named group of three, blank title → NULL, the DM lookup no longer returning the group, idempotent, DM→group on a third person, repeat adds nobody, self-deletion restored, phantom uuid dropped, stranger refused, `anon` holds no EXECUTE), the forged follow refused, the own follow readable. **Verified in-browser** (mock mode, fabricated session, 1440 + 390): the root's lane + empty state at desktop and the transferred list at phone width with zero horizontal overflow either way; New group → picker with the four tier groups, reasons and recency, chips, name field → create → `/messages/grp-launch-squad-2` with header, tabs, composer → send → after reload the message is in the feed and the group is listed and highlighted; global search reaching a directory hit; the share modal from a discovery card with all seven quick actions on one row, the ranked people, and a real send landing note + link in the pair's DM; `deno task check` (routes included) clean, `deno lint` clean, **1,314 unit tests** (25 new) with the only failures the four pre-existing `theme-engine.test.ts` ones (#96(a)). **NOT verified, stated rather than claimed:** the LIVE path over HTTP — Docker Desktop crashed mid-session after the SQL was applied and verified, so `live-suggestions.ts` and `live-conversation-writes.ts` are type-checked and reviewed against the migrations only, never fed a returned row; `:focus-visible` rendering; `navigator.share` (absent in the preview's Chromium, correctly rendering no `•••`). **No new Dev Context axis (§5), and that is a judgement:** the ranking is DATA plus the clock, guest/authed is server-resolved, and the share sheet's presence is a device capability — nothing here reads a `data-dev-*` attribute. No new `@projective/ui` primitive → no §C.1 change; no lifecycle state → no `PRODUCT_MANAGEMENT.md` change; the two RPCs, the DM-lookup guard and the follows policies are folded in place (§1) and documented in `comms/Functions.md` + `org/Policies.md`. **FLAGGED — surface, do not silently resolve:** (a) `org.education_entries`, `org.experience_entries` and `org.user_languages` are in the SAME state `profile_follows` was (RLS off under the blanket grant) — out of this blast radius, and each wants its own read of what a policy should admit; (b) the organisation tier resolves only for an organisation ADMIN — a plain member's roster is admin-readable by policy, so their organisation contributes nobody (a true answer, not a failure); (c) a project the viewer was hired onto surfaces its OWNER as the collaborator, never co-participants on a private project (their participant rows are owner-or-public), and `stage_assignments` is not consulted; (d) the profile's Share control lives in the concurrent session's `profile/` tree and is not yet pointed at `requestShare` — one call when that tree settles; (e) the desktop root lost the row-density control that lived in the retired footer band (the lane list has no density axis); (f) a created group's lane row reads "No messages yet" until reload because the write store exposes a count but not the last message's text; (g) `/projects` still has #63(a)'s mobile failure — its root body points at a lane that is not there; (h) during verification a service edit under HMR left two instances of the conversation store in one dev server (a created DM resolved but did not list until restart) — a dev-only artefact of the "restart after a `packages/*` edit" class, recorded so nobody debugs it as a store bug; (i) `MessagingService.create` is kept for the profile inquiry (peer's tree) and answers through the rewritten POST — the payload it sends is a subset of `CreateConversationSchema`. | root `CLAUDE.md` §1/§2/§3/§5/§6/§7 · `PRODUCT_SPEC.md` §Messaging E · `SYSTEM_ARCHITECTURE.md` §Messaging services · `documentation/database/{comms/Functions,org/Policies}.md` · `packages/types/messaging/{contacts,contacts.test,mod}.ts` · `packages/backend/services/messaging/{suggestion-fixtures,live-suggestions,conversation-store(+test),live-conversation-writes,MessagingBackendService,conversation-fixtures}.ts` · `apps/web/routes/api/messaging/{suggestions,conversations,conversations/members}.ts` · `apps/web/features/messaging/{core/{ContactsService,messaging-state,inbox-slots,inbox-state},hooks/useContactSearch,components/{ContactList,ContactPicker,NewConversationModal,MessagesRoot},islands/{MessagesEmptyState,MessagesSidebar,ConversationHeader,InboxView,InboxFooter},styles/{contact-picker,messages-root,messages,inbox}.css}` · `apps/web/features/share/**` · `apps/web/features/shell/components/{UserShell,GuestShell}.tsx` · `apps/web/features/{explore/islands/CardActions.island,view/islands/{EntityLane,ProjectLane}.island,projects/components/{ProjectCard,SidebarHeader}}.tsx` · `apps/web/routes/(dashboard)/{_layout,messages/index}.tsx` · `supabase/migrations/{00001300,00002001,00002010,00002510}*` · Decisions #39 / #49 / #50 / #52 / #62 / #63 / #79 / #82 / #83 |

| 103 | **Profile Hire invitation modal + the persistent floating messenger, and the inbox's first SEND (2026-09-18). EXTENDS Decisions #49 / #97 / #102.** Two flows on `/[handle]`, and the write both needed underneath. **(A) THE MESSENGER IS THE EXISTING POP-OUT, NOT A SECOND WINDOW.** The profile's Message control used to open its own `DraggablePopover` holding a bare composer that NAVIGATED to `/messages/dm-{handle}` on the first send. It now hands the conversation to the global `ChatPopoutHost` (Decision #49's sessionStorage-persisted floating chat) through `openPopout`, and that host is mounted by EVERY authenticated layout — `(dashboard)`, the authed `(public)` branch and the authed `/[handle]` branch — so a conversation opened on a profile is still open, at the same size and position, on Explore, the landing page, a project or the inbox: Fresh navigations are full documents (#52), so "persists across route transitions" is delivered by the store being the state and the window a projection rebuilt identically on the next page (verified: opened on `/@juno`, present with its message on `/explore` and on `/messages/dm-juno`, where the "Open in inbox" header action correctly withdraws). `DraggablePopover` gained an `anchor` (`bottom-end` · `bottom-start`, LOGICAL, resolved from the panel's measured size in a layout effect so it lands before the first paint) and a resize re-clamp; the state carries `source` (a profile-started window docks and reads "Open in inbox"), the counterparty's `avatar`, and the user's last SIZE beside the last position (both persisted; re-opening the same conversation keeps its geometry). Below 768px the host renders nothing and the profile opens the SAME `PopoutChat` body in a bottom-sheet `Drawer` (Part D.3). **(B) THE SEND IS ONE DOOR.** `SendConversationMessageSchema` (`@projective/types/messaging/send.ts`, one field apart from the project send) → `POST /api/messaging/messages/send` → `MessagingBackendService.sendMessage`; the shared `ChatComposer` posts there in its `conversation` scope and announces the SERVER's row on `MESSAGE_SENT_EVENT`, so the floating window, the pop-out and `/messages/[conversationId]` send through one endpoint and every feed on the page appends what was stored (measured: one send from the inbox footer appended to the page feed AND the open window). Live: a uuid is the thread, a unified `dm-{handle}` is resolved through `org.users_public` + `comms.get_or_create_dm_thread` so a first message CREATES the conversation in the same act, and the row is inserted under the new `send_dm_messages_if_participant` policy (`00002012`, `sender_user_id` pinned to `auth.uid()`; the inbox's ONE write, joining the SELECT-only set #82 established). Stub: a per-process message write store (`services/messaging/write-store.ts`) folds sent rows onto the latest page so they survive a reload, and a first message to a synthesised `dm-{handle}` is remembered in #102's conversation store so it joins the inbox list. `findConversationSummary` now falls back to the PROFILE corpus for any `dm-{handle}`, so Message works on every profile the site renders rather than the dozen in the inbox's own cast. **Attachment parity is structural** — the window mounts the same `ChatComposer` (Plus menu → Upload from Device · Attach from Library through the `AssetPicker`, paste, and `PopoutChat`'s whole-panel drop zone) — and the composer gained a `notices` mode: `inline` keeps #66's rule for the in-frame composer, `toast` (what the floating hosts pass) sends the same structured capture and send failures to the shared stack at `bottom-center`, which the host mounts once; bottom-anchored toasts now rise INTO view rather than dropping from above (`toast.css`). Measured with a simulated persisted block: `role="alert"`, the struck-mic mark, the browser-specific recovery steps, centred at 640 of 1280 and 0px from the bottom, and no inline notice in the window. The composer inside the window is pinned to its foot by the flex column (a sibling of the scroller, never a child — verified across a drag-resize to 798×517), its top padding drops from `--space-5` to `--space-2`, and its inner measure is capped at `min(100%, 40rem)` so a wide window does not stretch the bar edge to edge (measured 640px centred 78/78 in a 798px window). **(C) THE HIRE MODAL.** Picking a project in the Hire popover now closes it (the popover is controlled) and opens `HireInviteModal`: the project overview (title · lifecycle status · format · stage count · currency · a plain-text summary), the roster (avatar · name · role · assigned stages as middot text), a multiline intro message, stage checkboxes each with a compensation box, and a footer total. **The compensation field follows the engagement's shape, never a control**: `hirePricingModelFor(format, structure)` in the new Zod SSOT `@projective/types/projects/hire.ts` yields `per_ticket_stage` for a pipeline, `per_stage` for a multi-stage one-off and `task` for a single-stage one-off or Direct Deliverable (one price, no stage list — a list of one with a checkbox is a choice with nothing to choose), each seeded from the stage's configured `unitPriceCents` (a Direct Deliverable from the project budget). `HireBrief` is a COMPOSITION of the owner's `ProjectSetup` and the `MemberRosterPage` (`buildHireBrief`, pure), served by `GET /api/projects/hire` → `ProjectBackendService.hireBrief` — no fourth read of the project; `POST /api/projects/hire` → `ProjectBackendService.hire` re-validates the `HireInvitation` through the SAME `hireInvitationRefusal` the modal gates its Send with, records one pending `MemberInvite` per selected stage (`MemberInviteSchema` gained an optional `handle` — a client hiring from a profile names a person, not an inbox), and the roster read folds them onto its pending queue. Money enters in major units and is stored in minor units through the finance SSOT's exponent-aware converters; the total is the SSOT's `hireInvitationTotalCents`. A refusal (an unpriced selected stage, a task-priced project sent stage offers, an unknown or duplicate stage) is stated in place on the Send attempt, and a sent invitation stays on screen as a success state with the offer and a link to the roster. Verified in-browser (mock mode, forged session): popover → modal on all three models (a 7-stage pipeline at `US$2,550.00` across six selected stages after deselecting the unpriced one, a multi-stage one-off, a Direct Deliverable at its `US$4,800.00` budget), the refusal marking the unpriced box invalid, the POST landing `201` with six `pending` invites, and `/api/projects/members` listing them. **(D) A PRE-EXISTING FEED DEFECT FOUND BY THE PEER SESSION AND FIXED HERE:** `ChatFeed` decided its empty state from the SSR page's `total`, a constant, so the FIRST message into a brand-new conversation was sent, announced, appended to the list — and the feed still said "quiet" until a reload. The empty state now follows the live list (verified: empty → first send → the row, no reload). **No new `@projective/ui` primitive** (the `anchor` prop is an additive change to `DraggablePopover`) → no §C.1 change; no lifecycle state → no `PRODUCT_MANAGEMENT.md` change; no new simulatable axis a surface branches on (the modal's pricing model is DATA; the messenger's mobile/desktop split is a media query) → nothing to mirror in the Dev Context Switcher (§5). `PRODUCT_SPEC.md` §The Public Profile, `SYSTEM_ARCHITECTURE.md` §Messaging services and `comms/Policies.md` updated in the same change; the policy is folded in place (§1). `deno task check` clean over the whole tree, `deno lint` clean over 1,616 files, **1,314 unit tests** (10 new) with the only failures the four pre-existing `theme-engine.test.ts` ones (#96(a)). **NOT verified, stated rather than claimed:** the LIVE send path — Docker's Linux engine was down for the whole pass, so `insertDmMessage` and the INSERT policy are type-checked and structurally reviewed against the migrations only, never fed a returned row; the window's enter motion and the toast's rise (the pane composites no frames — geometry was measured, motion was not); `:focus-visible` rendering. **FLAGGED — surface, do not silently resolve:** (a) **the Hire invitation persists in the per-process store on BOTH sides of the gate**: `projects.project_invitations` addresses an invitee by `target_email`, which the inviter cannot resolve for another user under RLS (`org.user_emails` is own-rows-only), and carries neither the compensation nor the intro message — a live insert would record a success the row does not describe, so the live path needs an identity-addressed, compensation-bearing invitation shape first (a schema decision, §1). (b) A stub-path invitation's `MemberInvite.email` carries the `@handle` as its display line, because the queue's row prints that field and the invitee has no email the inviter may see. (c) A SESSION-format project reaches the Hire popover (it is an open project the viewer owns) and resolves to the `task` model; sessions are booked, not hired into, and the popover should probably not list them — a product question left to a human. (d) The hire brief is fetched on open (a modal), not in the first byte — the popover's rows are the SSR fact; the modal's contents are a click away. (e) Pop-out windows are desktop-only by construction; a drag-resized window that then shrinks the viewport is re-clamped but not re-docked. (f) The `useEffect` that forwards `quickMessageOpen` to the host consumes the flag, so the mobile sheet and the desktop window are decided by the live viewport at press time, not at page load. | root `CLAUDE.md` §1/§2/§3/§5/§6/§7 · `PRODUCT_SPEC.md` §The Public Profile · `SYSTEM_ARCHITECTURE.md` §Messaging services · `documentation/database/comms/Policies.md` · `packages/types/messaging/{send,mod}.ts` · `packages/types/projects/{hire,hire.test,members,mod}.ts` · `packages/backend/services/messaging/{MessagingBackendService,live-writes,write-store(+test),live-queries,conversation-fixtures}.ts` · `packages/backend/services/projects/{ProjectBackendService,write-store,live-writes}.ts` · `packages/ui/overlay/{islands/DraggablePopover.tsx,mod.ts}` · `packages/ui/feedback/styles/toast.css` · `apps/web/routes/api/{messaging/messages/send,projects/hire}.ts` · `apps/web/routes/{(public),[handle]}/_layout.tsx` · `apps/web/features/messaging/{islands/ChatPopoutHost.island,components/PopoutChat,core/{popout-state,MessagingService},styles/chat-popout.css}` · `apps/web/features/projects/{islands/{ChatComposer,ChatFeed}.island,core/HireService}` · `apps/web/features/profile/{components/HireInviteModal,islands/{ProfileHero,ProfileMessagePopover}.island,core/profile-model,styles/{profile-hire,profile-message}.css}` · `supabase/migrations/00002012_policies_comms.sql` · `.claude/launch.json` · Decisions #49 / #52 / #66 / #82 / #96 / #97 / #101 / #102 |
| 104 | **Sign-in form — one identifier (email OR username), the shared Checkbox, in-field floating labels, and Enter that moves before it submits (2026-09-18).** Four coupled changes to `CredentialsForm`, the ONE form both `/login` and the guest `SignInPrompt` dialog compose, so every one of them lands on both surfaces by construction. **(A) THE IDENTIFIER.** The email field becomes "Email or username": one predicate, `isIdentifier` (email shape OR the join wizard's `USERNAME_RE`), read by the form's instant check (`identifierError`, whose message names the vocabulary the reader was using — an `@` means they were typing an email) AND by `LoginSchema.identifier`'s refinement, with a test pinning that the two still agree — a value the form lets through and the route refuses is a submit button that does nothing. The route never inspects which it is: the fat `AuthBackendService.authenticate` tells them apart and resolves a username through a **service-role** read of `org.users_public` (the caller holds no session yet, so an RLS-scoped read would show them nothing) to the GoTrue identity's email — the address the password grant is actually checked against. The lookup asks for the value as typed AND lowercased (`username` is `UNIQUE` but not case-insensitively, while every handle the wizard writes is lowercase) and an exact match wins, so a mixed-case row is never shadowed by its lowercase twin; `.in()` rather than `ilike`, because `_` is a LIKE wildcard and a legal username character. **Every failure answers ONE refusal** — unknown username, unknown email, wrong password — or the form becomes a handle-enumeration oracle. The `requiresVerification` branch now returns the resolved `email`, because a reader who signed in by username cannot route themselves to `/verify` otherwise; GoTrue checks the password BEFORE it reports an unconfirmed email, so that branch is reachable only by somebody holding the credentials. Retired: `LoginSchema.email` (pinned absent by test). **(B) THE CHECKBOX** is the `@projective/ui` `Checkbox` bound to the `remember` signal; `.auth-remember` and its `accent-color` native input are deleted. **(C) IN-FIELD FLOATING LABELS.** `FormControl` keeps the hint/error wiring and gets no `label`; each control is wrapped in `FloatLabel variant="in"`, whose label stays INSIDE the box. That variant had never grown the field, so a floated label sat on top of the value; it now takes the two-line geometry `IftaLabel` uses, hoisted into shared `--fld-h-infield` / `--fld-infield-pt` / `--fld-infield-label-top` / `--fld-infield-label-fs` tokens so the two in-field patterns cannot show two heights. `DESIGN_SYSTEM.md`'s "FloatLabel … never a form carrying validation" is REFINED rather than contradicted: the collision it names is the `over` variant leaving the box; `in` composes with a reserved hint row, and the note now says which. The identifier→password gap went **~74px → 28px** (measured), with the hint row still reserved, so an appearing error moved the password field **0px**. A new `--fld-focus-fg` (`--primary` mixed 30% toward `--on-surface`) inks the focused label: raw `--primary` as 12px text measured 4.59:1 dark / 4.87:1 light against the focused fill — AA with under half a point spare — and the mix measures **6.25:1 / 6.93:1**, rising in both themes by construction because `--on-surface` sits opposite the brand in both. The label's `transform-origin` was physical (`left center`) and mirrors under `dir="rtl"` now (measured: 12px from the START edge either way, zero overflow). **(D) KEYBOARD.** `InputText`/`Password` gain an explicit `onKeyDown` (the `onBlur` reasoning: a prop that only works by accident is not a contract) plus `autoCapitalize`/`spellcheck` on `InputText`; the identifier carries `autocomplete="username" autocapitalize="none" spellcheck="false"`. Enter on the identifier moves focus to the password **when it is empty** — read from the DOM, not the signal, since a manager's autofill can land without an `input` event — and otherwise submits as Enter always has; Enter on the password submits through native implicit submission, which clicks the form's FIRST submit button, which is why the show/hide toggle's `type="button"` is load-bearing and is now documented as such. **Verified in-browser** (driven Chromium, both surfaces, light + dark, 1280 + 390, LTR + RTL): the label association, the payload (`{identifier, password, remember, redirectTo}` for both an email and `ada.lovelace`), focus moving with no POST, Enter-on-password POSTing with the mask untouched and `aria-pressed="false"`, the checkbox flipping `remember`, all three validation messages, zero page errors. `deno task check` clean over the whole tree, `deno lint` clean over 1,618 files, unit **1,328 passed** (14 new) with the only failures the four pre-existing `theme-engine.test.ts` ones (#96(a)), confirmed identical on a clean tree. **NOT verified, stated:** the LIVE username resolution — `PROFILE`/`AUTH_BACKEND_LIVE` were off and no local GoTrue was up, so `resolveUsernameEmail` is type-checked and its two pure halves unit-pinned, never fed a returned row; `:focus-visible` rendering (the pane takes no real keyboard focus). **FLAGGED — surface, do not silently resolve:** (a) a username miss returns without calling GoTrue, so it is FASTER than a wrong password — a timing side-channel on handle existence, mitigable with a constant-time floor if handle privacy matters; (b) `.ui-field-hint` is a `<p>` whose UA 1em bottom margin (13px) is reset nowhere in `field.css`, so every `FormControl` in the product carries it — zeroed here only inside `.auth-credentials`, because removing it globally re-spaces every form; (c) the stub sign-in accepts any identifier with any password (pre-existing — the gate is the whole check). | `PRODUCT_SPEC.md` §Authentication surfaces · `SYSTEM_ARCHITECTURE.md` §Authentication & Identity · `DESIGN_SYSTEM.md` §C.1 (fields) + the labelling-model note · `packages/ui/{styles/index.css,fields/{styles/{float-label,ifta-label}.css,components/FloatLabel.tsx,islands/{InputText,Password}.tsx}}` · `apps/web/features/auth/{components/CredentialsForm.tsx,core/{validate,schema}.ts,core/validate.test.ts,types/mod.ts,styles/auth-controls.css}` · `apps/web/routes/api/auth/login.ts` · `packages/backend/services/auth/AuthBackendService{,.test}.ts` · Decisions #8 / #10 / #62 / #76 / #92 / #97 |
| 105 | **`/join` — the Enter chain: next field → next step → submission (2026-09-18). EXTENDS #104.** The wizard renders its steps in a plain `<div>`, not a `<form>`, so Enter had no implicit submission to fall back on and did NOTHING on every field of every step. One rule now, delegated from the step root (`core/enter-chain.ts` + `StepForm`): Enter on a field focuses the step's next field; on the last field it does what Continue does — the next step, or submission on the final one; on a choice card it picks the card (which auto-advances) or, once picked, advances. **It acts only on an Enter nothing else claimed** — `defaultPrevented` is the whole test — so a control that owns Enter keeps it, and two controls had to be made honest about when they claim it: `TagSelect` prevented Enter unconditionally and, on an EMPTY draft, committed `suggestions[0]` — the first unselected preset the reader never typed — so it now commits only a draft and lets an empty Enter through; and the segmented `DatePicker` opened its calendar on Enter even over a COMPLETE date, so that Enter now passes to the form (an incomplete date still opens the grid; Space opens it at any time — the component's key table and §C.1 say so). A segmented date is ONE unit of the chain across its three inputs (`resolveEnterMove`, pure and unit-pinned), so Enter on its day segment lands on the password, not the month. A select trigger is a DESTINATION (Enter on the field before it lands on it) but never a SOURCE — it is a `<button>`, and Enter on it opens the panel. **Focus follows the wizard:** a newly-entered step gets its first field (the PICKED card on a choice step, so a reader who stepped Back and presses Enter goes on rather than re-picks), and a step that refuses gets its first `aria-invalid` field wherever the reader was — an effect keyed on the step id and the errors object, so it runs after the commit and after the controls have written `aria-invalid`; the initial render is left alone. **Verified in-browser on Enter alone:** Individual card → client card → tag draft "Design" committed and stayed → empty Enter → identity → First/Last/Username (auto `ada.lovelace`) → credentials → Email → DoB day segment; `12` + Enter opened the calendar, `12051990` + Enter handed on to Password → Confirm; an empty Confirm refused and kept focus on it; the last Enter POSTed the full payload with `dob: "1990-05-12"`. Organisation: the scale step lands on the employees Select trigger, Enter there opens its panel, Enter on Website with both selects empty is refused and focuses the employees trigger (first invalid), and Street → City → Postcode → Enter lands on the Country trigger. No focus is stolen on load. `deno task check` + `deno lint` clean; unit suite green bar the four pre-existing `theme-engine` failures (#96(a)). **NOT verified, stated:** the OAuth branch (DoB folded into the identity step, which then submits) — same code path, not driven. **FLAGGED — surface, do not silently resolve:** (a) a `Select` in mid-chain still needs Tab to leave after a pick (Enter re-opens it) — the control's own contract, left alone; (b) a `ResizeObserver loop completed with undelivered notifications` console line appears when a Select panel opens on `/join`, the same benign line memory records on `/explore`, pre-existing and not from this change. | `DESIGN_SYSTEM.md` §C.1 (DatePicker) · `packages/ui/fields/{components/DateSegmentedInput,islands/DatePicker}.tsx` · `apps/web/features/auth/{core/enter-chain{,.test}.ts,components/{StepForm,TagSelect}.tsx}` · Decisions #8 / #9 / #99 / #104 |

| 106 | **Product cards and the product showcase — the work-tile register, the showcase aspect band, and one masonry on three surfaces (2026-09-18).** The discovery `ProductCard` is rebuilt in the register of the profile's Selected-work tile (`pf-tile`): a prominent picture, then a compact muted body — a two-line title at `--text-sm`, one `Owner · Category` line at `--text-xs` (§B.11 inline metadata, the owner a real anchor stacked above the stretched link, the category inert), and a foot with the compact rating and a one-line price — at `--space-1` on the shell, a step tighter than a service card, because a masonry column holds four of these where a services grid holds one. **(A) THE PICTURE GOVERNS THE TILE, INSIDE A BAND.** New SSOT leaf `packages/types/files/aspect.ts`: `MediaDimensionsSchema` (the `width` · `height` · `aspectRatio` triple the upload extractor already stores on `files.items.metadata` — no schema change was needed, the pipeline had measured every image since Decision #84(G)), `SHOWCASE_ASPECT_MIN = 4/5` / `MAX = 16/9`, `showcaseAspectOf()` (clamp + a `fit: "natural" | "cover"` verdict) and `showcaseAspectNote()`. `ProductItem` gains an optional `mediaMeta`, and `productMediaAspect(item)` is the ONE derivation the masonry card, the fixed rail card, the profile's work tile (`piecesFor` now calls it, retiring a private `3/4` that disagreed with the corpus's 4:5 crop) and the marketing twin all call — measured dimensions first, the `span`-derived crop as the fallback, always clamped. The card writes the resolved ratio as ONE custom property (`--ex-prod-ratio`) so `.ex-media--product { aspect-ratio: var(--ex-prod-ratio) }` never branches on the picture, and a picture outside the band is cropped into the nearer bound by the base `object-fit: cover` with `data-fit="cover"` on the card. The old content-height frame (bounded 8–18rem, then clipped — a bound expressed as a height, which changes with column width) is deleted. **(B) TWO LAYOUTS, ONE PICTURE.** `layout="masonry"` (the default) lets the tile follow its ratio; `layout="fixed"` takes the family's 16:10 frame so a product sits in a uniform-height rail beside services with one flat base. `EntityCard` forwards a `productLayout`; Explore Home's rails and Recommended panel, the grouped-results rail, "Continue where you left off" and the `/view` `RelatedCarousel` all pass `fixed`, and the rail-track selectors (`.ex-rail__track`, `.ex-group__rail`) enforce it structurally for a product that reaches a rail through a switch that did not. Measured: eight rail cards at exactly 266px with feet on one line and every frame 1.6; the `/view` carousel slide at 16:10. **(C) ONE MASONRY.** `ProductMasonryGrid` replaces `ProductsMasonry` and the feed's own `.ex-masonry--feed`: a CSS-column masonry whose column count is a CONTAINER query on a wrapper (the list cannot query itself — Decision #101's trap, hit again here and fixed before it shipped) and lives in one custom property, `--ex-pmasonry-cols` (1 · 2 · 3 · 4 at 20 / 40 / 60rem of the container, so it answers to the feed's real width beside the lane, not the viewport). `/explore?category=products` measured 4 columns at a 1,060px feed with 4 tiles on the first row and zero overflow. **(D) `/[handle]` — Products directly beneath Services.** A new read (`ProfileBackendService.products` → `findProfileProducts`, the services precedent, resolved by the layout so it renders on every section) mounts `ProfileProductsSection` under the Services row. The `ServicesRow` island is GENERALISED into `RowClamp` (`count` · `id` · `selector`), so both regions share one clamp and one disclosure: collapsed, the services grid zeroes its later rows as before, while the masonry — which has no rows to zero — is swapped to a grid of exactly `--ex-pmasonry-cols` cells (the SAME property, read on the SAME element, so "one row" means the same count in both presentations) with `align-items: start` so the first row keeps the ragged bottom a masonry's top actually has. Measured on `/@juno`: heading "Products" 64px under the Services region, collapsed = `display: grid` × 4 columns with 4 of 8 cells `data-hidden`, "Show all 8" → `column-count: 4` with 0 hidden and "Show less", zero `--primary` leaks inside the region, 2 columns at 390px, zero horizontal overflow LTR and RTL. **(E) THE PIPELINE FLAGS, NEVER REFUSES.** `applyMediaFacts` now runs `flagShowcaseAspect` — an image or video frame outside the band gets `showcaseAspectNote()` appended to `metadata.notes` (idempotent, never past the schema's cap of 8) and is stored anyway: an upload has no idea whether it will become a chat attachment or a product cover, so refusing it would refuse the wrong thing; the note is what lets a showcase surface say "this will be cropped" without re-deciding the rule, and the tile applies the crop. **No DB migration** (`files.items.metadata` already carries the triple) → no `documentation/database/*` change; no new `@projective/ui` primitive → no `DESIGN_SYSTEM.md` §C.1 change; no lifecycle change → no `PRODUCT_MANAGEMENT.md` change; no new simulatable axis (the layout is a prop the host sets and the ratio is DATA) → nothing to mirror in the Dev Context Switcher (§5). `PRODUCT_SPEC.md` §The Public Profile + a new §Assets & Attachments "The showcase band" updated in the same change. Verified: `deno task check` (1,624 files) + `deno lint` clean, **1,349 unit tests pass** (16 new — the band, the flag, the derivation) with only the four pre-existing `theme-engine` failures (#96(a)); in-browser on `/explore`, `/explore?category=products`, `/@juno` (1440 + 390, LTR + RTL) and `/view/sv-brand-identity-sprint`. **NOT verified, stated:** the LIVE upload-complete path (`FilesBackendService.uploadComplete` still falls through to the fixture store on both branches, so the flag is proven by unit test and the fixture round-trip, not against Postgres); `:focus-visible` rendering. **FLAGGED — surface, do not silently resolve:** (a) **the catalogue does not yet REFUSE an out-of-band cover at publish** — `ListingDetail.media` is a list of URLs, not asset ids, so `publishReadiness` has no dimensions to check; the refusal belongs there once the listing carries the asset row, and the flag on the row is what it will read. (b) A profile's re-owned product ids (`pr-{handle}-{i}`) do not resolve in `findItem`, so a profile product tile lands on the not-found branch of `/[handle]/view/[item]` — PRE-EXISTING for the re-owned services and work pieces (Decision #96), inherited here, and the fixture-only shape of a live-path question. (c) The marketing landing's twin (`marketing/components/ProductCard.tsx`) shares the anatomy and the derivation but its corpus carries no measured cover, so it always takes the span fallback — by construction, and said so in its docblock. (d) The band's two bounds are the brief's example values (4:5 / 16:9) promoted to constants; if the product owner wants a different band it is two literals in one file and one test. | root `CLAUDE.md` §1/§2/§3/§5/§7 · `PRODUCT_SPEC.md` §The Public Profile / §Assets & Attachments · `packages/types/files/{aspect,aspect.test,mod}.ts` · `packages/types/explore/{items,items.test}.ts` · `packages/backend/services/files/{media-facts,media-facts.test}.ts` · `packages/backend/services/explore/fixtures.ts` · `packages/backend/services/profile/{profile-fixtures,ProfileBackendService}.ts` · `apps/web/features/explore/{components/cards/{ProductCard,EntityCard},components/collections/ProductMasonryGrid,components/{ExploreHome,ResultsGroupRow},islands/{SearchDashboard,ContinueRail}.island,styles/{explore,explore-home,explore-results}.css}` · `apps/web/features/profile/{islands/RowClamp.island,components/{ProfileServicesSection,ProfileProductsSection},core/{profile-model,profile-ssr},styles/profile-services.css}` · `apps/web/features/view/islands/RelatedCarousel.island.tsx` · `apps/web/features/marketing/components/ProductCard.tsx` · `apps/web/routes/[handle]/_layout.tsx` · Decisions #39 / #41 / #67 / #74 / #76 / #84 / #96 / #97 / #100 / #101 |
| 107 | **Video playback controls — `@projective/ui/display` `VideoPlayer`, one surface for the profile hero, the Selected-work tile and the entity canvas (2026-09-18).** An audit of every `<video>` on `/[handle]` found three unrelated treatments: the hero showreel was a bare `<video muted>` driven imperatively by the carousel with one centred Replay glyph and no pause, mute, scrubber or volume; a Selected-work video piece was a zero-JS poster inside an `<a>` that could never play and whose markup could not host a control (a button inside a link is two interactive elements in one); and the canvas's video-preset preview was the browser's own `controls` — the one default-styled control set on the route. **(A) ONE COMPONENT, TWO SIZES.** New `VideoPlayer` (`display/islands/VideoPlayer.tsx`, the `AudioVisualizer` precedent, plus a narrow `./display/video` sub-path for the reason `./display/image` exists): `compact` is Play ⁄ Pause + Mute ⁄ Unmute as two 32px ghost buttons in a corner; `full` is a glass transport bar — play · mute + a volume slider that expands beside it on hover ⁄ focus · the clock (`0:34 / 2:04`, `tabular-nums`) · a seekable scrubber whose spoken value is a TIME (`0:34 of 2:04`), never a percentage · a speed cycle (`1× → 1.5× → 2×`) — plus a centred paused-state mark. **The `<video>` element is the single source of truth**: every control writes to it and every piece of UI mirrors its events, so the carousel keeps OWNING playback (play ⁄ pause ⁄ rewind through `videoRef`) and the bar agrees with no second channel. Every control stops propagation, so a press never follows a host's link nor starts the carousel's swipe. Autoplay is always muted; an unplayable source (`#`, absent) renders the poster and NO controls (§3 gate 11); a `children` slot layers a host's link between the picture and the controls. Every colour is the scrim pair on `::before` glass at the profile rail's measured 58% (§B.4.3 — arbitrary photography); the two fills are geometry that encodes a fact and are never transitioned. The pure rules — reveal, slider arithmetic incl. the RTL mirror, the clock — are `core/video.ts`, 10 tests. Two registry glyphs, `volume` and `volume-off` (the set's one slash); §C.1 + Part C + §D.8.0 ⁄ §D.8.5 ("a native player" → the `VideoPlayer`) and `PRODUCT_SPEC.md` §The Public Profile updated in the same change. **(B) REVEAL.** Hidden at rest; a mouse pointer over the player, KEYBOARD focus inside it, or a tap on the picture reveals — a second tap hides, and a reveal on a playing video withdraws after 2.5s. `full` also stands while the video is not playing (a paused frame reads as a still). A click on the picture toggles playback; on touch a tap while paused plays. **A pointer click focuses the button it lands on, and counting that pinned the cluster to a tile after every press** — found by measurement, so `:focus-visible` decides (`focusVisible: true` after Tab, the cluster hidden after a click-and-leave). **(C) THE TILE'S LINK MOVES INSIDE THE PLAYER.** The image tile stays a zero-JS anchor; a video piece renders the new `WorkVideoTile` island — a `div.pf-tile` holding the player, with the piece's link as the player's CHILD: a stretched anchor under the controls carrying the same scrim and caption, so the cluster fades in with the caption on the tile's own hover, and the box is reserved from the piece's aspect (`--pf-tile-ratio`). Where hover does not exist the cluster is pinned visible (`(hover: none)`), because a tile whose surface IS a link cannot spend its tap on a reveal — the caption's own rule. Nothing autoplays; `preload="none"`. The fixtures now mint one video piece in four (`pieceMediaFor`) and give the video-preset preview a real CC0 clip instead of `#`, so both branches are reachable from the stub. `useReducedMotion` extracted from the hero so the two islands cannot answer the question differently. **(D) THE HERO.** A deactivated slide is paused, rewound AND re-muted — a viewer who unmuted one slide has not asked the next lap to play sound at them. The bar sits above the pagination rail (`--video-bar-inset-block-end`, measured 8px clear, 12px insets) and the swipe now takes pointer capture only once the pointer has TRAVELLED 6px: capturing on the press retargeted the release to the viewport and the click a still pointer produces never reached the video surface. **TWO DEFECTS FOUND BY MEASUREMENT, ONE RACE IN TWO HALVES.** The element is server-rendered WITH its `src`, so an undecodable clip raised `error` BEFORE hydration attached a listener: the player kept drawing live controls for a dead video (`error.code 4`, state `paused`), and — because that error fired while slide one was active — the carousel never learned it and awaited a video that could never end. The mount effect now mirrors `video.error`, and the showcase remembers failed slides and treats arriving on one as finished (measured: state `failed`, 0 controls, active 1 → 2 after the dwell). Also: the unplayed track measured **1.89:1** against the bar over a white frame at a 32% ink; 60% measures **3.06:1** there, 7.37:1 over black — the played fill is then only 1.74:1 brighter than the rest on a white frame, which the handle and the clock carry redundantly. **VERIFIED IN-BROWSER** (headless Chromium over the mock server; the corpus clip is H.264, which the open-codec shell cannot decode, so it is answered with a range-capable VP8 WebM — without `Accept-Ranges` + 206 Chromium reports `seekable [0,0]` and every seek snaps to 0): at 1440 and 390, LTR + RTL, touch, reduced motion — every button 32×32 with a registry glyph and a `Tooltip`; unmute → `muted false`, `volume` glyph; ArrowDown ×5 → 75%; pause → centre mark, bar stands, carousel holds; End → `0:06 of 0:06`; a mid-track click → 50% with the slide unchanged; a quarter-from-left click under `dir="rtl"` → **75%** with the fill anchored right; speed → 1.5×; touch tap reveals ⁄ hides ⁄ auto-hides; the tile plays without following its link (URL unchanged), reveals with the caption, hides on leave, ends on Replay; keyboard Tab reaches Play, Shift+Tab draws the image tile's ring on the tile; the canvas player fills the 4:3 stage exactly with `contain` and 5 controls; zero horizontal overflow everywhere; `deno task check` + `deno lint` (1,632 files) clean, **1,360 unit tests** with only the four pre-existing `theme-engine` failures (#96(a)). **NOT verified, stated:** real H.264 playback and sound (the shell has neither the codec nor an audio sink), `:focus-visible` paint. **FLAGGED — surface, do not silently resolve:** (a) the played fill vs the unplayed track is 1.74:1 on a white frame by arithmetic — a white fill cannot clear 3:1 against a track that itself clears 3:1 against a 107-grey glass; (b) `EntityMedia` gallery items of `kind: "video"` still render as a still (their `src` is a poster URL and the schema carries no poster field) — the canvas's video control is the PRODUCT PREVIEW only; (c) `audio.ts` gained a `/// <reference lib="dom" />` so `formatClock` can be reached from a standalone test, the `grid-paint.ts` precedent; (d) `AudioVisualizer` still hand-authors its two `<svg>` glyphs against §B.7 — pre-existing, not touched. | `DESIGN_SYSTEM.md` §C.1 / Part C / §D.8.0 / §D.8.5 · `PRODUCT_SPEC.md` §The Public Profile · `packages/ui/display/{islands/VideoPlayer.tsx,core/{video,video.test,audio}.ts,styles/video-player.css,video.ts,mod.ts}` · `packages/ui/deno.json` · `packages/ui/icons/core/paths.tsx` · `apps/web/features/profile/{components/{ProfileShowcase,work/WorkMasonry},islands/{WorkVideoTile,ProfileHero}.island,hooks/useReducedMotion,styles/{profile-showcase,profile-work}.css}` · `apps/web/features/view/{islands/EntityCanvas.island,styles/entity-view.css}` · `packages/backend/services/{profile/profile-fixtures,explore/view-fixtures}.ts` · Decisions #34 / #39 / #62 / #92 / #96 / #101 |
| 108 | **Profile Hire and Add-to-project rebuilt — popovers at the control, the split modals, the intake SSOT, placeholder assignments, custom-start consultations and the type wizard (2026-09-19). EXTENDS Decisions #97 / #103.** The `/[handle]` seller rig's two transactions were a single popover holding the viewer's projects and a one-piece `HireInviteModal`; both are replaced. **(A) THE RIG IS DECIDED BY ONE PURE RULE**, `rigFor(kind, { hasServices, offersConsultation })`: a SELLER (freelancer · team) leads with **Hire** when it has a listing or a consultation to offer and otherwise lets **Add to project** lead; a BUYER keeps Message. The brief's literal Hire condition — "the viewer has active projects they may hire into" — is NOT what shipped and is flagged below. **(B) TWO POPOVERS, ANCHORED AT THE BUTTON, NEVER A SCROLL.** `HireMenu` lists the seller's listings (thumbnail · title · base price through `MoneyView` · compact rating) with a pinned **Book consultation** footer that renders ONLY when the seller offers one — price or "Free" — and is otherwise absent (a greyed row would imply a capability that does not exist, the #80(D) rule). `AddToProjectMenu` lists the viewer's open projects published first, drafts after, a persistent **Create new project** row, and the honest empty state; the rows are resolved SERVER-SIDE (`resolveHireProjects`, the `/projects` feed under the viewer's own actor with `involvement: "owner"`) so the first byte paints them. **(C) ONE SPLIT FRAME, TWO BODIES.** `SplitModal` (a `Dialog` over a wrapper-queried `.pf-splitscope` grid — a container cannot query itself, the #101 trap; DOM order kept below 40rem, never `order`, §D.7.8) hosts the **Service Detail modal** and the **Project Assignment modal**: LEFT the buyer's message and the seller's own requirements, RIGHT a read-only `EntityPreview` / `ProjectBriefPreview` in the `/view` vocabulary (cover · meta line · summary · seller line · stage ledger · checklist — zero cards, zero chips), FOOTER the price/rating/meta summary and exactly one filled primary whose label is the SSOT's own `resolveCta` (`Proceed to checkout` · `Add to projects` · `Open project →` · `Assign to project`). **(D) THE INTAKE IS A ZOD SSOT, NOT A FORM.** New leaf `@projective/types/services/intake.ts` — nine field kinds (text · textarea · number with an optional slider · boolean · select · multiselect · radio · pills · checkboxes), `intakeRefusal` as the ONE rule that decides whether an answer set is acceptable (required · too_long · not_a_number · below_min · above_max · unknown_option · too_many · wrong_shape, first refusal only), carried as `ServiceView.intake` on a listing and `ProfileView.hireIntake` on a profile, answered through `answers` on `SessionBookingInput` / `ServiceBriefInput` / `HireInvitation`, and enforced AGAIN by the fat services on the way in (422 `answers.<fieldId>`) so the modal's gate and the server's cannot drift. `IntakeFields` is a RENDERER that maps a kind onto the `@projective/ui` control that already exists for it. After a refused attempt the rule re-runs on every answer (§A.7.5: nothing paints at rest; submit force-reveals). **(E) A DRAFT TAKES A PLACEHOLDER, PRICED OR NOT.** `resolveHireOffer` is the one place the offer is resolved (a caller's figure → the stage's configured rate → nothing) and `placeholder` is true on ANY unpublished project — the seller is attached to the stages now and the terms are settled at publish, because there is no live engagement to invite anybody into yet — while `totalCents` still carries the sum where the figures exist. `hireInvitationRefusal` refuses an unpriced stage on a LIVE project and allows it on a draft; `MemberInvite.placeholder` records the outcome on the roster (an ATTRIBUTE, not a state — `PRODUCT_MANAGEMENT.md` §3.5). A staged draft with NO stages withholds the primary and says where the stages are made. **(F) THE CONSULTATION MODAL REUSES THE BOOKING RAIL AND ADDS A CUSTOM START.** `DateRail` + `SlotPicker` verbatim, a courtesy ⁄ paid switch where the seller offers both, a platform `SelectButton` restricted to `PublicCallOffer.platforms` (additive; `ConferencingProviderSchema` promoted from the `CONFERENCING_PROVIDERS` tuple), and — below the listed slots — a `TimeTumbler` for any minute inside the day's open bands with the END computed from the seller's configured duration. The pure pre-flight rule (`customStartRefusal` — inside_minimum_notice · beyond_booking_horizon · outside_call_window · taken) is the SSOT's and the server re-walks it (`resolveCustomStart`: notice, horizon, band, blackout, a buffered overlap with every taken slot), so `DiscoveryCallRequest` now carries EITHER a `slotId` or a `startsAt` (refined to exactly one) plus an optional `platform`, refused as `platform_not_offered` / `required`. `SlotGrid` gained `bands` and `bookableFrom` so the client can draw the window it is choosing inside. **(G) THE WIZARD** is two steps: three type cards with a prominent registry glyph — Task · One-off project · Pipeline — and the Quick-Init fields; a Task is `format: "one_off", hasStages: false` through the SAME `createFormatToColumns` the setup form reads back with (`CreateProject` gained `hasStages` and a plain-text `description` folded to the rich-text column through the new `plainTextToHtml`); on Create it mints the draft and redirects to `/projects/[slug]`. **(H) FOUR READS THAT DID NOT RESOLVE, all found by driving the surface:** a profile-scoped listing id (`sv-{handle}-{i}`, what the Services row mints) 404'd on `/api/explore/view`, on the slot grid and in the basket — `findReownedListing` now backs `findItem` and EVERY basket corpus lookup (eight direct `CORPUS.get` sites), closing Decision #106(b); a project minted through the stub create had NO roster, so its hire brief 404'd (`createdMemberRoster`: the creator as the sole client seat); and **"Add to projects" on a pipeline listing minted a draft the draft store remembered and every ordinary read forgot** — the board it navigated to rendered beside a lane reading "Project not found" (pre-existing since #80). `instantiateService` now also records the draft as a created project (`buildInstantiatedSetup`: the blueprint's stages at their floor prices in the listing's currency), `archiveDraft` archives that record too, and the feed, the lane, the setup page, the hire brief and the Add-to-project menu all resolve it. **(I) FOCUS IS NEVER DROPPED.** A refused attempt moves focus to the first marked control (`focusFirstRefused`, from an effect keyed on the attempt — the marks land on the NEXT render, the #105 ordering); a success moves it onto the Done action (`focusSuccessAction`, the #68 defect); the custom-time disclosure hands focus to the reel it reveals and back to the button it restores; and when a modal closes, the rig trigger that opened its popover takes focus back (`useReturnFocus`) — polled until the closing dialog has let go, because the Dialog stays mounted for its exit motion with its trap holding the background `inert` and a focus call on an inert element is silently ignored. **(J) MEASURED, NOT INSPECTED:** the placeholder path end to end (an unpriced pipeline draft: refusal → answer → "staged" → the roster's pending placeholder invite); the priced-draft path on an instantiated pipeline (£370.08 · Per ticket · 4 stages · Staged until published); the wizard round trip; the consultation round trip with a keyboard-driven custom start (9:15 refused as an overlap with a booked 9:20 slot, 10:00–10:20 accepted, Zoom, confirmed); a session listing booked from the profile into the basket (£141.73, Thu 23 Jul 15:00 BST); a pipeline listing instantiated from the profile and opened on a board whose lane resolves; light (worst text 5.13:1, the brand pair) and dark; `dir="rtl"` mirroring the split columns and the footer with zero horizontal overflow; 375px with the rig on one row, both popovers inside the viewport, the split modal stacking in DOM order and the wizard cards stacking, all with zero overflow. `deno task check` + `deno lint` (1,648 files) clean; **1,384 unit tests** (8 new in `hire-from-profile.test.ts`, plus the intake, consultation-model and hire suites) with only the four pre-existing `theme-engine.test.ts` failures (#96(a)). **No new Dev Context Switcher axis (§5) — a judgement:** the rig and the intake are DATA, the consultation offer re-reads the existing `callOffer` seam axis through `useBookingSeam`, guest ⁄ authed ⁄ owner are server-resolved, and nothing here reads a `data-dev-*` attribute that did not exist. **No new `@projective/ui` primitive** → no §C.1 change; the placeholder is an attribute, not a state → `PRODUCT_MANAGEMENT.md` §3.5 gained a note, not a row; **no DB migration** (every write lands in the per-process stores behind the existing gates; `Ticket`/`intake` shapes map onto columns that do not yet exist and are the live-path TODO). **FLAGGED — surface, do not silently resolve:** (a) **the brief's Hire condition was reinterpreted.** It gates Hire on the VIEWER ("has active projects where they have permissions to hire"), which describes the Add-to-project control's data, not a reason to hide the seller's listings; Hire is gated on what the SELLER offers — reverse this if the literal reading was meant. **Amended 2026-09-20 (product owner):** the two conversion buttons lost their `chevron-down` glyphs; **Add to project** carries a leading `plus` glyph and renders for a SIGNED-IN viewer only (`RigInput.authed` → `ProfileRig.addToProject`; its rows are the viewer's own projects, of which a guest has none); Hire still renders for a guest but its PRESS opens the `SignInPrompt` in place of the listings popover (`gate("hire")` before `api.toggle()`). Verified in-browser: guest → no Add-to-project, Hire → "Sign in to hire" dialog with `redirectTo=/@juno` and the popover closed; member → both buttons, the plus glyph at 16px, the popover opening with its rows; zero chevrons in either state. (b) **The "Hire hidden / Add-to-project leads" shape is unreachable from the stub corpus** — every seller fixture carries nine re-owned listings — and is pinned by `rigFor`'s unit tests only. (c) The pane's synthetic Enter ⁄ Space does NOT synthesise a native click on a focused `<button>` (a plain probe button counted 0 clicks), so `SelectButton`'s single-select Enter path is verified by its own contract, not in-browser. (d) A hover ⁄ typing burst in the pane can land a coordinate click on a different element than the ref it was resolved from once a dialog reflows — clicks in this pass were re-resolved by `find` after every layout change. (e) The authenticated shell paints its frame seam at `--shell-frame-inset-inline: 64px` on a 375px viewport where the rail is hidden — a pre-existing shell issue on every authenticated route at phone width, out of scope. (f) `Formats you need back` and the other multi-choice intake fields are seller-authored FIXTURES; the seller-side editor for a listing's intake is the catalogue's to build. (g) The instantiate route still carries no workspace beyond the payload's `workspaceId` and the draft store keys drafts per `(user, workspace)`; the created-project bridge reconstructs the same owner from those two, which is exactly what `actorFrom` produces for a personal context and a team one, and diverges only for a hand-built actor with an empty `contextId`. | `PRODUCT_SPEC.md` §The Public Profile · `PRODUCT_MANAGEMENT.md` §3.5 · `packages/types/services/{intake,intake.test,contact,booking}.ts` · `packages/types/scheduling/{meeting,calls,booking}.ts` · `packages/types/projects/{hire,hire.test,members,create}.ts` · `packages/types/profile/profile.ts` · `packages/types/explore/view.ts` · `packages/types/richtext/plain-text.ts` · `packages/backend/services/booking/{BookingBackendService,intake-fixtures,call-store}.ts` · `packages/backend/services/scheduling/{slot-fixtures,ScheduleBackendService}.ts` · `packages/backend/services/projects/{ProjectBackendService,write-store,draft-store,live-writes,hire-from-profile.test}.ts` · `packages/backend/services/{explore/{query,view-fixtures},profile/profile-fixtures,finance/basket-fixtures}.ts` · `apps/web/routes/api/{explore/view,services/{call-offer,slots}}.ts` · `apps/web/routes/[handle]/_layout.tsx` · `apps/web/features/profile/{islands/ProfileHero.island,components/hire/*,core/{profile-model,profile-ssr,consultation-model},styles/{profile-hero,profile-hire}.css}` · `apps/web/features/view/{components/EntityPreview,core/BookingService,islands/{ScopeBriefModal,SessionBookingModal}.island,styles/entity-view.css}` · `apps/web/features/{explore/core/ExploreService,projects/{core/HireService,components/QuickInitModal}}` · Decisions #68 / #80 / #97 / #101 / #103 / #105 / #106 |
| 109 | **The schema behind Decision #108 — intake columns, identity-addressed invitations, the call-platform allow-list, and the seller-side write contract (2026-09-19). COMPLETES #108.** An audit of the consolidated schema against every fact the profile Hire / Add-to-project / Consultation flows read or write found the DATABASE could not carry five of them, while everything else already had a home (a custom start → `discovery_calls.proposed_start/_end`; the platform → `provider_slug`; the agenda and requester zone; booking answers → `finance.basket_items.metadata`; `hasStages` → derived from `structure_variation`; slot bands → derived from `availability_rules`). **All five are folded in place (root §1 — zero `ALTER TABLE ADD COLUMN` in the diff), Zod and `documentation/database/*` updated in the same change, and VERIFIED BY EXECUTION**: a full `supabase db reset` (88 migrations + 6 seeds clean) and then every constraint, index and policy exercised inside `BEGIN … ROLLBACK` on the live local Postgres. **(A) The seller's questions have a column.** `marketplace.service_blueprints.intake_fields jsonb NOT NULL DEFAULT '[]'` + `ck_service_blueprints_intake_shape` (array, ≤ 12) — a jsonb DOCUMENT for the same reason `session_template_rules` is one (the seller authors it whole, every reader takes it whole, nothing queries across listings by field), with the element shape owned by `IntakeFieldSchema` and the database refusing only what no reader could interpret. Mirrored as `org.freelancer_profiles.hire_intake` and `org.teams.hire_intake` (a team is a seller too, #61) for the Project Assignment modal. `ListingDetail.intake` / `UpdateListingInput.intake` are the catalogue console's edit surface, and `serviceToDetail` derives the SAME `serviceIntakeFor` the `/view` listing reads, so the console edits the questions a buyer is actually asked rather than a second copy. **A contract test that reads the migrations** (`intake.contract.test.ts`, the `slug.contract.test.ts` technique) pins every column's SQL cap to `INTAKE_FIELDS_MAX` — confirmed to FAIL when the cap is set to 13, because a cap that drifted would not fail loudly anywhere else. **(B) `projects.project_invitations` is identity-addressed.** `target_email` relaxed to nullable beside a new `target_user_id` FK → `org.users_public`, with `ck_project_invitations_addressee` requiring EXACTLY one — a hire from a profile names a person the platform already knows, and addressing them by email would require the inviter to resolve an address `org.user_emails` withholds from everyone but its owner (#103(a)). Plus `message`, `offer_price_cents` (`CHECK >= 0`; NULL = the stage's configured rate — a caller-stated figure and an absent one are different claims), `answers jsonb` (`CHECK` object) and `placeholder` (derived server-side by `resolveHireOffer`, never trusted from a caller — a caller who could mark a live project's invitation "placeholder" could invite somebody onto a priced stage while stating no price). The invitee SELECT policy gains `OR target_user_id = auth.uid()`; `uq_project_invitations_open_seat` — partial unique on `(project_id, project_stage_id, target_user_id) NULLS NOT DISTINCT WHERE status = 'pending'` — stops a double-press stacking a second offer under the first, and `NULLS NOT DISTINCT` is load-bearing: without it every whole-project invitation (`project_stage_id` NULL) would be unique and the guard would hold for exactly the rows a task-priced engagement never writes. `live-members.ts` reads the new columns (`@handle` for an identity row, `placeholder`). **(C) `scheduling.call_platforms`** — the ordered allow-list of conferencing providers a host OFFERS, a join table with a real FK rather than a `text[]` so a slug naming no provider is unrepresentable; the public offer is the INTERSECTION with the host's active `conferencing` connections and an empty list means "every connected provider". RLS on, view/manage policies mirroring `call_settings`, grants (SELECT anon+authenticated). `CallPlatformSchema` / `UpdateCallPlatformsSchema`. **(D) `discovery_calls.service_blueprint_id`** (SET NULL) — the listing a call was booked ABOUT, NULL from a profile; `DiscoveryCallSchema.serviceBlueprintId`, and `call-store.ts`'s `subjectId` now names its column. **(E) The seller-side write contract is written down**: new `documentation/flows/ServiceCreation.md` states, per buyer surface, the column the service-creation / catalogue-editor pages must write, the validation each side owns, the live SQL, the field-id immutability rule (renaming an intake id after a sale orphans every stored answer), and the three open questions; `API_BACKLOG.md` points at it from its head. **Probes, all as designed:** a seeded blueprint takes an intake and refuses a scalar and a 13th field; both hire-intake CHECKs refuse a scalar; an identity-addressed invitation round-trips with offer/message/answers/placeholder; a second pending offer for the same seat collides, and so do two whole-project offers; both-addressees, neither, a negative price and array answers are each refused on the right constraint name; an email-addressed row still works; as `authenticated` the invitee sees exactly their 2 identity-addressed rows and not the email one, a stranger sees 0; `call_platforms` refuses a fake slug and a duplicate, a guest reads a published schedule's platforms in order, a stranger's INSERT is refused by RLS. `deno task check` 0 errors, `deno lint` clean, `deno task test:unit` **1,386 passed** (2 new) with only the four pre-existing `theme-engine.test.ts` failures (#96(a)). **Found, not a defect:** my probe used `google_meet`; the seeded slug and the Zod tuple both say `google` — the two vocabularies agree. **NOT VERIFIED, stated:** no live INSERT through the app's fat services (they remain stub-first behind their gates; the SQL was exercised directly); the catalogue console's intake EDITOR does not exist yet (the read/write shape does). **FLAGGED — surface, do not silently resolve:** (a) `token` is still `NOT NULL UNIQUE` on an identity-addressed invitation whose accept path could key on `target_user_id = auth.uid()` — keep minting one for a uniform accept path, or relax it; (b) `org.education_entries` / `experience_entries` / `user_languages` remain RLS-off under the blanket grant (#102(a)), untouched here; (c) the pre-existing `deno fmt` finding at `catalogue-fixtures.ts:80` (`mockAvatar(…)`) is untouched by this change and is not reformatted (#69(d)); (d) `git stash`/`pop` on this tree un-stages a staged deletion — re-stage with `git rm --cached`. | root `CLAUDE.md` §1/§6/§7 · `supabase/migrations/{00000011,00000014,00000015,00000022,00002001,00002011,00002015,00002520,00004003}*` · `documentation/database/{marketplace,org,projects,scheduling,finance}/Tables.md` · `documentation/database/{projects,scheduling}/Policies.md` · `documentation/flows/ServiceCreation.md` · `API_BACKLOG.md` · `packages/types/{scheduling/calls,catalogue/catalogue,services/intake.contract.test}.ts` · `packages/backend/services/{catalogue/catalogue-fixtures,projects/live-members,booking/call-store}.ts` · Decisions #61 / #67 / #69 / #83 / #88 / #102 / #103 / #108 |
| 110 | **Offline handling refactored — the navigation guard, the write guard, the interstitial, the Ribbon and the list-tail notice (2026-09-20).** The app already had the FOUNDATION and none of the behaviour: `utils/network.ts` (one shared `isOnline` signal, `navigator.onLine` trusted in the `false` direction only), `static/sw.js` (a runtime shell cache with an inline fallback page) and the project editor's IndexedDB write queue — while every other surface, offline, went native: a link fetched into the dark and landed on the SW's fallback, a write was sent and folded into "Network error", a stalled feed said nothing. Everything below sits ON that foundation; the SW fallback is kept as the floor for what no in-page code can see (a cold load, a programmatic `location.assign`, a native form post, a click an island stopped propagating). **(A) ONE ISLAND, TWO GUARDS.** `OfflineBridge` (`features/shell/islands/`, mounted from `_app.tsx` beside the other bridges) installs a delegated BUBBLE-phase `document` click listener and a reference-counted wrapper on `globalThis.fetch` (`utils/offline-guards.ts`), and the rules both apply are pure and unit-tested (`utils/offline.ts`, 11 tests). Not a `Link` component and not a new client, deliberately: ~1,500 anchors and a dozen transports, several still on raw `fetch`, and an adoption-based design is correct exactly where somebody remembered it. **Navigation:** only an unmodified, primary-button, same-tab, same-origin, non-`/api/`, non-`download`, non-fragment click is intercepted; the default is prevented SYNCHRONOUSLY (the cache lookup is async and a navigation cannot be un-started), then the shell cache is asked with the SAME two lookups `sw.js` makes — stored → the guard navigates itself; not stored → the interstitial. An island that already `preventDefault`ed (the Explore card, whose click opens the detail drawer) is honoured — bubble phase is what makes that knowable. **Writes:** a same-origin `POST`/`PUT`/`PATCH`/`DELETE` while offline is answered locally with the app's own `{ ok: false, message, code: "offline" }` envelope as a `503` + `x-pj-offline: 1` — a `Response`, not a rejection, because every transport folds a rejection into a generic sentence and forwards an envelope's `message` verbatim, so the envelope is what lets a surface say WHY. Reads pass through (their failure is the list's to report); cross-origin traffic (the signed storage `PUT`) keeps its own path; `keepalive`, `x-pj-background: 1` and `/api/logs` · `/api/telemetry` · `/api/analytics` are background and never refused. `apiFetch` repeats the check itself so the centralised client is right before the wrapper is installed, and no longer redirects to `/login` when a refresh could not be ASKED. **Nothing is replayed** — the only queue is the editor's own, which checks `isOnline` before it sends, because replaying a POST the guard did not understand is how a purchase happens twice. **(B) THE INTERSTITIAL** is the shared `Dialog` (`role="alertdialog"`, focus on the footer action, scroll locked, body-portalled) with the SW fallback's own copy word for word, on a NEW `backdropIntensity="heavy"` tier — `Backdrop intensity` → `--scrim-tint-heavy` (theme engine, dark 80 % / light 64 %) + `--scrim-blur-heavy` (18px). §B.10.2 said the scrim is not the caller's choice and it still is not: this is a TIER chosen by what the modal MEANS ("the page beneath cannot be used right now"), documented as the one exception, and a second modal reaching for it "for focus" is a finding. While the interstitial is open and the connection returns, a primary **Continue to page** appears — the reader asked to go somewhere and now can — and Stay steps down to outlined (§B.8.2 holds: one filled at a time). **(C) THE RIBBON** (`@projective/ui/feedback` `Ribbon`, new §C.1 row): a bottom-pinned strip, `--on-surface`-on-`--surface` inverted (≥ 14:1 in every theme state by construction; a severity fill was measured and rejected for the same ~3.2:1 light-mode reason the status chips mix toward `--on-surface`), controlled and network-agnostic; "You are offline…" while the connection is gone (assertive), "Back online." for 2.5 s after it returns (polite). **Amended 2026-09-20 (product owner): the strip is EMPTY by default.** As first shipped, the hidden strip stayed in the DOM carrying the check glyph and "Back online.", relying on `ribbon.css` to hide it — and that sheet rides the island bundle, so on every load the closed strip painted as a plain line reading "Back online." at the bottom of the page until the island's CSS applied (in dev, only once the module ran). Two independent fixes: the `Ribbon` box is always mounted (it is the `role="status"` live region, and assistive tech announces content ADDED to a region that already exists, not a region that appears with its text inside) but its content is presence-gated through `usePresence` — present only while visible or inside the `--dur-medium` exit grace, so the served HTML is an empty `<div … data-state="closed">` that paints nothing with no CSS at all; and the bridge derives the words from the LAST NOTICE (`lastNotice`, `null` until the first offline state), never from "not offline ⇒ Back online", so before an outage has been established the strip is handed no words whatever the phase machine does. Measured: the served `/explore` document contains zero occurrences of "Back online." (previously one on every page); at rest the strip has 0 children; offline → the sentence, assertive, 40px reserved; online → "Back online.", polite; +2.6 s closed and still carrying the words for the slide-out; +3.0 s empty. It reserves its room: `:root[data-ribbon="open"]` → `--ribbon-inset`, added by the middle-nav footer band, the three bottom toast anchors and the page tail to their own offsets — measured on `/wallet`: the money rig's bottom edge lands exactly on the strip's top edge, zero overflow. On a phone it stacks above `.ui-bottom-nav` (keyed on the nav's PRESENCE, so the focus chrome drops it back down) — and there the sentence WRAPS, which made the strip taller than `--ribbon-h` and put the rig's first row under it at 375px; so the island now writes its MEASURED height as an inline `--ribbon-inset` through a `ResizeObserver`, with the stylesheet minimum as the floor. `_app.tsx` pre-paints the attribute from `navigator.onLine` so a shell-cached page opened offline does not shift up under the reader on hydration, and the island seeds its phase from the browser on its first client render for the same reason. **(D) THE LIST-TAIL NOTICE** (`InlineNotice`, new §C.1 row — unboxed, meta register, one underlined `Button --link` action re-inked to the page's ink, §B.4): `utils/use-offline-stall.ts` is the ONE rule, wired into every infinite-scroll island (explore feed, chat feed at its TOP — that list loads upward, so its loading edge is the top —, file + submission explorers, files hub, wallet ledger, catalogue console): a next page that fails while the browser is offline appends "You are currently offline." + **Retry** at the loading edge and BLOCKS the sentinel so it cannot loop; Retry keeps the notice mounted and `aria-busy` for the whole attempt (clearing the stall first unmounted and re-mounted it 300 ms later — measured, a flicker that read as the page having done something); the offline→online edge retries on its own, because the sentinel is already in view and nothing else would ever fire again. Never the interstitial. The three click-driven loaders ("Show more" / "Load earlier") keep their own error line and button and only take the offline sentence. **Verified in-browser** (mock server, forced `navigator.onLine`): the ribbon's open/close, text, `aria-live` and reservation at 1280 and 375; a card click still opening its drawer (honoured `preventDefault`), a rail link opening the interstitial with the heavy scrim (`blur(18px)`, 80 % tint), focus on "Stay on this page", Continue navigating to `/wallet` once online; a `POST /api/wallet/action` refused with the envelope + one warning toast while a `keepalive` send and a `GET` passed through; the explore feed stalling with the notice, a failed retry keeping it (busy, disabled, dimmed), a good retry loading the page, and the reconnection auto-retry clearing it; `deno task check` (routes included) and `deno lint` clean, 559 package + utils tests passing with only the four pre-existing `theme-engine.test.ts` failures (#96(a)). **NOT verified, stated:** a genuinely severed connection (the harness cannot cut the network — offline was simulated by forcing `navigator.onLine` and rejecting the one read), the SW cache-hit branch (development unregisters the worker, so every offline navigation here took the interstitial), and `:focus-visible` paint. **FLAGGED — surface, do not silently resolve:** (a) the write guard REFUSES; it does not queue. A generic replay queue is a product decision about idempotency the transports do not yet carry (the basket's `idempotencyKey` is the one exception), and this pass declines to make it. (b) The DevTools log inspector also wraps `globalThis.fetch`; the two chain correctly in either order, but a third wrapper that captured the native function BEFORE either would bypass both — the same class as any global patch. (c) A click an island stops propagating (`stopPropagation`) is invisible to the guard by construction and lands on the SW fallback; none found today, but it is the one navigation the interstitial cannot own. (d) The `/api/explore/search` read was stubbed to fail in verification; the Explore corpus has 144 services, so a reconnection auto-retry loads every remaining page in one burst — the existing infinite-scroll behaviour, not this pass's. | root `CLAUDE.md` §2/§3/§7 · `SYSTEM_ARCHITECTURE.md` §Service Workers (Offline handling) · `DESIGN_SYSTEM.md` §B.10.2 / §C.1 / Part C · `packages/ui/styles/index.css` (`--scrim-*-heavy`, `--ribbon-h`/`--ribbon-inset`) · `packages/ui/system/core/theme-engine.ts` · `packages/ui/overlay/{islands/Backdrop.tsx,styles/backdrop.css}` · `packages/ui/feedback/{islands/{Dialog,Ribbon}.tsx,components/InlineNotice.tsx,styles/{ribbon,inline-notice,toast}.css,mod.ts}` · `packages/ui/navigation/styles/{middle-nav,page-canvas}.css` · `apps/web/utils/{offline,offline-state,offline-guards,use-offline-stall,api-client,network}.ts` + `offline{,-guards}.test.ts` · `apps/web/features/shell/{islands/OfflineBridge.island.tsx,styles/offline.css}` · `apps/web/routes/_app.tsx` · `apps/web/features/{explore/islands/SearchDashboard,projects/islands/{ChatFeed,FileExplorer,SubmissionExplorer},wallet/islands/LedgerTable,catalogue/islands/CatalogueScreen,files/islands/{FilesHub,DriveBrowser,AssetPicker}}.island.tsx` · `apps/web/features/messaging/components/PopoutChat.tsx` · `.claude/launch.json` · Decisions #14 / #39 / #46 / #50 / #60 / #63 / #67 / #92 / #96 |
| 111 | **Profile chrome follows the reader — the migrated header band returns with the rig, the tab bar floats as a glass pill, and the Explore tree gets one contextual Back (2026-09-20). PARTLY REVERSES Decision #96** ("no migrating sticky header") at the product owner's direction, and `PRODUCT_SPEC.md` §The Public Profile moves with it (root §0). **(A) THE BAND.** `ProfileStickyHeader` is threaded into the SAME two homes the entity view's band has — the authenticated frame's `ui-middle-nav__header` (the profile now registers a header, so `UserShell` renders the laneless `MiddleNav` frame, column 1 resolving to 0px) and the guest shell's floating `guest-shell__subheader` — on the `.pf-stickyhead` skeleton (load-bearing: the guest slab is keyed on that literal selector) and reveals via `max-block-size` as the hero's ACTION RIG scrolls under the band's pinned line. It carries the way back, the avatar, the name + crest, `@handle · Kind`, the live Available-now ⁄ Away badge + local time (the SAME `useMinuteClock` the context bar ticks on, extracted so two clocks cannot disagree), the rating as one reviews jump, and **the rig itself** — the ONE thing §D.7.6 withholds from the entity band, and the exception is structural: a profile has no body-side transactional block for the duty to transfer to, so once the rig scrolls away nothing else on the page can open the conversation or the hire flow. `ProfileRig` is ONE component rendered by the hero and the band; `core/rig-actions.ts` is ONE module of handlers; the four flows' modals are mounted ONCE in the hero on shared module signals (`pickedService` · `pickedProject` · `consultOpen` · `wizardOpen` · `liveConsultation` · `followCelebrating` · `rigStatus`) — written on the CLIENT only, never seeded from props, because a module signal is shared across every request a server process renders (the Decision #69 race). Focus RETURN follows whichever rig opened the flow (`flowOpener`, `hooks/useReturnFocus.ts`). Verified live: a listing picked from the band's popover opens the hero's service modal, closing it returns focus to the band's Hire; Add to project → the wizard; Follow flips both rigs and ONE live region announces; Message opens the floating messenger. **(B) THE PROBE'S HYSTERESIS IS THE BAND'S HEIGHT, and it is load-bearing.** In the authenticated frame the reveal GROWS the band's grid row and shifts the page beneath it (Chromium's scroll anchoring then compensates by +48 on `scrollY` — measured — but Safari has no anchoring), so a single threshold oscillates: rig crosses → band reveals → page shifts the rig back below → band collapses. `useCondenseProbe` therefore leaves the condensed state one band-height below where it entered it; the guest band is `position: fixed` and takes an 8px margin. The line is read off the band ELEMENT (48 authed · 88 guest · all-zeros where `display: none`), never assumed from a token. **(C) THE TAB BAR** (`ProfileTabs` is now an island) has two states in ONE box: in flow a transparent, sharp-cornered full-width strip closed by the single hairline the active underline lands on; stuck — `--space-2` under the band (`--pf-tabs-top`: 104px authed, 144px guest desktop, 76px guest phone where the sub-header is not rendered) — a floating pill of the tabs' own content width taking the band's exact glass (`--surface-1` at `--glass-veil-chrome`, `--glass-blur`, `--glass-saturate`, `--hairline`, on a `::before` underlay) + `--elevation-medium`, the strip passing pointer events through around it. Only `border-color`, `opacity` and `box-shadow` transition, so CLS is 0 by construction. CSS has no `:stuck`: a zero-height sentinel marks the natural position, the island compares it against the bar's own computed `top` (the offset lives in the stylesheet), and re-measures a beat after the band flips (that reveal moves the page with no scroll event). The dock wrapper is `display: contents` — a wrapper WITH a box would be the bar's sticky containing block and it could never pin. **(D) SECONDARY TEXT ON GLASS IS `--on-surface-variant`.** Measured at the 76% chrome veil over a white tile: `--text-secondary` 3.59:1 dark / 3.28:1 light (a black tile), the variant 4.83 / 4.77 — so the band's meta, facts, rating and Back circle, and the stuck pill's inactive tabs, take the variant; the entity band's own seller line still uses `--text-secondary` and is flagged below. **(E) THE EXPLORE TREE'S BACK** (`features/explore/core/explore-history.ts`, pure and unit-tested; `ExploreBackNav.island.tsx`; a global `ExploreHistoryTracker` in `_app.tsx`): a session-scoped stack of `pathname + search` for `/explore`, `/view/[id]` and the `/[handle]` namespace. A visit outside the tree records a BREAK marker rather than clearing (the browser's Back may land on the page before the excursion, whose chain is intact); arrivals are told apart by the Navigation Timing TYPE — `back_forward`, or the control's own pop marker, walks the stack back; a forward link pushes even to a page seen earlier; a page that re-writes its own URL (`/explore` refining, `?as=` on Reviews) is re-recorded at `pagehide`; a bfcache restore re-records on `pageshow`. The control is a real anchor to the SSR fallback (`backHrefFor`) until hydration, its visible word is always "Back" and the destination is its accessible name (a label that changed after hydration would be a correction; a fallback label kept after hydration would be a lie). It leaves through `history.back()` ONLY when the Navigation API says the previous entry IS the target — `document.referrer` is deliberately not a proxy, because the ticket deep link pushes entries on top of the page it opens on — else a real navigation. Placed in the profile hero's breadcrumb position, at the start of both bands, and repointing the entity view's three existing links (start rail, inline, not-found) and the article's. Verified live through the browser's own history: `/explore?category=services&q=brand` → `/@renkoda` → `/view/sv-renkoda-0` → Back (`back_forward`) → `/@renkoda` → Back (`back_forward`) → the search with `q=brand` intact and the stack unwound; an excursion to `/projects` made the next profile's Back the plain `/explore`. **(F) THE RAIL:** `isExploreSurface` (`shell/core/explore-surface.ts`, one predicate for the desktop rail AND the phone bottom bar) lights Explore on a `/[handle]` the acting context does not own — decided on the chrome `handle`; a skeleton token with none lights it on the viewer's own page, the cosmetic miss, where the reverse was the defect. **Verified** at 1280 / 390, authed + guest, light + dark, `dir="rtl"` (zero horizontal overflow); `deno task check` clean, `deno lint` clean, 1,423 unit tests (36 new: the stack, the labels, the surface predicate) with only the four pre-existing `theme-engine.test.ts` failures (#96(a)). **NOT verified, stated:** the pane fires neither `scroll` nor IntersectionObserver callbacks, so both probes were driven by dispatching `resize` (a listener they share) — the shipped scroll path is the same code the entity view runs; a screenshot of a scrolled emulated viewport captures a window offset by `scrollY` in this pane, so the stuck states were photographed by shortening the page rather than by scrolling to them. **No DB migration** (nothing persisted but `sessionStorage`), **no new `@projective/ui` primitive** (the island reuses `Tooltip` · `Icon` · `Avatar` · `RatingStars` · `Popover` · `Button`) → no §C.1 change; no lifecycle change → no `PRODUCT_MANAGEMENT.md`; no new simulatable axis a surface branches on (own vs foreign profile is server-resolved; the stack is data) → nothing to mirror in the Dev Context Switcher (§5). **FLAGGED — surface, do not silently resolve:** (a) a guest PHONE loses the rig once the hero scrolls away — `.guest-shell__subheader` is `display: none` ≤767px (Part D.5), so a signed-out phone reader has no persistent Hire/Message; the authenticated phone keeps the band. (b) The brief's literal `top: 0; z-index: 40` and `width: 100%` are not what shipped: the band pins under the shell's own chrome at the token line (`--shell-topbar-h` / `--site-header-h`) and the z-scale is the token layer's, per §3. (c) The brief names TWO glass references that disagree — the band's `--surface-1` at the 76% chrome veil with `saturate`, and `/messages/[id]/files`' `.fx-toolbar` at a literal `--surface` 62% with no saturate and a literal `50px` radius; the band's is the documented floor for text-bearing chrome and is what the pill takes, and `fx-toolbar.css` is now the drift the veil scale was created to close. (d) The entity band's seller line (`evp-stickyhead__seller`/`__kind`) still inks `--text-secondary` on the same glass — the same 3.59:1 miss, untouched here. (e) The band's reveal still shifts the authenticated page by 48px where scroll anchoring is absent — the shipped mechanism of #36/#79, not re-decided. (f) `Add to project` folds to its glyph ≤767px in the band by hiding its label under a stated `aria-label` — a text button's shell, not a §B.6 icon-only control; a future reader looking for the latter should know. | `PRODUCT_SPEC.md` §The Public Profile · `DESIGN_SYSTEM.md` §D.5 / §D.7.6 · `ROUTING.md` §Global routing rules · `apps/web/features/profile/{islands/{ProfileStickyHeader,ProfileTabs,ProfileHero,ProfileAvailability}.island,components/ProfileRig,core/{rig-actions,profile-state},hooks/{useCondenseProbe,useMinuteClock,useReturnFocus},styles/{profile-band,profile-rig,profile-tabs,profile-hero,profile}.css}` · `apps/web/features/explore/{core/{explore-history,explore-history.test,routing},islands/{ExploreBackNav,ExploreHistoryTracker}.island,styles/back-nav.css}` · `apps/web/features/shell/core/{explore-surface,nav-model,nav-model.test,bottom-nav-model}` · `apps/web/features/shell/components/UserShell.tsx` · `apps/web/features/view/{components/{EntityViewPage,ArticleViewScreen},islands/EntityStickyHeader.island,styles/entity-view.css}` · `apps/web/routes/{_app,[handle]/_layout}.tsx` · `apps/web/utils/storage-keys.ts` · Decisions #36 / #38 / #60 / #69 / #79 / #96 / #97 / #103 / #108 |
